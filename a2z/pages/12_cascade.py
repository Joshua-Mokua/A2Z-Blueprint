...
COUNT_KPIS = {
    ...
    "Cards Issued",
    "Bancassurance",           # number of policies sold (at branch level)
    "Loans Referred",
}
def is_count_kpi(k):
  ...
  with tabs[_tab_idx_bank_targets]:
    ...
    with st.expander("⏰ Configure master cascade timeline", expanded=not bool(existing_tl)):
        ...
        cascade_end = tl_c1.date_input(
            ...
            value=(_dt.date.fromisoformat(existing_tl["cascade_end_date"])
        ...
        for lvl in mgr_levels:
            ...
            default_conf = (_dt.date.fromisoformat(ex["confirm_by"])
                            if ex.get("confirm_by") else today+_dt.timedelta(days=5))
            default_casc = (_dt.date.fromisoformat(ex["cascade_by"])
    ...
    t_period = hc2.selectbox("Period",[_gfy(),"2025"],key="bank_period")
...
if _tab_visible_fixed_kpis:
  with tabs[_tab_idx_fixed_kpis]:
    if not is_md:
        ...
        fix_period = fp2.selectbox("Period", [_gfy(),"2025"], key="fix_period")
    ...
    if not tab_visible(ud,"set_targets") or not is_mgr:
      ...
      if len(d_reports) == 0 and my_rl != "Regional Head":
        ...
        h1.markdown(
            ...
            unsafe_allow_html=True)
        alloc_year = st.selectbox("Period", [_gfy_casc(), str(int(_gfy_casc())-1)] if _gfy_casc() else [_gfy(),"2025"], key="alloc_yr")
        ...
        max_d   = (_dt.date.fromisoformat(tl_now["cascade_end_date"])
...
👉 **To fix permanently:** go to Admin → Users → Edit this user → set Staff Code to `{_resolved_code}`
    ...
    if my_dl:
        today_d  = _dt.date.today()
        conf_due = _dt.date.fromisoformat(my_dl["confirm_by"])
        casc_due = _dt.date.fromisoformat(my_dl["cascade_by"])
    ...
    if my_given or locked_me:
        ...
        if not locked_me and is_mgr:
            if act2.button("🔽 Mark cascade complete", use_container_width=True):
                ...
                st.toast("✅ Cascade marked complete", icon="✅")
                st.rerun()
...
if _tab_visible_cascade_tree:
  with tabs[_tab_idx_cascade_tree]:
    ...
    t_per   = t2.selectbox("Period",[_gfy(),"2025"],key="tree_per")
