...
try:
    from utils.core import get_org_config as _goc_opex
    _org_opex  = _goc_opex()
    _bname     = _org_opex.get("bank_name","Your Bank")
    _bcode     = _org_opex.get("bank_code","YB")
except:
    _bname = "Your Bank"; _bcode = "YB"
...
st.markdown(
    "<div style=\'padding:16px 22px;background:#C0392B;border-radius:12px;margin-bottom:20px;box-shadow:0 2px 12px rgba(0,0,0,0.15)\'><div style=\'display:flex;align-items:center;justify-content:space-between\'><div><div style=\'color:var(--color-background-primary);font-size:16px;font-weight:700;letter-spacing:-0.2px\'>Operating Leverage & CIR Analysis</div><div style=\'color:rgba(255,255,255,0.65);font-size:11px;margin-top:3px;font-weight:400\'>Cost-to-income · Branch P&L · Industry benchmarking · Turnaround propositions</div></div><div style=\'opacity:0.12;font-size:36px;line-height:1;color:white\'>◆</div></div></div>",
    unsafe_allow_html=True)

# ── OPTIONAL DATA UPLOADS — shown inline at page top ─────────────────
with st.expander("📁 Upload additional data files (optional)", expanded=False):
    ...
    with up1:
        ...
        ind_file = st.file_uploader(
            ...
            help="39-bank Kenya CBK data for CIR, NIM, LDR benchmarking")
    with up2:
        st.markdown("**SBU profitability**")
        fin_file = st.file_uploader(
            "SBU_Profitability.xlsx",
            type=["xlsx","xls"], key="fin_upload",
            help="SBU P&L file for detailed profitability analysis")

# Also keep sidebar uploads for quick access
with st.sidebar:
    st.markdown("---")
    st.markdown("**Industry data**")
    ind_file_sb = st.file_uploader(
        "Industry_Financial_Review.xlsx",
        type=["xlsx","xls"], key="ind_upload_opex_sb",
        help="Upload for industry benchmarks")
    st.markdown("**SBU P&L data**")
    fin_file_sb = st.file_uploader(
        "SBU_Profitability.xlsx",
        type=["xlsx","xls"], key="fin_upload_sb",
        help="Upload for SBU profitability detail")

# Use whichever was uploaded (inline takes priority)
ind_raw = cache_upload(ind_file, "_ind_raw_bytes_opex") or cache_upload(ind_file_sb, "_ind_raw_bytes_opex_sb")
fin_raw = cache_upload(fin_file, "_fin_raw_bytes")      or cache_upload(fin_file_sb, "_fin_raw_bytes_sb")
        ...
        for metric, vals in ECO_BENCHMARKS.items():
            if metric in latest.columns:
                ...
                if len(col_data) > 5:
                    ...
                    eco_row = ind_df[ind_df['Bank'].str.contains(_bname, case=False, na=False)]
                    if len(eco_row):
    ...
    if not branch_pnl.empty:
        ...
        with fc1:
            fig_reg = px.bar(reg, x='Region', y=['Income','Opex','PBT'],
                              ...
                              color_discrete_map={'Income':'var(--brand-primary,#006B3F)','Opex':'#E24B4A','PBT':'#F5A623'})
        ...
        with fc2:
            ...
            fig_cir_reg.add_hline(y=80.6, line_dash='dot', line_color='#E24B4A',
                                   annotation_text=f'{_bname} current 80.6%')
        ...
        st.markdown(
            ...
            f"<b>💡 CIR gap insight:</b> {_bname}'s branch network CIR of <b>{blended_cir:.1f}%</b> "
                    ...
                    if len(target_row):
                        ...
                        if len(ach_row):
                            ...
                            for i, (sbu, ach) in enumerate(ach_vals.items()):
                                pct = ach * 100
                                clr = 'var(--brand-primary,#006B3F)' if pct >= 90 else ('#F5A623' if pct >= 60 else '#E24B4A')
                                ac_cols[i].markdown(
...
with tabs[2]:
    ...
    if branch_pnl.empty:
        ...
        pbt_colors = {
            ...
            '🟠 Below target': '#E67E22',
            '🟡 On track':     '#F5A623',
            '🟢 Exceeding':    'var(--brand-primary,#006B3F)',
        }
        ...
        if len(view_nonzero) > 0:
            ...
            fig_sc = px.scatter(
                ...
                color_discrete_map={'Central':'var(--brand-primary,#006B3F)','North':'#185FA5','South':'#F5A623'},
            ...
            fig_sc.add_vline(x=80.6, line_color='#E24B4A', line_dash='dot',
                              annotation_text=f'{_bname} avg 80.6%')
        ...
        def hl_pbt(v):
            try:
                ...
                if float(v) > 5: return 'color:var(--brand-primary,#006B3F);font-weight:600'
            except: pass
            return ''
        ...
        def hl_cir(v):
            try:
                ...
                if fv < 55:  return 'color:var(--brand-primary,#006B3F);font-weight:600'
            except: pass
            return ''
...
with tabs[3]:
    ...
    if not branch_pnl.empty:
        ...
        cc1.metric(f"{_bname} CIR",       f"{eco_cir:.1f}%",
        ...
        st.markdown(
            f"<div style='padding:12px 16px;background:var(--brand-light,#E8F5EE);"
            f"border-left:4px solid var(--brand-primary,#006B3F);border-radius:0 8px 8px 0;margin:8px 0'>"
        ...
        for threshold, color, label in [
            (55, 'var(--brand-primary,#006B3F)', 'Excellent <55%'),
            (65, '#F5A623', 'Target <65%'),
        ...
        with bc1:
            fig_band = px.pie(band_df, names='Band', values='Branches',
                               ...
                               color='Band',
                               color_discrete_map={
                                   '🟢 Excellent (<55%)':'var(--brand-primary,#006B3F)',
                                   '🟡 Good (55–65%)':'#F5A623',
                                   '🟠 High (65–80%)':'#E67E22',
                                   '🔴 Critical (>80%)':'#E24B4A',
                               })
...
# ════════════════════════════════════════════════════════════════
with tabs[4]:
    st.subheader(f"Industry benchmarking — {_bname} vs Kenya banking sector")
    st.caption(
        f"Comparing {_bname} against all 39 licensed commercial banks. "
    ...
    # ── BENCHMARK CARDS ──────────────────────────────────────
    st.markdown(f"#### Key ratio benchmarks — {_bname} vs industry")
    ...
    for i, (metric, vals) in enumerate(ECO_BENCHMARKS.items()):
        ...
        if lb:
            vs_med = eco - med
            vs_top = eco - topq
            clr = 'var(--brand-primary,#006B3F)' if eco < topq else ('#F5A623' if eco < med else '#E24B4A')
    ...
    if not ind_df.empty and 'Bank' in ind_df.columns and 'Period' in ind_df.columns:
        ...
        eco_row  = latest[latest['Bank'].str.contains(_bname, case=False, na=False)]
        ...
        if len(metric_data) > 3:
            ...
            # Highlight Ecobank
            metric_data['_is_eco'] = metric_data['Bank'].str.contains(_bname, case=False, na=False)
            ...
            fig_ind.add_vline(x=topq_v, line_dash='dot',
                               line_color='var(--brand-primary,#006B3F)', line_width=2,
            ...
            if eco_v is not None:
                ...
                st.markdown(
                    ...
                    f"border-left:4px solid var(--brand-primary,#006B3F);font-size:12px;margin:8px 0'>"
                    f"{_bname} ranks <b>#{rank} of {total}</b> banks on {sel_metric}: "
                    f"<b>{eco_v:.1f}</b> vs median <b>{median_v:.1f}</b> and top-quartile <b>{topq_v:.1f}</b>."
                    f"</div>", unsafe_allow_html=True)

        # ── TIME SERIES ──────────────────────────────────────
        st.markdown("---")
        st.markdown(f"#### {_bname} trend vs industry median — over time")
        eco_ts  = ind_df[ind_df['Bank'].str.contains(_bname, case=False, na=False)].copy()
        if not eco_ts.empty and sel_metric in eco_ts.columns:
            ...
            eco_ts2 = eco_ts[['Period', sel_metric]].rename(columns={sel_metric:_bname})
            merged_ts = eco_ts2.merge(ind_med_ts, on='Period', how='inner').dropna()

            if len(merged_ts) > 1:
                fig_ts = go.Figure()
                fig_ts.add_scatter(x=merged_ts['Period'], y=merged_ts[_bname],
                                    name=_bname, line=dict(color='#E24B4A', width=3),
                ...
                fig_ts.update_layout(
                    height=320, title=f'{sel_metric} — {_bname} vs industry median over time',
                    legend=dict(orientation='h', y=-0.2),
                    plot_bgcolor='rgba(0,0,0,0)', paper_bgcolor='rgba(0,0,0,0)')
        ...
        st.markdown(f"#### {_bname} vs industry — key gaps")
        gap_data = []
        for metric, vals in ECO_BENCHMARKS.items():
            lb = vals['lower_better']
        ...
        def hl_status(v):
            ...
            return 'color:var(--brand-primary,#006B3F);font-weight:500'
        st.dataframe(
        ...
        st.markdown(
            ...
            f"<b>💡 The LDR opportunity:</b> {_bname}'s LDR of 21.8% vs industry median of 63.2% "
...
with tabs[5]:
    ...
    if not branch_pnl.empty:
        ...
        for num, title, priority, desc in playbook:
            pclr = '#E24B4A' if '🔥' in priority else ('#F5A623' if '⚠️' in priority else 'var(--brand-primary,#006B3F)')
            st.markdown(
