...
try:
    from utils.core import get_org_config as _goc_comp
    OUR_BANK = _goc_comp().get('bank_name', 'Your Bank')
except:
    OUR_BANK = 'Your Bank'
ECO_GREEN = 'var(--brand-primary,#006B3F)'; ECO_GOLD = '#F5A623'
...
st.markdown(
    ...
    f"<div style='color:var(--color-background-primary);font-size:18px;font-weight:500'>"
    f"Kenya Banking Industry — Competitor Intelligence</div>"
    f"<div style='color:var(--brand-accent,#9FE1CB);font-size:12px;margin-top:3px'>"
    f"{OUR_BANK} vs 39 peers · Quarterly data 2021–2025 · "
...
if raw is None:
    st.markdown(
        "<div style='padding:32px;text-align:center;background:var(--brand-light,#E8F5EE);"
        "border-radius:12px;border:1px solid var(--brand-primary,#006B3F)44'>"
        "<div style='font-size:28px;margin-bottom:12px'>📊</div>"
        "<div style='font-size:16px;font-weight:500;color:var(--brand-primary,#006B3F)'>"
...
# ════════════════════════════════════════════════════════════════
with tabs[0]:
    st.markdown(f"#### {OUR_BANK} market position — {eco_period}")
    ...
    with pc1:
        ...
        plot_df['Is {OUR_BANK}'] = plot_df['Bank'] == OUR_BANK
            ...
            fig.add_scatter(
                ...
                marker=dict(size=max(8, row['PBT_abs']/1e8),
                            ...
                            line=dict(width=1.5 if row['Bank']==OUR_BANK else 0.5,
                                      color='black' if row['Bank']==OUR_BANK else 'var(--color-background-primary)')),
        ...
        # Legend markers
        for label, color in [(OUR_BANK,ECO_GREEN),('Beat list',ECO_GOLD),
    ...
    with pc2:
        ...
        if rank_rows:
            ...
            def hl_rank(v):
                ...
                return ''
            st.markdown(f"**{OUR_BANK} rankings vs industry**")
...
with tabs[1]:
    ...
    st.markdown("#### Performance heatmap — {OUR_BANK} + peer cluster")
    ...
    st.markdown("#### {OUR_BANK} vs size-tier peers")
...
with tabs[2]:
    ...
    with c1:
        ...
        st.caption("Banks just ahead of {OUR_BANK} — overtake these to move up the rankings.")
    ...
    with c2:
        ...
        st.caption("Banks below {OUR_BANK} that could close the gap if {OUR_BANK} loses momentum.")
        for _, r in watch_list.iterrows():
            ...
            threat   = '🔴 Growing faster than {OUR_BANK}' if faster else '🟢 Slower growth'
            ...
            st.markdown(
                f"<div style='padding:12px 14px;background:var(--color-background-secondary);"
                f"border-radius:8px;border-left:4px solid {'#E24B4A' if faster else 'var(--brand-mid,#1D9E75)'};"
                f"border-top:0.5px solid var(--color-border-tertiary);"
                f"border-right:0.5px solid var(--color-border-tertiary);"
                f"border-bottom:0.5px solid var(--color-border-tertiary);margin:4px 0'>"
    ...
    # Radar chart — {OUR_BANK} vs nearest rival
    st.markdown("---")
    st.markdown("#### Radar comparison — {OUR_BANK} vs selected peer")
    ...
    if sel_peer and radar_metrics:
        ...
        fig_r.update_layout(polar=dict(radialaxis=dict(visible=True,range=[0,1])),
                             height=380, title=f'{OUR_BANK} vs {sel_peer} (normalised, higher=better)',
...
# ════════════════════════════════════════════════════════════════
with tabs[3]:
    st.markdown("#### Growth trajectories — {OUR_BANK} vs industry")
    ...
    if len(ts_df):
        ...
        fig_ts = px.line(ts_df, x='Period', y='Value', color='Bank',
                         title=f'{growth_metric} trend — {OUR_BANK} vs peers (KES 000)',
    ...
    for bank in ts_banks:
        ...
        if len(vals) >= 5:
            ...
            growth_rows.append({
                ...
                'YoY Growth %':  f"{yoy:+.1f}%",
                'vs {OUR_BANK}':    '✅ Faster' if bank == OUR_BANK else
    ...
    # {OUR_BANK} own trend — all key metrics
    st.markdown("#### {OUR_BANK} quarterly trend — all key metrics")
    ...
    if trend_metrics:
        ...
        if trend_data:
            ...
            fig_tr = px.line(tr_df, x='Period', y='Value', color='Metric',
                             title='{OUR_BANK} ratio trends (own quarters)',
                             markers=True)
...
# ════════════════════════════════════════════════════════════════
with tabs[4]:
    st.markdown("#### What industry leaders do that {OUR_BANK} doesn't yet")
    ...
    def gap_card(title, eco_val, leader_val, leader_name, metric, unit='%',
                 lower_better=False, insight='', color='var(--brand-primary,#006B3F)'):
        ...
        clr   = 'var(--brand-primary,#006B3F)' if gap > 0 else '#E24B4A'
        st.markdown(
    ...
    with sg1:
        ...
        gap_card("ROE — Return on Equity",
                 ...
                 insight="Higher ROE signals better capital efficiency. {OUR_BANK} needs to grow PBT faster than equity to close this gap.",
                 color='#534AB7')
        ...
        gap_card("CIR — Cost-to-Income",
                 ...
                 lower_better=True,
                 insight="High CIR means {OUR_BANK} is less operationally efficient. Target is below 65%. Every 1% CIR reduction = direct PBT uplift.",
                 color='#E24B4A')

    with sg2:
        st.markdown("**Balance sheet & risk gaps**")
        # LDR gap — {OUR_BANK} loans very low
        ...
        if len(best_ldr_bank):
            ...
            gap_card("LDR — Loans to Deposits",
                     ...
                     insight=f"{OUR_BANK} LDR at {eco['LDR']:.1f}% is critically low. Industry best practice: 60-75%. Excess liquidity is unutilised earning capacity — each 1% LDR increase on {eco['Deposits']/1e9:.0f}B deposits = ~{eco['Deposits']*0.01*0.13/1e6:.0f}M KES in NII.",
        ...
        gap_card("NPL Ratio — Asset Quality",
                 ...
                 lower_better=True,
                 insight="Lower NPL = better asset quality. {OUR_BANK} NPL is relatively controlled — leverage this as a strength in pricing.",
                 color='var(--brand-mid,#1D9E75)')
    ...
    def hl_pri(v):
        ...
        if '🟢' in str(v): return 'color:var(--brand-primary,#006B3F)'
        return ''
...
with tabs[5]:
    ...
    st.markdown(
        f"<div style='padding:16px 20px;background:var(--brand-light,#E8F5EE);border-radius:10px;"
        f"border:1px solid var(--brand-primary,#006B3F)44;margin-bottom:16px'>"
        f"<div style='font-size:14px;font-weight:500;color:var(--brand-primary,#006B3F);margin-bottom:10px'>"
        f"Executive summary</div>"
        f"<div style='font-size:13px;line-height:1.7'>"
        f"{OUR_BANK} Kenya ranks <b>#{rank_eco('Assets')[0]}</b> by assets in a 39-bank industry with "
    ...
    with b2:
        ...
        for metric, val, bench, lb, note in concerns:
            ...
            clr = 'var(--brand-primary,#006B3F)' if ok else '#E24B4A'
            ico = '✅' if ok else '⚠️'
            st.markdown(
                f"<div style='display:flex;justify-content:space-between;align-items:center;"
    ...
    resolutions = [
        ...
        ("Competitive response", f"Note competitive pressure from {', '.join(watch_list['Bank'].tolist()[:3])} who are growing faster than {OUR_BANK}. Approve strategic response plan."),
