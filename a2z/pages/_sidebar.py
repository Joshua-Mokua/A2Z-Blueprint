...
from utils.core import (audit_log, process_kpi_data, build_staff_scores,
                         build_staff_registry, ExecuteManager)
...
def show_sidebar():
    ...
    with st.sidebar:
        ...
        elif "staff_scores" in st.session_state:
            n = len(st.session_state["staff_scores"])
            st.caption(f"✅ Data loaded — {n} staff")
...
def _process_upload(uploaded_file):
    """Process uploaded Excel — handles both multi-sheet and single-sheet formats."""
    try:
