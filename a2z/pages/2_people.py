...
with tabs[3]:
    ...
    if not lm or not lm.records:
        ...
        for _r in lm.records:
            try:
                _s = date.fromisoformat(str(_r.get("start_date",""))[:10])
                _e = date.fromisoformat(str(_r.get("end_date",""))[:10])
        ...
        if _mo_leaves:
            ...
            for _snm in _lc_staff:
                ...
                for _d in range(1, _days_in_month+1):
                    ...
                    _on = any(date.fromisoformat(str(l.get("start_date",""))[:10]) <= _dt <=
                              date.fromisoformat(str(l.get("end_date",""))[:10])
        ...
        _lc_st2.metric("Total leave days", sum(
            (min(_mo_end, date.fromisoformat(str(l.get("end_date",""))[:10])) -
             max(_mo_start, date.fromisoformat(str(l.get("start_date",""))[:10]))).days + 1
            for l in _mo_leaves
