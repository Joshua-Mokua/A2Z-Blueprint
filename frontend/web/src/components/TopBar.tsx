...
const ROUTES: RouteEntry[] = [
  ...
  { label: 'A2Z Sales Pro',       domain: 'Business Development',    path: '/pipeline',         match: (p) => p.startsWith('/pipeline') && !p.startsWith('/pipeline/queues') },
  { label: 'A2Z Sales Pro Analytics', domain: 'Business Development', path: '/analytics',        match: (p) => p.startsWith('/analytics') },
  { label: 'Manager Queues',      domain: 'Business Development',    path: '/pipeline/queues',  match: (p) => p.startsWith('/pipeline/queues') },
  { label: 'A2Z Sales Referral',  domain: 'Business Development',    path: '/referrals',        match: (p) => p.startsWith('/referrals') },
