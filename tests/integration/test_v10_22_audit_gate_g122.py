...
class TestV1022G122GateRegistered(unittest.TestCase):
    ...
    def test_total_gate_count_is_122(self):
        """At v10.22 closure there were 122 gates. Future arc closures
        (v10.27 added G123) grow the count, so we assert ≥122."""
        from scripts.audit import GATES
        self.assertGreaterEqual(
            len(GATES), 122,
            f"Expected ≥122 gates (122 at v10.22 closure); got {len(GATES)}")
...
class TestV1022MasterPromptVersion(unittest.TestCase):
    def test_master_prompt_at_v10_22_or_later(self):
        """At v10.22 closure the master prompt was stamped v10.22.
        Future batches bump it (v10.23+); accept any v10.22+ stamp."""
        import re
        content = Path("Master_Prompt_v3.md").read_text(encoding="utf-8")
        # Find any v10.NN stamp
        matches = re.findall(r"v10\.(\d+)", content)
        self.assertTrue(matches,
                          "master prompt has no v10.X stamp")
        max_minor = max(int(m) for m in matches)
        self.assertGreaterEqual(
            max_minor, 22,
            f"master prompt latest v10.{max_minor} is below v10.22")
