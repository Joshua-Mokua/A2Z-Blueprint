"""pages/15_strategy_arc_cockpit.py — Strategy Arc Cockpit (v10.141).

Locks the v10.46 Lean+Compact protocol amendment for the Strategy
module closure (8th closed arc). This page makes all 15 v10.135-v10.140
engines (ENH-141..155) operator-driveable from the browser.

This cockpit fulfills the doc-spec pages from Continuation.docx:
- pages/150_strategy_dashboard.py (Standard #150) → Tab 3 Health
- pages/151_sto_toolkit.py (Standard #154) → Tab 6 STO
...
reasons when data missing.

Per Rule 7, every engine surfaces honestly:
- ENH-148 returns "deferred" stubs when AI hooks not injected
- ENH-149 returns score=None level="no_data" when no pulse responses
- ENH-150 weights re-normalize transparently when components missing
- ENH-151 returns "Insufficient data" when baseline missing
- ENH-152 explicit DELIVERY_PREPARED when no adapter (does NOT pretend
  messages were sent)
- ENH-154 read-only contract — never writes to performance.* tables
- ENH-155 indirect benefits LABELED is_estimate=True with ±20% band

This cockpit reads engines, never writes — strategy data is created
through the existing 83_strategy.py initiative-management page.

This module also has React-ready FastAPI endpoints in utils/api_strategy.py
exposing each engine's main method via JSON-serializable HTTP endpoints
(JWT auth via get_current_user). The cockpit and API call the same
engine layer — engine is source of truth for both.
"""
...
st.markdown(
    ...
    "STRATEGY ARC · LIVE COCKPIT · v10.141</div>"
    "<div style='font-size:30px;font-weight:800;margin-top:8px'>"
    "🎯 Strategy Module — 15/15 Standards Active</div>"
    "<div style='font-size:14px;opacity:0.95;margin-top:6px'>"
    "ENH-141 through ENH-155 · Phase 1 Strategy CLOSED · "
    "G145 closure gate locked · G146 UI integration ratchet</div>"
    "</div>",
    unsafe_allow_html=True)


# ══════════════════════════════════════════════════════════════════════
# Module summary metrics (top strip)
# ══════════════════════════════════════════════════════════════════════

@st.cache_data(ttl=60)
def _module_summary():
    """Lightweight summary read for header metrics."""
    tk = STOToolkit()
    ...
    return {
        ...
        "rag":            portfolio.get("rag_distribution", {}),
        "n_risks":        risks.get("n_risks", 0),
        "by_level":       risks.get("by_level", {}),
    }


_summary = _module_summary()
m1, m2, m3, m4, m5 = st.columns(5)
m1.metric("Active engines", "15/15")
m2.metric("Initiatives", _summary["n_initiatives"])
m3.metric("Completion", f"{_summary.get('completion_rate', 0)}%")
m4.metric("Strategy risks", _summary["n_risks"])
m5.metric("HIGH-risk pillars",
          _summary["by_level"].get("HIGH", 0),
          delta_color="inverse")


# ══════════════════════════════════════════════════════════════════════
# Tabs covering 15 engines across 7 lifecycle phases
# ══════════════════════════════════════════════════════════════════════

tab_form, tab_cascade, tab_health, tab_exec, tab_learn, tab_sto, tab_roi = (
    st.tabs([
        "🎯 Formulation",       # ENH-141/142/143/144
        "📊 Cascade",            # ENH-145/153
        "📈 Health",             # ENH-150
        "🔍 Execution",          # ENH-146/147/151
        "🧠 Learning",           # ENH-148/149/152
        "🏢 STO",                # ENH-154
        "💰 ROI",                # ENH-155
    ]))
...
# ══════════════════════════════════════════════════════════════════════
# Tab 1 — Formulation (ENH-141/142/143/144)
# ══════════════════════════════════════════════════════════════════════
