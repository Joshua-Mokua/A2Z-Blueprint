...
_prop_tag = get_user_proposition()
if _prop_tag:
    live_deals = [d for d in live_deals if d.get("proposition_tag") == _prop_tag]
    try:
        import json as _pj; from pathlib import Path as _pp
        _pcfg2 = _pj.loads((_pp(__file__).parent.parent/"data"/"proposition_config.json").read_text())
        _pname = _pcfg2["propositions"].get(_prop_tag,{}).get("name", _prop_tag)
        _picon = _pcfg2["propositions"].get(_prop_tag,{}).get("icon", "🎯")
        st.info(f"{_picon} **{_pname} view** — {len(live_deals)} proposition-tagged deals")
    except Exception: pass

# ── Month-end pipeline review prompt ─────────────────────────
import calendar as _cal_p
_last_d = _cal_p.monthrange(date.today().year,date.today().month)[1]
_dleft  = _last_d - date.today().day
if _dleft <= 3:
    _stale_p=[d for d in live_deals if d.get("stage") in ACTIVE_STAGES and
                  d.get("last_updated") and 
                  (date.today()-_safe_date(str(d["last_updated"])[:10])).days>7]
    if _stale_p:
        st.error(f"🔴 Month-end ({_dleft}d left) — {len(_stale_p)} deals not updated in 7+ days")
    else:
        st.success(f"✅ Pipeline up to date — {_dleft}d to month end")
...
from datetime import date as _dt_pl
...
_today_pl = str(_dt_pl.today())
_overdue_pl = [d for d in active if any(
    str(a.get("due_date","9999-12-31")) < _today_pl
    for a in (d.get("actions_due") or []))]
if _overdue_pl:
    st.error(f"🔴 **{len(_overdue_pl)} deal(s)** have overdue action items — review highlighted deals below")
    ...
    with st.form(_fk, clear_on_submit=False):
        ...
        if _is_biz:
            sector = r1c.selectbox("Industry / CBK sector",
                ["— Select —"]+CBK_SECTORS, key="pl_sector_new")
        else:
            sector = r1c.selectbox("Occupation / profile",
                ["— Select —"]+INDIVIDUAL_SECTOR_LIST, key="pl_sector_new2")
        ...
        if _is_biz and not is_ntb:
            ...
            _pos_sel = r2c.selectbox("Contact title", CONTACT_POSITIONS, key="pl_pos_sel_new")
            ...
            contact_person = r3a.text_input("Contact person *",
                placeholder="e.g. James Mwangi",
                value=st.session_state.get("_pf_contact",""))
            _pos_sel = r3b.selectbox("Contact title", CONTACT_POSITIONS, key="pl_pos_sel_new2")
            contact_position = _pos_sel if _pos_sel not in ("— Select position —","Other (specify below)") else ""
            decision_level = r3c.selectbox("Decision level", DECISION_LEVELS, key="pl_decision_level_new")
        else:
            biz_name=""; contact_person=""; contact_position=""

        if _is_biz and not is_ntb:
            decision_level = r2a.selectbox("Decision level", DECISION_LEVELS, key="pl_decision_level_new2")
        elif not _is_biz:
            decision_level = ""
                ...
                if _sd:
                    ...
                    if _can_edit:
                        ...
                        _nnd = _e1.date_input("Next action date",
                            value=_safe_date(_sd["next_action_date"]) if _sd.get("next_action_date") else date.today(),
                            key="pl_qnd")
                    else:
                    ...
                    if _ub1.button("💾 Save changes", type="primary", key="pl_qsave",
                        ...
                        if _note.strip() or _nst!=_sd["stage"]:
                            ...
                            if _nst in _LMS_STAGES and _nst != _sd.get("stage"):
                                try:
                                    _lam = _LAM()
                                    _existing = next(
                                        (a for a in _lam.apps
                                         if a.get("pipeline_deal_id") == _sd["id"]),
                                        None)
                                    if not _existing:
                                        from datetime import date as _dt
                                        _lam.apps.append({
                                            "id": f"LMS{str(len(_lam.apps)+1).zfill(5)}",
                                            "pipeline_deal_id": _sd["id"],
                                            "client_name":  _sd.get("client_name",""),
                                            "client_cif":   "",
                                            "product":      _sd.get("product",""),
                                            "amount":       float(_sd.get("deal_value",0)),
                                            "currency":     "KES",
                                            "swim_lane":    (
                                                "Express"  if float(_sd.get("deal_value",0)) <= 5_000_000
                                                else "Complex" if float(_sd.get("deal_value",0)) >= 100_000_000
                                                else "Standard"),
                                            "status":       "submitted",
                                            "application_date": str(_dt.today()),
                                            "rm_code":      str(_sd.get("staff_code","")),
                                            "rm_name":      _sd.get("staff_name",""),
                                            "rm_unit":      _sd.get("unit",""),
                                            "analyst":      None,
...
with tabs[2]:
        ...
        for _dr in _my_drafts:
            with st.expander(f"📝 {_dr.get('client_name','—')} — {_dr.get('product_type','—')} (draft)"):
                ...
                _dnd_edit = st.date_input("Next action date",
                    value=_safe_date(_dr["next_action_date"]) if _dr.get("next_action_date") and _dr["next_action_date"] != str(date.today()+timedelta(days=7)) else date.today()+timedelta(days=3),
    ...
    if overdue:
        for _d in sorted(overdue, key=lambda x: x.get("next_action_date","")):
            _dl = (date.today()-_safe_date(_d["next_action_date"])).days
            st.markdown(
                f"<div class='alert-banner alert-red'>"
                f"<div style='flex:1'>"
...
# ── RM League Table ─────────────────────────────────────────────────
with st.expander("🏆 RM Pipeline League Table", expanded=False):
    ...
    for d in all_deals:
        ...
        if d.get("stage")=="Closed Won":
            _rm_totals[rm]["won"] += 1
            _rm_totals[rm]["won_val"] += float(d.get("amount",0))
    _lt_rows = [{"RM":rm[:25],"Active Deals":v["deals"],
                  "Pipeline Value (M)":round(v["value"]/1e6,1),
                  "Won Deals":v["won"],"Won Value (M)":round(v["won_val"]/1e6,1),
                  "Win Rate":f"{v['won']/max(v['deals'],1)*100:.0f}%"}
                 for rm,v in sorted(_rm_totals.items(),key=lambda x:-x[1]["value"])
                 if rm and rm!="Unknown"][:20]
    if _lt_rows:
        st.dataframe(_pd_lt.DataFrame(_lt_rows), use_container_width=True, hide_index=True)
