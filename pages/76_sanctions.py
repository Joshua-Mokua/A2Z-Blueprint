...
def _load():
    p = DATA/"sanctions_register.json"
    return json.loads(p.read_text(encoding="utf-8")) if p.exists() else []

@st.cache_data(ttl=60)
def _cfg():
    mc = DATA/"module_config.json"
    return json.loads(mc.read_text(encoding="utf-8")).get("sanctions_screening",{}) if mc.exists() else {}

def _save(data):
    (DATA/"sanctions_register.json").write_text(json.dumps(data,indent=2))
    st.cache_data.clear()
