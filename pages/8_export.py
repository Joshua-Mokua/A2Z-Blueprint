...
st.markdown(
    ...
    "<span style='font-size:22px;font-weight:800'>📥 Export</span>"
    "<span style='font-size:13px;color:var(--color-text-secondary);margin-left:12px'>"
    "Data extracts · Regulatory reports</span></div>",
...
DATA8 = _P8(__file__).parent.parent / "data"

st.markdown("**Available exports:**")
export_opts = {
    "BSC Scores (all staff)":       ("feb_2026_staff_scores.json", "bsc_scores"),
    "Loan Applications":            ("loan_applications.json",     "loan_apps"),
    "Pipeline Deals":               ("pipeline.json",              "pipeline"),
    "Revenue Assurance":            ("revenue_assurance.json",     "revenue_assurance"),
    "RMS Reconciliations":          ("rms_reconciliations.json",   "rms"),
    "IFRS 9 — Loan Staging":        ("ifrs9_loans.json",           "ifrs9_loans"),
    "Campaigns":                    ("campaigns.json",              "campaigns"),
}

sel_export = st.selectbox("Select dataset to export", list(export_opts.keys()), key="exp_sel")
...
if st.button("📥 Generate Excel export", type="primary", key="exp_btn"):
    ...
    try:
        ...
        buf.seek(0)
        st.download_button(
            f"📥 Download {sel_export}",
            data=buf.getvalue(),
            file_name=f"{sheet}_{__import__('datetime').date.today()}.xlsx",
            mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
            key="exp_dl")
