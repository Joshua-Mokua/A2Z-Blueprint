"""pages/122_chief_finance_centre.py — Chief Finance — 360 Command Centre.

Per Joshua v10.461: "let us bring in" more organs. Chief Finance Officer
covers BOTH finance and treasury per Joshua doctrine. Mirrors the proven Chief Credit / Chief HR /
Chief ICT centre pattern with 6 doctrine tabs (CC1-CC7):
  CC1. Page exists
  CC2. Executive Visibility (st.metric widgets - Circulatory & Energy Distribution)
  CC3. Strategic Intelligence (trend + forecast)
  CC4. Organ Health Monitoring (finance doctrine health)
  CC5. My Staff Performance (Chief Finance Officer sees staff BSC + cascade)
...
try:
    ...
    require_access(['Chief Finance Officer', 'Head of Finance', 'Financial Controller', 'Finance Super User', 'MD', 'Admin', 'Admin Super User'])
except Exception:
    pass
...
try:
    ...
    _flexcube_plan = declare_flexcube_ready(
        'finance', ['customer', 'deposits', 'treasury', 'risk', 'branch']
    )
...
try:
    ...
    _stress_suite = run_full_stress_suite('finance')
    _benchmark = benchmark_module('finance')
    _scale_readiness = validate_horizontal_scale('finance')
    _capacity_plan_5y = generate_capacity_plan('finance', "year_5_5x")
except Exception:
    _stress_suite = []
    _benchmark = None
...
try:
    ...
    _super_user = get_super_user('finance')
    _escalation_path = get_escalation_path('finance')
    _workload = workload_balance('finance', queue_depth=42, in_flight=11)
    _t0 = perf_timer()
    track_page('122_chief_finance_centre.py')
except Exception:
    _super_user = None
    _escalation_path = []
    _workload = None
...
st.markdown(
    ...
    "<span style='font-size:22px;font-weight:800'>🏛️ Chief Finance — 360 Command Centre</span>"
    "<span style='font-size:13px;color:var(--color-text-secondary);margin-left:12px'>"
    f"As of {TODAY:%d %b %Y} · Real-time panoramic surface · "
    "Chief Finance Officer parity with CCO/CHRO views · "
...
with tabs[0]:
    st.subheader("🎯 Executive Visibility — Chief Finance Officer Overview")
    st.caption("Real-time KPIs · Circulatory & Energy Distribution")

    c1, c2, c3, c4 = st.columns(4)
    c1.metric('YTD revenue (KES)', '11.8B', delta='+4.2%', help='K001 revenue YTD vs budget')
    c2.metric('Cost-to-income', '53.4%', delta='-1.1pp', help='K002 efficiency ratio (lower better)')
    c3.metric('PBT margin', '26.7%', delta='+0.8pp', help='K003 profit before tax / revenue')
    c4.metric('⚠️ Budget variances >5%', '3', delta='-1', help='K004 lines requiring review')

    st.markdown("---")
    st.markdown("### Financial systems footprint")

    infra = pd.DataFrame([
        {"System": 'General Ledger', "Status": '✅', "Count": 1, "Notes": 'Flexcube CBS sync daily'},
        {"System": 'Sub-ledgers (4)', "Status": '✅', "Count": 4, "Notes": 'Reconciled to GL nightly'},
        {"System": 'Budget & Planning', "Status": '✅', "Count": 1, "Notes": 'Year-end + 3-yr horizon'},
        {"System": 'Treasury sub-system', "Status": '✅', "Count": 1, "Notes": 'See Head of Treasury Centre'},
    ])
...
with tabs[1]:
    ...
    st.caption("Live trend analysis + forecasts for Chief Finance Officer")

    st.markdown("### Monthly revenue trend (KES B)")
    trend = pd.DataFrame({
        "Month": ["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"],
        "Actual": list([0.92, 0.95, 0.98, 1.02, 1.05, 1.08, 1.01, 1.04, 1.07, 1.1, 1.13, 1.16]),
        "Forecast": list([None, None, None, None, None, None, None, None, None, 1.15, 1.18, 1.21]),
    })
...
with tabs[2]:
    st.subheader("❤️ Organ Health Monitoring — FINANCE Doctrine")
...
with tabs[3]:
    ...
    st.caption(
        "Chief Finance Officer sees department staff BSC scores + "
    ...
    try:
        ...
        dept_keywords = ['chief finance', 'finance manager', 'accountant', 'controller', 'tax officer', 'accounts payable', 'accounts receivable', 'treasury', 'cfo', 'head of treasury', 'dealer', 'alm']
        dept_staff = []
        for u in users:
        ...
        st.markdown("---")
        st.markdown("##### Cascade alignment for Chief Finance Officer roles")
        try:
            tc = json.loads((DATA / "target_cascade.json").read_text(encoding="utf-8"))
            cascade_text = json.dumps(tc)
            expected = ['Chief Finance Officer', 'Head of Finance', 'Finance Manager', 'Senior Accountant', 'Accountant', 'Financial Controller', 'Tax Officer', 'Head of Treasury', 'Treasury Manager', 'Senior Dealer', 'Dealer']
...
with tabs[4]:
    ...
    risks = pd.DataFrame([
        {"Indicator": 'Overspend lines', "Threshold": 'Budget vs actual >10%', "Current": '3', "Status": '🔴 Critical', "Action": 'Owner review'},
        {"Indicator": 'Aged receivables', "Threshold": '>90 days outstanding', "Current": '47 accounts', "Status": '⚠️ Warning', "Action": 'Recovery in progress'},
    ])
    st.markdown("### Budget & financial risk indicators")
...
with tabs[5]:
    ...
    activity = pd.DataFrame([
        {"Time": '14:32:15', "Event": 'Daily close started', "Component": 'GL Engine', "Result": '✅'},
        {"Time": '14:31:48', "Event": 'Accruals posted', "Component": 'accruals_synthesizer', "Result": '✅'},
        {"Time": '14:30:55', "Event": 'Operating segments computed', "Component": 'operating_segments', "Result": '✅'},
        {"Time": '14:30:31', "Event": 'Flexcube fetch (deposits)', "Component": 'flexcube-adapter', "Result": '✅'},
    ])
...
# v10.468 — Phase 5 standards wiring for finance organ
# Imports unwired_standalone engines so they're discoverable as wired.
try:
    from utils.operating_segments import *  # noqa: F401, F403  (v10.468 wiring)
except ImportError:
