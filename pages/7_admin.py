...
# ════════════════════════════════════════════════════════════════
with tabs[10]:
    st.subheader("⚖️ LMS Configuration")
    st.markdown(
        "Configure loan management system parameters. "
    ...
    _lcfg_tabs = st.tabs([
        ...
        "📑 Conditions & Docs", "🛡️ Compliance Flags",
        "💹 FD Rate Bands", "🏛️ Approval Authorities",
    ])

    # ── Swim lanes ─────────────────────────────────────────────────
    with _lcfg_tabs[0]:
        st.markdown("**Swim lane thresholds and SLA days**")
        _lanes = _lms_cfg.get("swim_lanes", {})
        for _lane_key, _lane in _lanes.items():
            with st.expander(f"🏊 {_lane.get('label', _lane_key)}", expanded=False):
                _lc1, _lc2 = st.columns(2)
                _new_sla = _lc1.number_input(
                    "SLA (days)", min_value=1, max_value=365,
                    value=int(_lane.get("sla_days", 10)),
                    key=f"lane_sla_{_lane_key}")
                if _lane_key == "express":
                    _new_max = _lc2.number_input(
                        "Max amount (KES)", min_value=0,
                        value=int(_lane.get("max_amount", 5_000_000)),
                        step=1_000_000, key=f"lane_max_{_lane_key}")
                elif _lane_key == "complex":
                    _new_min = _lc2.number_input(
                        "Min amount (KES)", min_value=0,
                        value=int(_lane.get("min_amount", 100_000_000)),
                        step=5_000_000, key=f"lane_min_{_lane_key}")
                if st.button(f"💾 Save {_lane.get('label',_lane_key)}", key=f"save_lane_{_lane_key}"):
                    _lms_cfg["swim_lanes"][_lane_key]["sla_days"] = _new_sla
                    if _lane_key == "express":
    ...
    # ── Decline / Rework reasons ───────────────────────────────────
    with _lcfg_tabs[1]:
        _dr1, _dr2 = st.columns(2)
        _dec_reasons = _lms_cfg.get("decline_reasons", [])
        _ret_reasons = _lms_cfg.get("rework_reasons",  [])

        with _dr1:
            st.markdown("**Decline reasons**")
            _dec_text = st.text_area(
                "One reason per line", value="\n".join(_dec_reasons),
        ...
        with _dr2:
            st.markdown("**Rework reasons**")
            _ret_text = st.text_area(
                "One reason per line", value="\n".join(_ret_reasons),
    ...
    # ── Condition types ────────────────────────────────────────────
    with _lcfg_tabs[2]:
        _cond_types = _lms_cfg.get("condition_types", [])
        st.markdown("**Pre-disbursement condition types**")
        _cond_text = st.text_area(
            "One condition per line", value="\n".join(_cond_types),
    ...
    # ── Compliance flags ───────────────────────────────────────────
    with _lcfg_tabs[3]:
        _flags = _lms_cfg.get("compliance_flag_types", [])
        st.markdown("**Compliance flag types and SLA days by risk level**")
        _sla_days = _lms_cfg.get("compliance_sla_days", {})
        _cs1, _cs2 = st.columns(2)
        for _rl, _sd in _sla_days.items():
            _new_sd = _cs1.number_input(
                f"SLA days — {_rl}", min_value=1, max_value=90,
                value=int(_sd), key=f"comp_sla_{_rl}")
    ...
    # ── FD Rate bands ──────────────────────────────────────────────
    with _lcfg_tabs[4]:
        _bands = _lms_cfg.get("fd_rate_bands", {})
        st.markdown("**Treasury FD rate bands by tenure**")
        for _tenure, _band in _bands.items():
            _fb1, _fb2, _fb3 = st.columns(3)
            _fb1.markdown(f"**{_band.get('label',_tenure)}**")
            _min_r = _fb2.number_input(
                "Min %", min_value=0.0, max_value=25.0,
                value=float(_band.get("min_rate",7.0)), step=0.25,
                key=f"fd_min_{_tenure}")
            _max_r = _fb3.number_input(
                "Max %", min_value=0.0, max_value=25.0,
                value=float(_band.get("max_rate",13.0)), step=0.25,
                key=f"fd_max_{_tenure}")
    ...
    with _lcfg_tabs[5]:
        _auths = _lms_cfg.get("approval_authorities", [])
        st.markdown("**Approval authority levels by loan amount**")
        rows_a = [{"Level": a["level"], "Authority": a["label"],
                   "Min (KES M)": round(a["min_amount"]/1e6, 1),
                   "Max (KES M)": round(a["max_amount"]/1e6, 1)}
                  for a in _auths]
        st.dataframe(pd.DataFrame(rows_a), use_container_width=True, hide_index=True)
        st.info("💡 To edit approval authorities, update lms_config.json directly "
                "or contact your system administrator.")

with tabs[11]:
    # ─────────────────────────────────────────────────────────────
    # PROPOSITIONS CONFIGURATION
    # ─────────────────────────────────────────────────────────────
    st.subheader("🎯 Proposition / Segment Overlay Configuration")
    st.caption(
        "Configure the bank's horizontal propositions. Each proposition has its own "
        "KPI set that tracks influence metrics — not portfolio volumes — so there is "
        "no double-counting with the primary BSC.")

    _prop_cfg_path = Path(__file__).parent.parent / "data" / "proposition_config.json"
    _prop_perf_path= Path(__file__).parent.parent / "data" / "proposition_performance.json"
    ...
    # ── PT1: All Propositions overview ──────────────────────────
    with _ptabs[0]:
        st.markdown(f"**{len(_props)} propositions configured**")
        _pt_cols = st.columns(4)
        for _pi, (_tag, _prop) in enumerate(_props.items()):
            _pc = _pt_cols[_pi % 4]
            _clr = _prop.get("color","#6B7280")
            _active = _prop.get("active", True)
            _kpi_n  = len(_prop.get("kpis",[]))
            _pc.markdown(
                f"<div style='background:{_clr}10;border:1.5px solid "
                f"{'#' + _clr[1:] + '40' if _clr.startswith('#') else _clr};"
                f"border-radius:10px;padding:12px;margin-bottom:8px'>"
                f"<div style='font-size:20px'>{_prop.get('icon','🏦')}</div>"
                f"<div style='font-size:12px;font-weight:700;color:{_clr}'>"
                f"{_prop.get('name','')}</div>"
                f"<div style='font-size:10px;color:var(--color-text-tertiary)'>"
                f"Tag: {_tag} · {_kpi_n} KPIs · "
                f"{'🟢 Active' if _active else '🔴 Inactive'}</div>"
                f"</div>", unsafe_allow_html=True)

        st.markdown("---")
        st.markdown("**Toggle proposition active/inactive:**")
        _tog_cols = st.columns(4)
        for _pi, (_tag, _prop) in enumerate(_props.items()):
            _tc = _tog_cols[_pi % 4]
            _new_active = _tc.toggle(
                f"{_prop.get('icon','')} {_prop.get('name','')[:15]}",
                value=_prop.get("active", True),
    ...
    # ── PT2: Add new proposition ────────────────────────────────
    with _ptabs[1]:
        st.markdown("**Add a new proposition**")
        st.info("Create a new segment overlay (e.g. Youth Banking, Islamic Finance, "
                "Green Finance). Propositions track influence KPIs — not portfolio volumes.")
        _na1, _na2, _na3 = st.columns(3)
        _new_tag  = _na1.text_input("Tag code (2–4 letters, e.g. YTH)", max_chars=4,
                                     key="new_prop_tag", placeholder="YTH").upper().strip()
        _new_name = _na2.text_input("Proposition name", key="new_prop_name",
                                     placeholder="e.g. Youth Banking")
        _new_icon = _na3.text_input("Emoji icon", key="new_prop_icon",
                                     value="🏦", max_chars=2)
        _nb1, _nb2 = st.columns(2)
        _new_color= _nb1.color_picker("Brand colour", value="#3B82F6", key="new_prop_color")
        _new_role = _nb2.selectbox(
            "Proposition head role",
            ["— select —"] + sorted(set(d.get("role","") for d in um.users.values() if d.get("role",""))),
            key="new_prop_head_role")
        _new_desc = st.text_area("Description (one line)", key="new_prop_desc", height=68,
                                  placeholder="Brief description of this proposition's focus")
        _new_rms  = st.text_input(
            "RM role keywords (comma-separated — which roles contribute)",
            key="new_prop_rms", placeholder="e.g. Youth, Student, Relationship")

        if st.button("✅ Create proposition", key="create_prop_btn", type="primary",
                     disabled=not (_new_tag and _new_name and _new_role != "— select —")):
            if _new_tag in _props:
                st.error(f"Tag '{_new_tag}' already exists.")
            else:
                _pcfg["propositions"][_new_tag] = {
                    "name":        _new_name,
                    "tag":         _new_tag,
                    "icon":        _new_icon,
                    "color":       _new_color,
                    ...
                    "head_role":   _new_role,
                    "rm_roles":    [r.strip() for r in _new_rms.split(",") if r.strip()],
                    "period":      "2026",
                    "kpis":        []
                }
                _prop_cfg_path.write_text(json.dumps(_pcfg, indent=2))
    ...
    # ── PT3: Edit KPIs & Targets ────────────────────────────────
    with _ptabs[2]:
        st.markdown("**Edit KPIs and targets per proposition**")
        _sel_prop_tag = st.selectbox(
            "Select proposition",
            list(_props.keys()),
            format_func=lambda t: f"{_props[t].get('icon','')} {_props[t].get('name','')}" if t in _props else t,
            key="edit_prop_sel")

        if _sel_prop_tag and _sel_prop_tag in _props:
            _ep = _props[_sel_prop_tag]
            st.markdown(f"**{_ep.get('icon','')} {_ep.get('name','')}** — "
                        f"{len(_ep.get('kpis',[]))} KPIs | "
                        f"Period: {_ep.get('period','2026')}")

            # Period selector
            _ep_period = st.selectbox("Reporting period", ["2026","2025","2027"],
                                       index=["2026","2025","2027"].index(_ep.get("period","2026")),
                                       key=f"ep_period_{_sel_prop_tag}")
            if _ep_period != _ep.get("period"):
                _pcfg["propositions"][_sel_prop_tag]["period"] = _ep_period

            st.markdown("---")
            st.markdown("**Existing KPIs:**")
            _ep_kpis = _ep.get("kpis", [])
            _updated_kpis = []
            for _ki, _kpi in enumerate(_ep_kpis):
                _ek1,_ek2,_ek3,_ek4,_ek5,_ek6 = st.columns([3,1,1,1,1,0.5])
                _new_kname = _ek1.text_input("KPI name", value=_kpi["name"],
                                              key=f"kname_{_sel_prop_tag}_{_ki}",
                                              label_visibility="collapsed")
                _new_unit  = _ek2.selectbox("Unit", ["count","%","KES","days","score"],
                                             index=["count","%","KES","days","score"].index(
                                                 _kpi.get("unit","count")) if _kpi.get("unit","count") in
                                                 ["count","%","KES","days","score"] else 0,
                                             key=f"kunit_{_sel_prop_tag}_{_ki}",
                                             label_visibility="collapsed")
                _new_dir   = _ek3.selectbox("Dir", ["higher","lower"],
                                             index=0 if _kpi.get("direction","higher")=="higher" else 1,
                                             key=f"kdir_{_sel_prop_tag}_{_ki}",
                                             label_visibility="collapsed")
                _new_tgt   = _ek4.number_input("Target", value=float(_kpi.get("target",0)),
                                                key=f"ktgt_{_sel_prop_tag}_{_ki}",
                                                label_visibility="collapsed", step=1.0)
                _new_wt    = _ek5.number_input("Weight", value=float(_kpi.get("weight",0.10)),
                                                min_value=0.01, max_value=1.0, step=0.05,
                                                key=f"kwt_{_sel_prop_tag}_{_ki}",
                                                label_visibility="collapsed")
                _keep = not _ek6.button("🗑️", key=f"kdel_{_sel_prop_tag}_{_ki}",
                                         help="Remove this KPI")
                if _keep:
                    _updated_kpis.append({**_kpi,
                        "name":_new_kname,"unit":_new_unit,"direction":_new_dir,
                        "target":_new_tgt,"weight":_new_wt})

            st.markdown("**Add KPI:**")
            _ak1,_ak2,_ak3,_ak4,_ak5 = st.columns([3,1,1,1,1])
            _add_kname = _ak1.text_input("New KPI name", key=f"add_kname_{_sel_prop_tag}",
                                          placeholder="e.g. Youth Accounts Opened")
            _add_unit  = _ak2.selectbox("Unit", ["count","%","KES","days","score"],
                                         key=f"add_kunit_{_sel_prop_tag}")
            _add_dir   = _ak3.selectbox("Direction", ["higher","lower"],
                                         key=f"add_kdir_{_sel_prop_tag}")
            _add_tgt   = _ak4.number_input("Target", value=100.0, step=1.0,
                                            key=f"add_ktgt_{_sel_prop_tag}")
            _add_wt    = _ak5.number_input("Weight", value=0.20, min_value=0.01,
                                            max_value=1.0, step=0.05,
                                            key=f"add_kwt_{_sel_prop_tag}")

            _c1, _c2 = st.columns(2)
            if _c1.button("➕ Add KPI", key=f"add_kpi_btn_{_sel_prop_tag}",
                           disabled=not _add_kname.strip()):
                _kpi_id = f"{_sel_prop_tag}_{_add_kname.upper()[:8].replace(' ','_')}"
                _updated_kpis.append({
                    "id": _kpi_id, "name": _add_kname.strip(),
                    "unit": _add_unit, "direction": _add_dir,
                    "weight": _add_wt, "target": _add_tgt, "pillar": "Customer Focus"
                })
                st.success(f"Added: {_add_kname}")

            if _c2.button("💾 Save all KPI changes", key=f"save_kpis_{_sel_prop_tag}",
                           type="primary"):
                # Renormalise weights to sum to 1.0
                _total_w = sum(k["weight"] for k in _updated_kpis)
                if _total_w > 0:
                    for k in _updated_kpis:
                ...
                st.cache_data.clear()
                st.success(f"✅ {len(_updated_kpis)} KPIs saved for {_ep.get('name','')}")
                st.rerun()

            # Weight check
            _tw = sum(k["weight"] for k in _ep_kpis)
            if abs(_tw - 1.0) > 0.01:
                st.warning(f"⚠️ Weights sum to {_tw:.2f} — will be auto-normalised on save")

    # ── PT4: Head Role Mapping ───────────────────────────────────
    with _ptabs[3]:
        st.markdown("**Map each proposition to the correct head role in your org chart.**")
        st.caption("The head receives the proposition scorecard and sees the full overlay view.")
        _all_roles = sorted(set(d.get("role","") for d in um.users.values()
                                if d.get("role","").strip()))
        for _tag, _prop in _props.items():
            _hr1, _hr2, _hr3 = st.columns([1, 3, 2])
            _hr1.markdown(
                f"<div style='padding:8px 0'>{_prop.get('icon','')} "
                f"**{_prop.get('name','')}**</div>",
                unsafe_allow_html=True)
            _cur_role = _prop.get("head_role","")
            _new_head_role = _hr2.selectbox(
                "Head role",
                _all_roles,
                index=_all_roles.index(_cur_role) if _cur_role in _all_roles else 0,
                key=f"head_role_{_tag}", label_visibility="collapsed")
            _head_user = next(((u,d) for u,d in um.users.items()
                               if d.get("role","") == _new_head_role), (None,{}))
            _hr3.markdown(
                f"<div style='font-size:12px;padding:10px 0;color:var(--color-text-secondary)'>"
                f"{_head_user[1].get('full_name','Not found') if _head_user[0] else '—'}"
                f"</div>", unsafe_allow_html=True)

        if st.button("💾 Save role mappings", key="save_role_maps", type="primary"):
            for _tag, _prop in _props.items():
    ...
    with _ptabs[4]:
        st.markdown("""
### 📖 Proposition Configuration Guide

**What is configurable via this panel:**

| Setting | Where | Notes |
|---------|-------|-------|
| Proposition name, icon, colour | Add/Edit tabs | Bank-specific branding |
| Active / Inactive toggle | All Propositions tab | Inactive propositions hidden from scorecard |
| KPI names and targets | Edit KPIs tab | Set annually before period begins |
| KPI weights | Edit KPIs tab | Auto-normalised to sum to 1.0 on save |
| KPI direction (higher/lower) | Edit KPIs tab | Affects achievement calculation |
| Head role mapping | Head Role tab | Controls who sees proposition scorecard |
| Reporting period | Edit KPIs tab | Must match BSC period |

**What is hardcoded (by design):**

| Setting | Reason |
|---------|--------|
| Scoring thresholds (1.0–5.0) | Consistent with primary BSC — must match |
| Achievement formula (actual/target) | Standard across all KPIs |
| The two-layer model | Architecture — changing breaks audit trail |
| Segment tag application | CIF-level — managed via segment_tags.json |

**Anti-double-counting guarantee:**

Proposition KPIs must never be the same as primary portfolio KPIs.
Allowed: acquisition count, penetration %, wallet share, events, NPS.
Not allowed: total deposits, total loans, total NFI (these belong to branch BSC).

If you add a KPI with "Deposit" or "Loan" in the name, the system will still
accept it — but the bank controller should verify it is a tagged-customer
sub-segment view, not the branch total.
""")

with tabs[12]:
    # ─────────────────────────────────────────────────────────────
    # RA (REPORTING & ANALYTICS) CONFIGURATION
    # ─────────────────────────────────────────────────────────────
    st.subheader("📊 Reporting & Analytics Configuration")
    st.caption("Configure what the RA module shows and to whom. "
    ...
    # ── RA T1: Display settings ────────────────────────────────
    with _ra_tabs[0]:
        st.markdown("**General display settings:**")
        _ra_c1, _ra_c2 = st.columns(2)
        _ra_months = _ra_c1.slider(
            "Default date range (months)",
            min_value=3, max_value=24,
            value=_ra_cfg.get("date_range_months", 12),
            key="ra_months")
        _ra_show_names = _ra_c2.toggle(
            "Show staff names in league tables",
            value=_ra_cfg.get("show_staff_names", True),
            key="ra_show_names")
        _ra_allow_export = st.toggle(
            "Allow data export from RA module",
            value=_ra_cfg.get("allow_export", True),
            key="ra_allow_export")
        if st.button("💾 Save display settings", key="ra_save_display", type="primary"):
    ...
    # ── RA T2: Thresholds ─────────────────────────────────────
    with _ra_tabs[1]:
        st.markdown("**Colour-coding thresholds** — used in scorecard bands and SLA indicators:")
        thr = _ra_cfg.get("thresholds", {})
        _th1, _th2, _th3 = st.columns(3)
        _bsc_green  = _th1.number_input("BSC Green threshold (≥)",  value=float(thr.get("bsc_green",3.5)),  step=0.1, format="%.1f", key="ra_bsc_green")
        _bsc_amber  = _th2.number_input("BSC Amber threshold (≥)",  value=float(thr.get("bsc_amber",2.5)),  step=0.1, format="%.1f", key="ra_bsc_amber")
        _bsc_red    = _th3.number_input("BSC Red threshold (<)",     value=float(thr.get("bsc_red",2.0)),    step=0.1, format="%.1f", key="ra_bsc_red")
        _sla_green  = _th1.number_input("SLA Green (≥%)",           value=float(thr.get("sla_green",80)),   step=5.0, format="%.0f", key="ra_sla_green")
        _sla_amber  = _th2.number_input("SLA Amber (≥%)",           value=float(thr.get("sla_amber",60)),   step=5.0, format="%.0f", key="ra_sla_amber")
        if st.button("💾 Save thresholds", key="ra_save_thr", type="primary"):
            _ra_full_cfg["ra_config"]["thresholds"] = {
    ...
    # ── RA T3: Tab access ────────────────────────────────────
    with _ra_tabs[2]:
        st.markdown("**Control which roles can see each RA tab:**")
        ACCESS_OPTIONS = ["all_managers","credit_and_above","exec_only"]
        tab_access = _ra_cfg.get("tab_access", {})
        _updated_access = {}
        for _tab_name, _cur_access in tab_access.items():
            _ta1, _ta2 = st.columns([2,2])
            _ta1.markdown(f"**{_tab_name}**")
            _new_access = _ta2.selectbox(
                "Access", ACCESS_OPTIONS,
                index=ACCESS_OPTIONS.index(_cur_access) if _cur_access in ACCESS_OPTIONS else 0,
                key=f"ra_tab_{_tab_name.replace(' ','_')}",
                label_visibility="collapsed")
    ...
    # ── RA T4: What's configurable ───────────────────────────
    with _ra_tabs[3]:
        st.markdown("""
### 📊 RA Module — Configurable vs Hardcoded

**Configurable via this panel:**

| Setting | Location | Effect |
|---------|----------|--------|
| Date range (months) | Display Settings | How far back trend data goes |
| Show/hide staff names | Display Settings | Privacy control in league tables |
| Allow data export | Display Settings | Export buttons on/off |
| BSC colour thresholds | Thresholds | Red/amber/green band boundaries |
| SLA colour thresholds | Thresholds | SLA compliance colour bands |
| Tab-level access | Tab Access | Which roles see which tabs |

**Hardcoded by design (cannot change via UI):**

| Setting | Reason |
|---------|--------|
| BSC scoring formula | Must match primary BSC — consistency |
| Achievement calculation | actual÷target×100 is standard |
| Data sources (JSON files) | Architecture — changing breaks audit trail |
| Chart types | UX consistency across the bank |
| The 1–5 scoring scale | Matches BSC — cannot differ in RA |
| Proposition KPI formulas | Defined in Proposition config, not RA |
""")
...
with tabs[13]:
    ...
    with _r1:
        st.markdown("**Fee types** (configurable — defines what fees appear in waiver/leakage logs):")
        _fee_list = st.text_area("Fee types (one per line)",
            value="\n".join(_rac.get("fee_types",[])), height=200, key="rac_fees")
        st.markdown("**Waiver reasons:**")
        _waiver_list = st.text_area("Waiver reasons (one per line)",
            value="\n".join(_rac.get("waiver_reasons",[])), height=120, key="rac_waivers")
    ...
    with _r2:
        ...
        limits_dict = _rac.get("approval_limits", {})
        new_limits = {}
        for role_k, limit_v in limits_dict.items():
            col_a, col_b = st.columns([3,2])
            col_a.markdown(f"**{role_k}**")
            new_limits[role_k] = col_b.number_input("Limit (KES)", value=float(limit_v),
                step=10000.0, key=f"ral_{role_k}", label_visibility="collapsed")
...
with tabs[14]:
    # ── RMS CONFIG ────────────────────────────────────────────────
    st.subheader("🔄 Reconciliation Management System (RMS) Configuration")
    _rms_cfg = _rac_full.get("rms_config", {})
    _rm1,_rm2,_rm3 = st.tabs(["Recon Types","SLA Days","Accounts"])
    with _rm1:
        st.markdown("**Reconciliation types:**")
        _rt_list = st.text_area("Recon types (one per line)",
            value="\n".join(_rms_cfg.get("recon_types",[])), height=180, key="rms_types")
        _bt_list = st.text_area("Breaker types (one per line)",
            value="\n".join(_rms_cfg.get("breaker_types",[])), height=120, key="rms_breakers")
    ...
    with _rm2:
        ...
        new_sla = {}
        for s_k, s_v in sla_d.items():
            c1,c2=st.columns([3,2])
            c1.markdown(f"**{s_k}**"); new_sla[s_k]=c2.number_input("Days",value=int(s_v),min_value=0,step=1,key=f"rsla_{s_k}",label_visibility="collapsed")
        thres1,thres2 = st.columns(2)
        new_auto = thres1.number_input("Auto-match threshold (KES)", value=float(_rms_cfg.get("auto_match_threshold_kes",1000)), step=500.0, key="rms_auto")
        new_esc  = thres2.number_input("Escalation after (days)", value=int(_rms_cfg.get("escalation_days",30)), min_value=1, key="rms_esc")
        if st.button("💾 Save SLA", key="rms_save_sla", type="primary"):
    ...
    with _rm3:
        ...
        df_acc = pd.DataFrame(accs) if accs else pd.DataFrame(columns=["code","name","type"])
        st.dataframe(df_acc, use_container_width=True, hide_index=True)
        st.caption("Add new accounts by editing proposition_config.json directly or contact system admin. Hardcoded: matching algorithm, GL posting logic.")

with tabs[15]:
    # ── EDMS CONFIG ───────────────────────────────────────────────
    st.subheader("📁 EDMS Configuration")
    _edms_cfg = _rac_full.get("edms_config", {})
    _ed1,_ed2,_ed3 = st.tabs(["Categories","Retention Periods","Access Levels"])
    with _ed1:
        st.markdown("**Document categories:**")
        _cat_list = st.text_area("Categories (one per line)",
            value="\n".join(_edms_cfg.get("categories",[])), height=200, key="edms_cats")
        _alert_days = st.multiselect("Expiry alert thresholds (days before expiry)",
            options=[7,14,30,60,90,180], default=_edms_cfg.get("expiry_alert_days",[30,60,90]), key="edms_alerts")
    ...
    with _ed3:
        ...
        for lvl, desc in acc_lvls.items():
            st.markdown(f"  **{lvl}:** {desc}")
        st.caption("Hardcoded: audit trail, versioning, encryption, file storage structure.")

with tabs[16]:
    # ── TREASURY CONFIG ───────────────────────────────────────────
    st.subheader("💹 Treasury Configuration")
    _tc  = _rac_full.get("treasury_config", {})
    _t1,_t2,_t3,_t4 = st.tabs(["FX & Rates","IFRS 9","Liquidity","Products & Limits"])
    with _t1:
        st.markdown("**Reference rates (update daily or link to feed):**")
        fx_rates_cfg = _tc.get("fx_reference_rates",{})
        new_fx = {}
        fcols = st.columns(len(fx_rates_cfg) if fx_rates_cfg else 1)
        for col, (ccy, rate) in zip(fcols, fx_rates_cfg.items()):
            new_fx[ccy] = col.number_input(f"{ccy}/KES", value=float(rate), step=0.01, format="%.4f", key=f"fx_{ccy}")
        cbk_r = st.number_input("CBK Rate (%)", value=float(_tc.get("cbk_rate",13.0)), step=0.25, key="t_cbk")
    ...
    with _t2:
        ...
        ecl = _tc.get("ifrs9_ecl_rates",{})
        new_ecl = {}
        ec1,ec2,ec3 = st.columns(3)
        for col, (stage, rate) in zip([ec1,ec2,ec3], ecl.items()):
            new_ecl[stage] = col.number_input(f"{stage} (%)", value=float(rate)*100, step=0.1, format="%.2f", key=f"ecl_{stage}") / 100
        st.markdown("**IFRS 9 classifications allowed:**")
        clf_list = st.multiselect("Classifications", ["HTM","AFS","FVTPL","FVOCI"],
            default=_tc.get("ifrs9_classifications",["HTM","AFS","FVTPL"]), key="t_clf")
    ...
    with _t3:
        ...
        liq_min_cfg = _tc.get("liquidity_ratios_minimum",{})
        new_liq = {}
        for ratio, val in liq_min_cfg.items():
    ...
    with _t4:
        ...
        _fx_ccys = st.multiselect("FX currencies traded",
...
with tabs[17]:
    ...
    with _sa1:
        st.markdown("**Credit assessment parameters:**")
        _sc1, _sc2, _sc3 = st.columns(3)
        _sa_dsr   = _sc1.number_input("DSR Limit (%)",          value=float(_sa_cfg.get("dsr_limit",40)),           step=1.0, min_value=20.0, max_value=60.0, key="sa_dsr")
        _sa_mnths = _sc2.number_input("Min statement months",   value=int(_sa_cfg.get("min_months",3)),             step=1,   min_value=1,    max_value=12,   key="sa_mnths")
        _sa_liv   = _sc3.number_input("Living expense (% income)",value=float(_sa_cfg.get("living_expense_pct",30)), step=1.0, min_value=10.0, max_value=50.0, key="sa_liv")
        _sa_rate  = _sc1.number_input("Interest rate (%/month)",value=float(_sa_cfg.get("interest_rate_monthly",1.5)),step=0.05,min_value=0.5, max_value=5.0, key="sa_rate", format="%.2f")
        st.caption("**Hardcoded:** CBK DSR methodology, affordability formula (income − obligations − living), Claude API model. Changing DSR limit here automatically updates all new analyses.")
        if st.button("💾 Save thresholds", key="sa_save_thr", type="primary"):
            _sa_full["statement_analyzer_config"]["dsr_limit"]            = int(_sa_dsr)
    ...
    with _sa2:
        st.markdown("**Risk keywords** (transactions containing these words are flagged in the analysis):")
        _sa_kws = st.text_area("Keywords (one per line)",
            value="\n".join(_sa_cfg.get("risk_keywords",[])), height=250, key="sa_kws")
    ...
    with _sa3:
        st.markdown("**Auto-decision triggers** — conditions that automatically recommend Approve or Decline:")
        st.markdown("*Auto-approve triggers (ALL must be met):*")
        _aa = _sa_cfg.get("auto_approve_triggers", [])
        for trigger in _aa:
            st.markdown(f"  ✅ {trigger}")
        st.markdown("*Auto-decline triggers (ANY triggers decline):*")
        _ad = _sa_cfg.get("auto_decline_triggers", [])
        for trigger in _ad:
            st.markdown(f"  ❌ {trigger}")
        st.caption("These are advisory thresholds — Claude's analysis may still override based on context. Hardcoded: the actual Claude analysis logic cannot be changed via UI.")
        st.markdown("**What is always hardcoded (cannot change):**")
        st.markdown("""
| Item | Reason |
|------|--------|
| Claude API model | Fixed to claude-sonnet-4-20250514 for consistency |
| JSON output schema | Must match page display logic |
| CBK 40% DSR rule | Regulatory — CBK Prudential Guideline |
| Affordability formula | Standard banking: Net income − obligations − living |
| Audit trail of analyses | Compliance requirement |
...
# ── Organisation Management Tabs ─────────────────────────────────
if is_admin:
    render_dept_manager(tabs[19],  uname)
    render_branch_manager(tabs[20], uname)
    render_module_assignment(tabs[21], uname)
    render_roles_manager(tabs[22], uname)
