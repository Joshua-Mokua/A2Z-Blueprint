...
# -*- coding: utf-8 -*-
r"""
A4 - ranking drill-down, and met-vs-not-met analytics down to the unit.

YOUR ASKS
  "on the ranking why not have the filters e.g like i can expand and see towers
   per individual like on the daily log"
  "we need a pie showing % met daily vs not met down to the units also on
   cumulative"

MET vs NOT MET - the definition matters more than the chart. A person-day counts
only if it CARRIED A TARGET. Rest days, gazetted holidays and excused absences
have no target, so including them would either flatter the rate (a Sunday is
never missed) or punish someone for approved leave. This reuses the same
carried_forward() rows the history grid and the leaderboard use, so all three
report the same achievement for the same population.

WHAT THIS ADDS
  /leaderboard rows now carry met_days, scored_days and met_rate, at every
  level, plus bank-wide totals on the envelope.

  Leaderboard
    * a met/not-met donut on the current scope, with the day counts spelled out
    * a Met % column, coloured green/amber/red at 60/30
    * ROW EXPANSION: clicking a unit, branch or role row opens the individuals
      inside it, fetched with that row applied as a filter - the same drill the
      daily-log roll-up uses. Top 40 shown, with a count when there are more.

  DailyLogAnalytics
    * a bank-wide met/not-met donut
    * a stacked bar BY UNIT, cumulative over the window, PROPORTIONAL - so a
      six-person unit and a thirty-person unit are compared on rate rather than
      volume, which is the only fair comparison. Hover gives the day counts.

  The per-unit data is sourced from the leaderboard rather than recomputed, so
  the analytics and the ranking cannot disagree about the same number.

Verified: py_compile clean, tsc --noEmit clean, vite build clean.

Usage (from project root, .venv active):
    python scripts\patch_a4_drilldown.py            # dry run
    python scripts\patch_a4_drilldown.py --apply    # write + .pre_a4 backups
"""
import os
import shutil
import sys
...
def main():
    ...
    for p in (API, APITS, LEAD, ANAL):
        if not os.path.isfile(p):
            print("ABORT: %s not found." % p)
            print("       Apply patch_a3_analytics.py and its predecessors first.")
            return 1
    ...
    if "met_rate" in api:
        print("ABORT: /leaderboard already reports met_rate - A4 looks applied.")
        return 1
    if "/leaderboard" not in api:
        print("ABORT: apply patch_a1_leaderboard.py first.")
        return 1

    a = api.index('@router.get("/leaderboard")')
    b = api.index('@router.get("/analytics")', a)
    api = api[:a] + LEADERBOARD_NEW + api[b:]
    print("  ok  /leaderboard - met_days / scored_days / met_rate")

    i = ts.index('// \u2500\u2500 Cumulative leaderboard (staff / role / branch / unit) \u2500\u2500')
    j = ts.index('export async function fetchBranchLogRanking(', i)
    ts = ts[:i] + TS_NEW + ts[j:]
    print("  ok  api.ts - met/not-met fields on the leaderboard types")

    # post-checks
    if api.count('@router.get("/leaderboard")') != 1:
        print("ABORT: post-check - leaderboard route count is not 1.")
        return 1
    if "met_rate" not in api or "scored_days" not in api:
        print("ABORT: post-check - met/not-met fields missing from the endpoint.")
        return 1
    ...
    for name, blob in (("Leaderboard", LEAD_NEW), ("DailyLogAnalytics", ANAL_NEW)):
        for o, c in (("{", "}"), ("(", ")")):
            if blob.count(o) != blob.count(c):
                print("ABORT: %s unbalanced %s%s." % (name, o, c))
                return 1
    for token in ("met_rate", "expand(", "openRow"):
        if token not in LEAD_NEW:
            print("ABORT: Leaderboard missing %r." % token)
            return 1
    for token in ("met vs not met", "stackOffset", "byUnit"):
        if token not in ANAL_NEW:
            print("ABORT: DailyLogAnalytics missing %r." % token)
            return 1
    ...
    for path, content in ((API, api), (APITS, ts), (LEAD, LEAD_NEW), (ANAL, ANAL_NEW)):
