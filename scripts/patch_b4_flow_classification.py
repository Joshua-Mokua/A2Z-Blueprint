...
# -*- coding: utf-8 -*-
r"""
B4 - HOTFIX: loans were shown the account journey, and every value read zero.

TWO REGRESSIONS, both surfaced by the bucket migration and both mine.

1. EVERY DEAL CLASSIFIED AS "other", so 28 loans were shown the four-step
   ACCOUNT journey (Initiation / Documentation / Approval / Opening) instead of
   the credit journey. flow_for_deal called utils.core._stage_flow_for, WHICH
   DOES NOT EXIST, and its except-branch then read deal_class / product_class -
   fields the deals do not carry. Everything fell to "other".

   It now classifies by PRODUCT through utils.core.get_pipeline_category, the
   mapper the pipeline already uses, with a keyword fallback:

       Business Loan / Personal Loan        -> asset      (credit journey)
       Current Account (CASA) / Fixed Deposit -> liability (Initiation->Opening)
       Bancassurance                        -> insurance
       nothing resolvable                   -> other

   You said it plainly: "the bank is very keen on deposit mobilization and loans
   disbursement". Those two must classify correctly even on a sparsely filled
   deal, which is why the keyword fallback exists rather than requiring a
   perfectly populated product field.

2. ASSET / LIABILITY / TOTAL ASSURED ALL READ ZERO. utils.core.ACTIVE_STAGES is
   built from the HARDCODED PIPELINE_STAGES_LOAN list - the retired vocabulary.
   After the migration no deal matched it, so "active" was empty and every
   headline value collapsed. That is the same hardcoded-stages fault the ruling
   forbids, one layer deeper than F1 reached.

   pipeline_funnel.all_active_stages() derives the list from the configured
   buckets, and all THREE call sites in api.py now use it. The legacy list
   survives only as a fallback for a build with no buckets configured.

Verified: a Business Loan at Initiation, a Personal Loan at Credit Analysis and
a Fixed Deposit at Documentation now all count as ACTIVE, and split asset 2 /
liability 1.

ON THE LIGHT-TOUCH PRODUCTS: accounts and deposits keep the four-step journey
deliberately - they are largely same-day, so tracking them heavily would add
ceremony without insight, while still giving the bank the deposit-mobilisation
view it wants.

Usage (from project root, .venv active):
    python scripts\patch_b4_flow_classification.py            # dry run
    python scripts\patch_b4_flow_classification.py --apply    # write + .pre_b4 backups
"""
import os
import shutil
import sys
...
BACKUP_SUFFIX = ".pre_b4"

IMPORT_OLD = "        ACTIVE_STAGES, ALL_ACTIVE_STAGES,"
IMPORT_NEW = "        ACTIVE_STAGES as _LEGACY_ACTIVE, ALL_ACTIVE_STAGES as _LEGACY_ALL_ACTIVE,"

LIVE_OLD = '    live = [d for d in deals if not d.get("draft") and not _referral_blocked(d)]'
LIVE_NEW = '''    # ACTIVE STAGES FROM CONFIG, not from the hardcoded PIPELINE_STAGES lists.
...
ACT_OLD = "    from utils.core import ACTIVE_STAGES as _ACTIVE"
ACT_NEW = '''    # Config-derived active stages; the hardcoded list is only a fallback.
    try:
...
def main():
    ...
    for p in (MOD, API):
        if not os.path.isfile(p):
            print("ABORT: %s not found - apply patch_b3_funnel_shape_and_cleanup.py first." % p)
            return 1

    cur = open(MOD, encoding="utf-8").read()
    api = open(API, encoding="utf-8").read()

    if "all_active_stages" in cur:
        print("ABORT: all_active_stages already present - B4 looks applied.")
        return 1
    if "bucket_health" not in cur:
        print("ABORT: apply patch_b3_funnel_shape_and_cleanup.py first.")
        return 1
    if api.count(IMPORT_OLD) != 1:
        print("ABORT: analytics import matched %d times." % api.count(IMPORT_OLD))
        return 1
    if api.count(LIVE_OLD) != 1:
        print("ABORT: live-deals line matched %d times." % api.count(LIVE_OLD))
        return 1
    n_act = api.count(ACT_OLD)
    if n_act != 2:
        print("ABORT: expected 2 ACTIVE_STAGES imports, found %d - refusing to")
        print("       half-migrate, which would leave one screen reading zero.")
        return 1

    for token in ("get_pipeline_category", "all_active_stages", "_CATEGORY_TO_FLOW"):
        if token not in MODULE:
            print("ABORT: embedded module missing %r." % token)
            return 1
    if "_stage_flow_for" in MODULE:
        print("ABORT: embedded module still calls the helper that does not exist.")
        return 1

    api = api.replace(IMPORT_OLD, IMPORT_NEW, 1)
    api = api.replace(LIVE_OLD, LIVE_NEW, 1)
    api = api.replace(ACT_OLD, ACT_NEW)          # BOTH sites
    print("  ok  api.py - all three active-stage sites config-derived")

    # Count the newline-terminated alias: "_cfg_active" (the analytics site)
    # CONTAINS "_cfg_act", so a bare substring count returns 3 and this guard
    # rejected a perfectly correct patch.
    if api.count("as _cfg_act\n") != 2:
        print("ABORT: post-check - expected both drill sites repointed, found %d."
              % api.count("as _cfg_act\n"))
        return 1
    if "_cfg_active" not in api:
        print("ABORT: post-check - the analytics site was not repointed.")
        return 1
    ...
    for path, content in ((MOD, MODULE), (API, api)):
    ...
    print("\nRestart uvicorn. Loans should now show the credit journey and the")
    print("Asset / Liability / Total Assured figures should return.")
    return 0
