...
export default function HistoryGrid({ grid, loading, days, onDaysChange }: HistoryGridProps) {
  ...
  const units = useMemo(
    () => Array.from(new Set(allRows.map((r) => String(r.unit ?? '').trim()).filter(Boolean))).sort(),
    [allRows],
  );
  const people = useMemo(() => {
    const seen = new Map<string, string>();
    for (const r of allRows) {
      if (unitFilter && String(r.unit ?? '').trim() !== unitFilter) continue;
  ...
  const rows = useMemo(() => allRows.filter((r) =>
    (!unitFilter || String(r.unit ?? '').trim() === unitFilter)
    && (!staffFilter || String(r.staff_code ?? '') === staffFilter)), [allRows, unitFilter, staffFilter]);
  ...
  function exportCsv() {
    const head = ['Date', 'Staff', 'Name', 'Role', 'Unit', ...cols.map((c) => c.label),
                  'Index', 'Target', 'Variance', 'C/F balance', 'Notes', 'Manager note'];
    const body = rows.map((r) => [
      r.log_date, r.staff_code, cleanName(r.staff_name), r.role, r.unit,
        ...
        <div className="flex flex-wrap items-center gap-2 text-xs">
          ...
          {units.length > 1 && (
            <select
              ...
              value={unitFilter}
              onChange={(e) => { setUnitFilter(e.target.value); setStaffFilter(''); }}
            >
          ...
          {(unitFilter || staffFilter) && (
            <button type="button"
              onClick={() => { setUnitFilter(''); setStaffFilter(''); }}
            ...
            <thead>
              <tr>
                ...
                <th className={th + ' sticky z-30 border-r border-gray-300 bg-gray-100 text-gray-600'}
                    style={{ left: L_UNIT, top: 0, minWidth: W_UNIT }}>Unit</th>
                {cols.map((c) => (
                  <th key={c.key}
              ...
              {rows.map((r: HistoryGridRow, i) => {
                ...
                return (
                  <tr key={String(r.staff_code) + '-' + String(r.log_date)}
                    ...
                    <td className={frozen + ' border-r border-gray-300 text-gray-600'} style={{ left: L_UNIT }}
                        title={String(r.unit ?? '')}>
                      <span className="block overflow-hidden text-ellipsis" style={{ maxWidth: W_UNIT - 12 }}>
                        {r.unit}
                      </span>
                    </td>
