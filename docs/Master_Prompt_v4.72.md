# A2Z MIS 360 — Master prompt (v4.72)
...
**Current version: v10.429 (May 2026) - Cascade-BSC linkage / BSC RESCUE COMPLETE.** Per v10.424 audit: 10 cascade staff missing from BSC by code. Root cause: 10 senior staff (Veronica Mutai = Head of Branches + 9 Area Managers) had BSC rows under WRONG codes (300001-300010 colliding with chief officers) instead of their canonical 301500-301509. NEW utils/bsc_cascade_linkage_engine.py (~300 LOC) API-first (ZERO streamlit, dry_run=True default). Public API: audit_bsc_code_alignment + fix_bsc_codes. Strategy: rewrite BSC Staff Code to match staff_register canonical code by name. 3 JSON-serializable dataclasses. Live migration: 10 staff corrected; 206 BSC rows updated. **BSC RESCUE COMPLETE - overall health 85.7% -> 100% (all 7 categories clean: staff coverage, KPI completeness, pillar canonical, weight normalization, library alignment, cascade linkage, duplicate rows)**. NEW scripts/fix_bsc_codes.py runner with --confirm. NEW 2 FastAPI endpoints /api/v1/bsc-codes/audit + fix. 23rd React-ready engine. Engine preserved 0/0/0/0.
