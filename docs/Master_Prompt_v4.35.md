# A2Z MIS 360 — Master prompt (v4.35)
...
**Current version: v10.392 (May 2026) — MD↔CRBO Circular Cascade SURGICALLY REMOVED.** First execution batch of Phase C2 (Target Cascade Rescue arc). Per v10.391 diagnosis Finding TC20 (CRITICAL). The cascade graph had exactly one 2-cycle: MD (300001) → CRBO (300002) [legitimate downstream direction] AND CRBO (300002) → MD (300001) [wrong upstream direction]. **Surgical fix**: for each of CRBO's 21 cascade entries, removed the single allocation with to_code=300001 (the MD); recomputed allocated_sum from remaining ~97 legitimate subordinate allocations. **MD→CRBO downstream direction preserved** (still cascades 21 KPIs to CRBO). Cascade graph 2-cycle count: 0 (was 1). MD receiver count: 0 (correct — MD is root). CRBO's cascade to other reports preserved (still cascades to 50+ distinct recipients). Total amount-value removed: KES 215.5B notional (the wrong-direction allocations that should never have existed). Backup at data/_v10392_backups/target_cascade.json.before. **No Joshua decisions required** — selected as lowest-risk Tier-1 first execution batch (zero C1-C6 dependencies). Pure data-file surgery; no engine code touched. v10.391 diagnosis test test_v10391_tc20_circular_md_crbo_cascade_present_RETIRED_v10392 obsolete (bug fixed; test skipped); replaced by 11 new tests in test_v10392_circular_cascade_fixed.py verifying post-fix state. Body-system framing: loop in endocrine system = uncontrolled feedback signal; v10.392 cuts the loop; body closer to homeostasis. Pattern: diagnosis (v10.391) → first surgical fix (v10.392) → next surgical fix (v10.393 cross-branch cleanup) → engine work (v10.394+ after Joshua decides C5). Ships docs/CIRCULAR_CASCADE_FIXED_v10.392.md (8 Parts).
