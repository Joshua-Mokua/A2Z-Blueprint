# A2Z MIS 360 — Master prompt (v4.78)
...
**Current version: v10.435 (May 2026) - Staff exit + target gap risk detection.** Per Joshua roadmap: after onboarding fit-in (v10.434), audit the inverse - what happens when staff exit? Risk to targets, who's a single point of failure, how would gaps be filled? NEW utils/staff_exit_engine.py (~700 LOC) API-first ZERO streamlit. 4 public functions: audit_exit_risk, audit_all_exit_risks (optimized in-memory iteration), simulate_redistribution, simulate_exit. 4 JSON-serializable dataclasses (StaffExitRisk, BankWideExitAudit, RedistributionPlan, ExitSimulation). Risk score (0-100) combines 5 dimensions: outgoing cascade size (0-25) + outgoing target value (0-20) + role uniqueness (0-25) + pillar criticality (0-15) + incoming reliance (0-15). Bands: Critical 75+, High 50-74, Medium 25-49, Low <25. 3 redistribution strategies: peer_split (equal share among unit role peers), manager_absorb (push to Reports To), hold_open (document gap). EXTENDED bsc_admin_panel render_exit_risk_panel with bank-wide rollup + critical/high tables + interactive simulator. EDITED pages/7_admin.py wires panel. NEW 3 FastAPI endpoints /api/v1/exit-risk/audit + audit/{code} + simulate/{code}. Bank-wide findings: 0 Critical, 95 High, 214 Medium, 1128 Low (78.5%); avg score 16.09; top drivers incoming_reliance + outgoing_cascade. MD scores 70 (High - 18 cascade entries, KES 315B). Branch Managers 50 (High). Tellers 15 (Low - 243 peers absorb). 28th React-ready engine. 360 harmony 100% preserved. BSC rescue 100% preserved. Engine 0/0/0/0.
