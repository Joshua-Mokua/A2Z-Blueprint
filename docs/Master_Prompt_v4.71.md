# A2Z MIS 360 — Master prompt (v4.71)
...
**Current version: v10.428 (May 2026) - Weight renormalization (BSC Rescue batch 4).** Per v10.424 audit: 491 staff had Weight column sums != 1.0 (range 1.0-4.28). Root cause: KPIs assigned with library default_weight values, never per-staff normalized. NEW utils/bsc_weight_normalize_engine.py (~350 LOC) API-first (ZERO streamlit, dry_run=True default). Public API: audit_actuals_weights + renormalize_actuals_weights. Strategy: per-staff proportional rescale preserving relative importance — new_weight[i] = old_weight[i] / sum(old_weights). 3 JSON-serializable dataclasses. WEIGHT_TOLERANCE = 0.01 (1%). Live migration: 491 staff renormalized; 10945 BSC rows touched; post sum range 1.0-1.0 (was 1.0-4.28). BSC health 71.4% -> 85.7% (+14.3 points; 6/7 categories now clean). NEW scripts/renormalize_bsc_weights.py runner with --confirm. NEW 2 FastAPI endpoints /api/v1/bsc-weights/audit + renormalize. 22nd React-ready engine. Engine preserved 0/0/0/0.
