# A2Z MIS 360 — Master prompt (v4.4)
...
**Current version: v10.360 (May 2026) — Branch Single Source of Truth.** Closes a foundational data-integrity gap Joshua flagged during the live bring-up: two parallel branch lists existed — utils.core.BRANCH_REGION (hardcoded 21 entries, 3 regions South/Central/North) and data/org_config.json::branches[] (94 entries, 7 regions: Nairobi 27, Other 45, Rift Valley 8, Coast 5, Central 5, Nyanza 2, Eastern 2; rich schema with id/name/region/branch_code/dept_id/opened_date/region_group). The two never overlapped cleanly (15-of-21 overlap by name-stripped). v10.360 unifies on **org_config.json as the single source of truth**: utils.core.BRANCH_REGION is now built at module-load time via `_build_branch_region_from_org_config()` (with the legacy 21-entry list retained only as a degraded-environment fallback); utils.virtual_bank_seed.ECOBANK_BRANCHES likewise reads via the new `get_ecobank_branches()` helper. All 7 BRANCH_REGION consumers (utils/finance_hub_render.py, pages/1_perform.py, pages/2_people.py, pages/15_optimize.py, pages/16_commission.py, pages/18_cims.py, pages/63_assets.py, pages/7_admin.py, pages/_sidebar.py) continue to read it as a dict and need no change. utils.core.REGIONS similarly built dynamically. SeedConfig.n_branches default changed from 21 to 0 (meaning "all available from ECOBANK_BRANCHES"). The seeded bank now spans 94 real Ecobank branches across 7 regions rather than 21. G246 audit gate locks the unification — verifies utils.core has `_build_branch_region_from_org_config`, virtual_bank_seed has `get_ecobank_branches`, and the runtime count from `get_ecobank_branches()` matches the active count in org_config.json. **Football Team Test chain still 6/7 WIRED** — v10.360 is a data-integrity batch, not chain closure. Link 7 (regional→MD tile) remains for v10.361.
...
**Open / partially-closed gaps at v10.360:**

- **Football Team Test end-to-end verification** — 6/7 chain links WIRED, 1/7 PARTIAL. Only Link 7 (regional→MD tile bank-targets binding) remains — v10.361. End-to-end integration test = v10.362 candidate; **after v10.362 Charter §2 passes**.
- **CBS-wired actuals arc — item 4 (deferred)** — PBT computation from CBS transactions. Re-prioritized after v10.362 closes the Football Team Test chain.
- **BSC coverage 2.78% → 100%** — only 40 of 1,439 active staff have BSC actuals. Now technically unblocked; sustained data-engineering effort.
- **System stocks live wiring** — 6/6 have snapshot accessors but currently return demo defaults. v10.359 bridge makes this trivially easy.
- **MD BSC bank-targets binding (roadmap item 2)** — Link 7 — v10.361.
- **Branch roles data generation (roadmap item 5)** — some branches lack the full role complement in synthesized data. With v10.360's 94-branch source, this is amplified (more branches need role coverage). Generator-script fix.
- **Audit performance** — full audit takes >5 min due to pre-existing slow gates. Candidate for parallelization / incremental caching.
- **strategy_simulator + hybrid_scheduling_simulator lack self_test** — surfaced by v10.357 readiness audit.
- **Bridge aging buckets are zeros** — `npl_aggregate.json::by_aging_kes` is zeroed.
- **Strategic Initiative engine / Partnerships P&L / B-027 tail** — earlier roadmap items partially built.
...
- **Documented data-shape divergences (deferred)** — `strategic_initiatives.rag_status`, `kpi_library.kpis[].direction`.
