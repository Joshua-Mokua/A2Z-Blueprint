# A2Z MIS 360 — Master prompt (v4.42)
...
**Current version: v10.399 (May 2026) — Joshua's 7-point HQ canonical corrections applied.** Per Joshua 2026-05-13: (1) **synthetic Managing Director deleted** (exec_md_001 removed; William Mwanake remains as sole Chief Executive & Managing Director — C1 outstanding question RESOLVED); (2) **Head of Digital Financial Services moved from CIO → CCO** — DFS is commercially-led, with DCOs at branches retaining dotted reporting line to Head of DFS while primary reports stay with Branch Manager same-branch; (3) Manager Card Operations chain now CCO → Head of DFS → Manager Card Ops (no role-level change needed); (4) Corporate Sales Dealer confirmed under Head of Treasury → CFO; (5) Trade Finance Back Office Manager confirmed under Head of Operations → COO; (6) Trade Finance split confirmed — relationships (SRM TF / RM TF) under CCO via Head Of Corporates & Trade Finance, operations (Senior TF Officer / TF Officer / TF Ops Officer) under COO via Head of Operations; (7) **Admin role moved from CHRO → MD** — Joshua identifies "Admin" as his developer/MD-login account used for viewing progress and as MD login backstop, not an HR Officer Admin position. Engine audit preserves zero-state from v10.398: 0 cycles ✓, 0 cross-branch ✓, 0 multi-sender ✓, **0 rep_critical** ✓. Cascade regenerated: 25,488 entries (preserved). users.json: 1450 → 1449 entries.
