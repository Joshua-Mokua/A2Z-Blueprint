# A2Z MIS 360 — Master prompt (v4.93)
...
**Current version: v10.450 (May 2026) - Credit 360 Review + HR 360 Fixes.** Per Joshua pushback: 'for credit am yet the confirmation of react readiness, fast api, etc... have we done functionality tests of every tab, have we confirmed that all the credit staff are in place with proper reporting lines and can get targets and their actuals can populate and bsc calculate accordingly and even feed to hr performance and their individual bsc are we really at 84.8% with all these things, our doctrine dictates a 360 review and revival.' Joshua was right - 84.8% was PARTIAL view. The doctrine's 360 review adds 6 new audit dimensions surfacing honest gaps: (1) API Coverage 0.0% - 0 of 8 credit engines have FastAPI endpoints (vs HR's 17 endpoints for 8 engines); (2) React Readiness 92.9% - 13 of 14 clean (21_loan_applications has 12 unsafe_allow_html instances); (3) PostgreSQL Backing 64.3% - 9 of 14 use PG adapter; (4) Staff Completeness 25.0% - only 3 of 12 expected credit roles in target_cascade (missing Head Of Credit + Senior Credit Analyst + Credit Risk Analyst + Branch Credit Manager/Officer + Credit Monitoring Officer + Debt Recovery Officer + Credit Administration Officer + Collateral Officer); (5) BSC Actuals Auto-Wiring 0.0% - 0 of 48 credit KPIs auto-populate, all 48 manual Excel; (6) Tab Functionality 100% - 54 tabs across 14 pages parse + imports resolve. **HONEST CREDIT HEALTH: 55.5% (not 84.8%).** 2 critical findings (API + BSC actuals at 0%). HR 360 fixes: (a) role-aware welcome via _resolve_chief_hr() surfaces actual Chief HR's name (not whoever is logged in) + 'Viewing as' disclosure when viewer != Chief HR; (b) NEW 'Staff Performance' tab[1] in Chief HR Centre showing all HR-dept staff with latest BSC scores + 4 metric cards (HR-dept staff/with BSC/avg score/top performers) + 5-band performance distribution (Outstanding/Exceeds/Meets/Below/Underperforming) + sortable staff table; tab count 6->7. CreditSectionAudit dataclass extended with 6 new optional fields + to_dict() updated. New composite formula: placement 10% + completeness 10% + wiring 10% + flow 10% + api 10% + react 5% + postgres 5% + staff 15% + bsc_actuals 15% + tab_func 10%. v10.451+ rescue: build credit FastAPI endpoints (Phase 3); build credit_actuals_engine like hr_actuals_engine (Phase 5); add 9 missing credit roles to target_cascade (Phase 4); wire analytics_credit_workbench out of admin-only; build 4 remaining stubs; build credit->HR performance bridge (Phase 7). Body health 91.1%; 360 100%; BSC rescue 100% preserved; engine 0/0/0/0.
