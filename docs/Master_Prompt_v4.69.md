# A2Z MIS 360 — Master prompt (v4.69)
...
**Current version: v10.426 (May 2026) - BSC Library Register (BSC Rescue batch 2).** Joshua directive: register all unregistered BSC KPIs in canonical library with NO duplicates or aliases. NEW utils/bsc_library_register_engine.py (~450 LOC) API-first (ZERO streamlit, dry_run=True default). 4-layer atomic migration: (1) Add aliases to existing library entries via KNOWN_ALIAS_MAP for 4 BSC names matching existing library KPIs (Bancassurance + Credit TAT x3); (2) Fix non-canonical pillars in library via LIBRARY_PILLAR_FIX_MAP - 13 entries with pillar="Process" normalized to "Operational Excellence"; (3) Resolve multi-pillar BSC actuals via MULTI_PILLAR_RESOLUTION - 5 KPIs (4 FD-prefix + NIM) tagged with two pillars across rows, normalized to single canonical pillar (16 rows flipped); (4) Register 76 truly-new canonical KPIs with full schema (id, name, pillar, weight, unit, direction, active, description, source, _origin). Also PATCHED v10.424 bsc_audit_engine.audit_library_alignment to consider aliases field (was only matching id/name, missing existing aliases). Library alignment 23.58% -> 100%. BSC health 42.9% -> 57.1% (+14.2 points). NEW scripts/register_bsc_library.py runner with --confirm. NEW 2 FastAPI endpoints /api/v1/bsc-library/audit + register. 20th React-ready engine. Engine preserved 0/0/0/0.
