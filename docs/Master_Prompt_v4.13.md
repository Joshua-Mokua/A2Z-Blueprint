# A2Z MIS 360 — Master prompt (v4.13)
...
**Current version: v10.369 (May 2026) — Per-Branch PBT Allocation.** Second concrete unification step from the v10.367 architecture arc. Replaces the legacy `aggregate_cbs_by_branch` naive formula (`int + fee - loans × 0.02`) with a proper allocation engine. New `utils/branch_pbt_allocator.py` (~440 LOC, 9 self-tests) exports `compute_pbt_by_branch(cbs_dir, allocation_rule=None, branch_fte_lookup=None) → Dict[str, PBTComponents]` keyed by branch_code. **The reconciliation identity holds**: `Σ(Branch PBT) == Bank PBT` within KES 100 (locked by G255; on seeded bank delta is -2 KES). Four allocation rules supported, all admin-configurable in new `data/branch_allocation_rules.json` (Rule N1): **`fte_weighted`** (default per Joshua's Q3 decision in v10.367 — allocates OpEx by branch FTE share, falls back to accounts-per-branch proxy if FTE data unavailable, falls back to equal split if no accounts), **`revenue_weighted`** (allocates by operating-income share), **`equal`** (splits evenly across branches with activity), **`hybrid`** (50% FTE + 50% revenue, weights configurable). FTE source chain: caller-provided dict → `data/branch_fte.json` → accounts-per-branch proxy → equal. Drift-absorption ensures exact reconciliation: any Decimal rounding remainder lands on the largest-OpEx branch so Σ(Branch OpEx) == bank.total_opex exactly. On 100-customer seeded bank, allocator produces 64 branches; on production 700K-customer Ecobank, would produce all 94 active branches with proportional OpEx. Module purity preserved: zero upward `utils.*` imports beyond `pbt_computation` (legitimate downward); self_test uses hand-rolled CSV fixtures per v10.364 lesson. G255 audit gate locks: module + canonical exports + config + rule values + identity + multi-branch result. Charter §2 still passes (G249); SBU identity still holds (G254); v10.367 reconciliation diagnostic still informational (Engine B refactor scheduled for v10.372). Bank-level `compute_pbt_from_cbs` unchanged; branch dimension is purely additive.
...
- **v10.370 — Per-RM canonical refactor.** `rm_profitability.py` (809 lines) refactored to consume from canonical engine. Per-RM PBTComponents from the same data pipeline as SBU and Branch. G256 locks Σ(RM PBT) = Bank PBT.
- **v10.371 — Multi-level bank_targets.json schema extension.** `PBT|<level>|<entity>|<year>` keys for SBU / Branch / RM. Σ(child targets) == parent target. Closes the top-down side of the reconciliation (currently `bank_targets.json` has only bank-level targets). **G253 ratchets** to CONVERGED (<1%).
- **v10.372 — Engine B refactor.** `sbu_pnl_rollup.bank_total_pnl` refactored to optionally consume from `compute_pbt_by_sbu` (Finance hub drill-down stays sourced from canonical). Proxy mode preserved for backward compatibility one batch; documented as deprecated.

**Open / partially-closed gaps at v10.369:**

- **FLEXCUBE production deployment** — code ready (v10.365), pending Apigee gateway access + credentials + mode flip.
- **Total NFI in compute_bank_aggregates** — still uses legacy formula. Cleanup candidate (now safe — PBT engine handles it correctly).
...
- **Per-branch FTE data not yet generated** — branch_pbt_allocator falls back to accounts-per-branch proxy. Once HR/staff register includes branch_code, FTE-weighted becomes exact.
...
*Master prompt v4.13 for v10.369 — Per-Branch PBT Allocation. **Fourteenth consecutive batch holding lockstep discipline.** Second concrete unification step from the v10.367 architecture arc. Replaces the legacy aggregate_cbs_by_branch naive formula (`int + fee - loans × 0.02`) with a proper allocation engine. Ships utils/branch_pbt_allocator.py (~440 LOC, 9 self-tests) with compute_pbt_by_branch returning Dict[str, PBTComponents] keyed by branch_code. THE SECOND RECONCILIATION IDENTITY: Σ(Branch PBT) == Bank PBT within KES 100 (delta -2 KES on seeded 64-branch bank). Four allocation rules in data/branch_allocation_rules.json (Rule N1, admin-editable): fte_weighted (default per Q3), revenue_weighted, equal, hybrid (50/50). FTE source chain has four fallback levels — caller dict, data/branch_fte.json, accounts-per-branch proxy, equal split. Drift-absorption: largest-OpEx branch absorbs Decimal rounding remainder so Σ(Branch OpEx) == bank.total_opex EXACTLY. Module purity preserved per v10.364 lesson (zero upward utils.* imports beyond pbt_computation downward dependency; self_test uses hand-rolled CSV fixtures). G255 locks: module exports + config + identity + multi-branch result. Identity holds across all four rules (test exercises this). SBU + Branch dimensions coexist cleanly (test exercises this — both reconcile to bank, and to each other within tolerance). Charter §2 still passes. Audit 255; verifier 245/245; tests 16/16 in v10.369 + 151 prior = 167 total passing. G162 baseline at 4022 (63 consecutive zero-drift batches).*
