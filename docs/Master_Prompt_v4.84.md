# A2Z MIS 360 — Master prompt (v4.84)
...
**Current version: v10.441 (May 2026) - HR Rescue Arc Batch 4: build onboarding+exit pages.** Per v10.436 audit: 2 v10.434/v10.435 engines existed with no user page. v10.441 builds the 2 missing pages. NEW pages/79_staff_onboarding.py (4 tabs: Simulate Onboarding + Validate Record + Per-Staff Audit + Bank-Wide Audit using validate_new_staff + simulate_onboarding + audit_staff_completeness + audit_all_staff_completeness). NEW pages/80_staff_exit.py (4 tabs: Per-Staff Exit Risk + Top Key-Person Risks + Redistribution Plan + Bank-Wide Exit Readiness using audit_exit_risk + audit_all_exit_risks + simulate_exit + simulate_redistribution). Both registered in manifest under people_hr dept with require_access strings. Backup data/_v10441_backups/_manifest.json.before. Stamp _v10441_new_pages on manifest. Engine extension: audit_module_placement.should_be_in_hr_but_arent now dynamic (was hard-coded list in v10.436) - returns [] when both pages exist. Result: HR engine wiring 75%->100% (all 8 HR-domain engines wired); HR Health 69.2%->76.2%; module placement 100% with 0 missing. Only 2 rescue priorities remain: stubs in workforce/disciplinary + API endpoints for 6 engines. UI-BUILD batch no engines built. 360 harmony 100% preserved; BSC rescue 100%; standards wiring 78.8%; engine 0/0/0/0.
