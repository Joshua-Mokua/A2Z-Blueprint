# A2Z MIS 360 — Master prompt (v4.8)
...
**Current version: v10.364 (May 2026) — PBT computation from CBS.** Closes the highest-priority MD BSC gap: `bank_targets.json::PBT|2026 = 650B` had no proper CBS-computable actual before v10.364 (the placeholder was the naive `bank["int"] + bank["fee"] - bank["loans"] * 0.02`, missing OpEx, impairment, and interest expense entirely). v10.364 adds `utils/pbt_computation.py` with `compute_pbt_from_cbs()` implementing the canonical bank P&L: `PBT = Operating Income - Total OpEx - Impairment`, where `Operating Income = NII + Non-Interest Income`, `NII = Interest Income - (Deposits × cost_of_funds_pct)`, `Non-Interest Income = Fee Income × (1 + non_interest_other_pct)`, OpEx loaded from `data/opex_data.json` (admin-configurable bank-wide breakdown: staff_costs, it_costs, premises, other_opex), and `Impairment = NPL Stage 3 × LGD%`. All assumption factors configurable in new `data/pbt_assumptions.json` (defaults: cost_of_funds_pct=3%, lgd_pct=45% per Basel, non_interest_other_pct=15%) per Rule N1. The `PBTComponents` dataclass exposes the full P&L drill-down (interest_income, interest_expense, nii, fee_income, non_interest_other, operating_income, staff_costs, it_costs, premises, other_opex, total_opex, npl_stage_3, impairment_charge, pbt, plus assumption snapshots and opex_source metadata) — executives see *why* PBT is what it is, not just the final number. `format_pbt_summary` renders a readable P&L. `compute_bank_aggregates` now wires PBT via `compute_pbt_from_cbs` and also returns NII and CIR (Cost-Income Ratio) alongside PBT for executive drill-down. G250 audit gate locks: module + PBTComponents + assumptions JSON + opex_data.json + actuals_engine wiring + e2e probe verifies compute_bank_aggregates returns PBT/NII/CIR with non-trivial values. **MD's BSC view now has a proper PBT actual** alongside the bank_targets target — the highest-impact KPI for the One Question is now data-driven.
...
**Audit gates: 250** (G1 through G250). Distribution: G1-G14 foundational; G15-G117 standards-coverage; G118+ QA framework; G128 structural audit baseline; G143 KPI source coverage; G161 manifest-canonical lock; G162 tenant-hardcoding ratchet (kaizen baseline 4022 — 58 consecutive zero-drift batches); G230 protected-files schema validation; G231 page module-load smoke; G232-G236 hub-consolidation thresholds; G237 redirect signaling; G238 static AST function checks; G239 dynamic render-function smoke; G240 CBS baseline snapshot; G241 live actuals YoY sidecar; G242 master prompt sync (anti-drift lockstep); G243 virtual bank readiness audit; G244 virtual bank seed determinism; G245 CBS persistence bridge integrity (Link 1); G246 branch single source of truth; G247 admin CRUD coverage; G248 Link 7 MD tile binding; G249 Charter §2 Football Team Test end-to-end verification; G250 PBT computation from CBS.
...
**Open / partially-closed gaps at v10.364:**

- **FLEXCUBE branch/staff/account live integration** — Stubbed at v10.361. When Apigee gateway access provisioned, replace stub bodies with real REST calls. No caller-code changes.
- **CBS accruals layer (interest_income_ytd / fee_income_ytd)** — bridge writes 0 by design ("accruals computed downstream"). Production: FLEXCUBE provides these. Development: a future batch could synthesize plausible accruals from loan rates × outstanding × elapsed days, allowing more realistic PBT simulation without FLEXCUBE.
- **Per-branch PBT in aggregate_cbs_by_branch** — still uses legacy naive formula. Branch-level PBT requires an allocation engine (allocate bank OpEx to branches by some rule). Future batch — different scope from v10.364's bank-level fix.
- **BSC coverage 2.78% → 100%** — only 40 of 1,439 active staff have BSC actuals. Sustained data-engineering effort.
- **System stocks live wiring** — 6/6 have snapshot accessors but return demo defaults. Wirable via compute_bank_aggregates which now includes PBT/NII/CIR.
...
- **Bridge NPL aging buckets are zeros** — affects v10.364's impairment computation (gets 0 NPL Stage 3 in seeded data). Production reads from FLEXCUBE.
...
*Master prompt v4.8 for v10.364 — PBT computation from CBS. **Ninth consecutive batch holding lockstep discipline.** Closes the highest-priority MD BSC gap: bank_targets.json::PBT|2026=650B had no proper CBS-computable actual before v10.364 (placeholder was bank[int]+bank[fee]-bank[loans]*0.02, missing OpEx, impairment, interest expense). Adds utils/pbt_computation.py (~290 LOC) with compute_pbt_from_cbs() implementing canonical bank P&L: PBT = Operating Income (NII + Non-Interest Income) - Total OpEx - Impairment. NII = Interest Income - (Deposits × cost_of_funds_pct). Non-Interest Income = Fee Income × (1 + non_interest_other_pct). OpEx loaded from data/opex_data.json (admin-configurable bank-wide breakdown). Impairment = NPL Stage 3 × LGD%. All assumption factors configurable in new data/pbt_assumptions.json (defaults: cost_of_funds_pct=3%, lgd_pct=45% Basel, non_interest_other_pct=15%) per Rule N1. PBTComponents dataclass exposes full P&L drill-down (15 fields + assumption snapshot + opex_source metadata) — executives see why PBT is what it is, not just the final number. format_pbt_summary renders a readable P&L. compute_bank_aggregates now wires the new PBT computation and additionally returns NII and CIR for executive drill-down. G250 audit gate locks: module present + PBTComponents + assumptions JSON + opex_data.json + actuals_engine wiring + e2e probe verifies compute_bank_aggregates returns PBT/NII/CIR with non-trivial values. MD's BSC view now has a proper PBT actual alongside the bank_targets target — the highest-impact KPI for the One Question is data-driven. Audit 250; verifier 199/199; smoke trio still green; tests 14/14 in v10.364 file + 80 prior = 94 total passing. G162 baseline at 4022 (58 consecutive zero-drift batches).*
