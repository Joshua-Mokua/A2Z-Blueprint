# A2Z MIS 360 — Master prompt (v4.14)
...
**Current version: v10.370 (May 2026) — Per-Customer + Per-Staff PBT (Atomic Unit).** Third concrete unification step from the v10.367 architecture arc. Establishes **per-customer PBT as the foundational atomic profitability unit** — every other rollup (Bank, SBU, Branch, Staff) is now Σ over this single ground truth. New `utils/customer_pbt_allocator.py` (~530 LOC, 11 self-tests) exports `compute_pbt_by_customer(cbs_dir) → Dict[CIF, PBTComponents]` and `compute_pbt_by_staff(cbs_dir, customer_pbts=None) → Dict[staff_code, PBTComponents]`. **Two reconciliation identities now hold**: G256 locks `Σ(Customer PBT) == Bank PBT` within KES 100 (OpEx reconciles EXACTLY — drift absorbed by largest-revenue customer); G257 locks `Σ(Staff PBT including Unassigned) == Bank PBT` within KES 100. Per-staff is a thin Σ over customers tagged via `customers.csv::rm_code`. On seeded 100-customer bank: 100 customer buckets, 28 staff buckets, delta 6 KES. Four allocation rules in new `data/customer_allocation_rules.json` (Rule N1): `revenue_weighted` (default — standard activity-based costing), `balance_weighted` (deposits+loans footprint), `equal`, `hybrid` (50/50). **Critical design decision regarding staff roles**: `compute_pbt_by_staff` returns ALL tagged staff (portfolio owners BRM/SRO/RO AND service staff Tellers/CSOs/BOS) without filtering by role. Per Joshua's framing, this separates two concerns: the data engine produces neutral per-staff aggregations; the UI layer filters by role (joining `staff_code → role` from `users.json` or `hr.json`) for sales attribution. Service-cost attribution (tellers transact on accounts but don't own relationships) is acknowledged as a future concern handled later via `cost_allocation_rules.json` matrix mode with per-transaction staff tags. Module purity preserved per v10.364 lesson; self_test uses hand-rolled CSV fixtures; zero upward imports beyond pbt_computation. The reconciliation tree is now four-deep: Bank PBT (v10.364) → SBU (v10.368) → Branch (v10.369) → Customer atomic + Staff aggregation (v10.370). Charter §2 still passes (G249); all prior identities still hold. Now three of five unification batches done; v10.371 (multi-level bank_targets schema) and v10.372 (Engine B refactor) remain.
...
**Profitability unification arc (in flight — 3/5 done, 2 remaining):**

- **v10.371 — Multi-level bank_targets.json schema extension.** `PBT|<level>|<entity>|<year>` keys for SBU / Branch / Staff / Customer. Σ(child targets) == parent target. Closes the top-down side of the reconciliation (currently `bank_targets.json` has only bank-level targets). **G253 ratchets** to CONVERGED (<1%).
- **v10.372 — Engine B refactor.** `sbu_pnl_rollup.bank_total_pnl` refactored to optionally consume from `compute_pbt_by_customer` (Finance hub drill-down stays sourced from canonical). Proxy mode preserved for backward compatibility one batch; documented as deprecated.
...
*Master prompt v4.14 for v10.370 — Per-Customer + Per-Staff PBT (Atomic Unit). **Fifteenth consecutive batch holding lockstep discipline.** Third concrete unification step. Joshua's framing: "I need to drill this down to profitability per customer ultimately." This batch establishes per-customer PBT as the foundational atomic unit — Bank, SBU, Branch, Staff all become Σ over this single ground truth. Ships utils/customer_pbt_allocator.py (~530 LOC, 11 self-tests) with compute_pbt_by_customer returning Dict[CIF, PBTComponents] and compute_pbt_by_staff returning Dict[staff_code, PBTComponents]. THE THIRD AND FOURTH RECONCILIATION IDENTITIES: Σ(Customer PBT) == Bank PBT (G256); Σ(Staff PBT including Unassigned) == Bank PBT (G257). OpEx reconciles EXACTLY for both (drift-absorbed by largest-revenue customer in per-customer path; preserved exactly through staff aggregation). On seeded 100-customer bank: 100 customer buckets, 28 staff buckets, delta 6 KES. Four allocation rules in data/customer_allocation_rules.json (Rule N1): revenue_weighted (default — standard activity-based costing), balance_weighted (deposits+loans footprint), equal, hybrid. Critical design re staff roles: compute_pbt_by_staff returns ALL tagged staff in customers.csv::rm_code field (portfolio owners BRM/SRO/RO AND service staff Tellers/CSOs/BOS) without role filtering. Joshua's framing: the rm_code field in production CBS contains whoever's tagged — could be either. The data engine remains role-neutral; UI filters by joining staff_code → role from users.json downstream. Service-cost attribution (tellers transact but don't own relationships) acknowledged as future concern handled via cost_allocation_rules.json matrix mode with per-transaction tags. Module purity preserved per v10.364 lesson (zero upward utils.* imports beyond pbt_computation; hand-rolled CSV fixtures in self_test). Reconciliation tree now four-deep: Bank → SBU → Branch → Customer/Staff. Audit 257; verifier 256/256; tests 18/18 in v10.370 + 167 prior = 185 total passing. G162 baseline at 4022 (64 consecutive zero-drift batches).*
