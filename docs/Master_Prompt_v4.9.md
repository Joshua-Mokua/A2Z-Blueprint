# A2Z MIS 360 — Master prompt (v4.9)
...
**Current version: v10.365 (May 2026) — FLEXCUBE live wire-up.** Replaces the v10.361 None-stubs in `fetch_branches_from_flexcube` and `fetch_staff_from_flexcube` with real `requests.get` patterns + OAuth2 bearer auth via Apigee gateway. Three modes now genuinely distinct: `synthetic` (returns None — preserves the `org_config.json` fallback chain established in v10.361); `mock` (reads `data/flexcube_mock_branches.json` and `data/flexcube_mock_staff.json` — exercises the **live code path** against local fixtures so the wiring can be proven structurally sound without a real FLEXCUBE); `live` (real REST calls to `{fcubs_rest}/branches` and `{hcm_rest or fcubs_rest}/employees` via Apigee, with OAuth2 client-credentials flow). The mock mode is the v10.365 contribution that makes this honest — it lets the live code path execute and produce correct results from a local fixture, which proves the URL construction, header structure, response mapping, and error handling are all sound. When Apigee gateway access is provisioned in production and `flexcube_config.json::mode` flips to `"live"`, the same code path executes against the real FLEXCUBE without caller-code changes. End-to-end verified: with mock mode active, `utils.virtual_bank_seed.get_ecobank_branches()` returns the FLEXCUBE-sourced 94 branches (priority chain: FLEXCUBE → org_config → empty). G251 audit gate locks: module surface (_live_/_mock_ helpers present); fixtures exist and are well-formed JSON lists; live functions actually call `requests.get` with `Bearer` auth (not stubs); synthetic mode returns None; mock mode reads fixtures correctly.
...
**Audit gates: 251** (G1 through G251). Distribution: G1-G14 foundational; G15-G117 standards-coverage; G118+ QA framework; G128 structural audit baseline; G143 KPI source coverage; G161 manifest-canonical lock; G162 tenant-hardcoding ratchet (kaizen baseline 4022 — 59 consecutive zero-drift batches); G230 protected-files schema validation; G231 page module-load smoke; G232-G236 hub-consolidation thresholds; G237 redirect signaling; G238 static AST function checks; G239 dynamic render-function smoke; G240 CBS baseline snapshot; G241 live actuals YoY sidecar; G242 master prompt sync; G243 virtual bank readiness audit; G244 virtual bank seed determinism; G245 CBS persistence bridge integrity; G246 branch single source of truth; G247 admin CRUD coverage; G248 Link 7 MD tile binding; G249 Charter §2 Football Team Test end-to-end verification; G250 PBT computation from CBS; G251 FLEXCUBE live wire-up (synthetic/mock/live modes with real requests.get + fixture-backed mock testing).
...
**Open / partially-closed gaps at v10.365:**

- **CBS accruals layer (interest_income_ytd / fee_income_ytd)** — bridge writes 0 by design ("accruals computed downstream"). Production: FLEXCUBE provides these via existing fetch_account_balance. Development: a future batch could synthesize plausible accruals from `loan_rate × outstanding × elapsed_days`, allowing more realistic PBT simulation without FLEXCUBE.
- **Per-branch PBT in aggregate_cbs_by_branch** — still uses legacy naive formula. Branch-level PBT requires an OpEx allocation engine — distributing the bank's 7.9B OpEx across branches by some rule (FTE, revenue contribution, square footage). Future batch with own design questions.
- **FLEXCUBE production deployment** — code is ready, requires Apigee gateway access provisioning + `FLEXCUBE_CLIENT_ID` / `FLEXCUBE_CLIENT_SECRET` env vars + flipping `flexcube_config.json::mode` to `"live"`. No code changes needed.
