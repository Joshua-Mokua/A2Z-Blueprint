# A2Z MIS 360 — Master prompt (v4.38)
...
**Current version: v10.395 (May 2026) — WITHIN_BRANCH_ROLE_PAIRS now DYNAMIC from admin config.** Per Joshua's directive: *"role names that apply to the hierarchy stem from the admin config like the KPIs since different banks may name different roles differently and we don't want those hardcoded"*. Replaced the hardcoded 15-pair set literal in `utils/cascade_structure_engine.py` with 4 helper functions that read from `data/org_hierarchy_config.json`: `load_role_tiers()` reads canonical role→tier mapping (0..6), `load_role_manager_whitelist()` reads canonical subordinate→[managers] mapping, `load_branch_tier_threshold()` reads `branch_tier_threshold` with default 4, `load_within_branch_role_pairs()` derives the set from above. **A pair is within-branch iff BOTH manager and subordinate tiers >= threshold** (default 4; tier 3 = regional supervision excluded since multi-branch is legitimate). Engine is now **bank-agnostic** — different banks with different role taxonomies deploy without code changes; admin updates org config, engine sees new pairs. Module constant WITHIN_BRANCH_ROLE_PAIRS populated at import time from dynamic load; refresh by calling function. Engine remains leaf-pure (AST-verified, zero upward utils.* imports). Cross-branch violation count changes 25,137 → 19,026 (canonical pairs differ from inspection-based set: 9 new pairs canonical recognizes, 6 spurious pairs removed). **v10.394 TC40 RESOLVED**. v10.393 self-test #9 updated to verify tier respect rather than specific literals; v10.393 test_v10393_canonical_within_branch_pairs updated; v10.394 test_v10394_engine_within_branch_pairs_diverges_from_canonical RETIRED (renamed _RETIRED_v10395). 12 new integration tests verify dynamic loading + threshold configurability + tier respect + bank portability. **Delivery: patch zip only** per Joshua's efficiency request (cumulative zip dropped, individual files dropped — apply patch on top of v10.394 cumulative).
