# A2Z MIS 360 — Master prompt (v4.22)
...
**Current version: v10.378 (May 2026) — Customer Master Merge (Phase B continues).** Per Joshua's "merge into 1" approval at v10.374 wrap-up. Establishes the **recognition / sensory layer** of the body-system framing: the bank recognizes its customers across both transactional (CBS) and intelligence (marketing) universes through one canonical engine. Three deliverables: (1) `docs/CUSTOMER_MASTER_MERGE_v10.378.md` (~7KB, 7 Parts) — merge strategy + conflict resolution rules + identity equation + storage policy. (2) `utils/customer_master_canonical.py` (leaf module — 10 self-tests) with `UnifiedCustomerRecord` + `compute_unified_customer_master` + `reconciliation_summary` + `get_customer` + `STATUS_CBS_ONLY/MARKETING_ONLY/BOTH` + `SRC_CBS/MARKETING/DERIVED`. Merges CBS customers.csv (100 seed / 700K production) + customer_intelligence.json (3,000 individuals, 9-digit CIF) + customer_intelligence_business.json (206 businesses, CIF<digits> format). Strict CIF match. Conflict resolution: CBS wins for full_name/segment/branch_code/rm_code (KYC-authoritative); marketing wins for clv/churn/nba/nps/propensity_scores/tags/digital_engagement (analytics). Field-level _field_lineage tracking. Identity equation |A∪B| = |A| + |B| - |A∩B| LOCKED by G264. (3) End-to-end demonstration: seeded bank → 3,306 unified records (100 cbs_only + 3,206 marketing_only + 0 both — disjoint by design in seed; production would have overlap). Read-only per constitution §4.3 — no new JSON output (canonical engine IS the source of truth, computable on demand). Body-system: skeleton + circulatory + nervous + RECOGNITION/SENSORY (new v10.378) + endocrine + brain. Phase B continues: v10.379 = write-bridge canonical → bsc_actuals via bsc_engine.submit. v10.380 = KPI-ID canonicalisation. v10.381/v10.382 = refactor customer_profitability/rm_profitability to consume unified master.
