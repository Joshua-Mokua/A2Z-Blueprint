# A2Z MIS 360 — Master prompt (v4.82)
...
**Current version: v10.439 (May 2026) - Standards-wide engine wiring diagnostic, leave no stone unturned.** Per Joshua directive after BSC question: extend HR rescue diagnostic pattern across the entire codebase. NEW utils/standards_wiring_audit_engine.py (~600 LOC) API-first ZERO streamlit. 4 audit functions (engine_inventory + standards_wiring + unwired_standalone + orphan_standards) + master standards_full_audit. 5 JSON-serializable dataclasses. AGGREGATOR_ENGINES whitelist (10 hub/aggregator engines: finance_hub_render + platform_hub_render + scenario_simulator + api_resource_optimization + 6 more - engines wired only through these are still user-accessible). EXPECTED_INFRASTRUCTURE whitelist (6: bsc_engine Std #1+#2 contract layer called by 16 engines + flexcube_adapter + api + db + core + static_check). DOMAIN_PREFIXES classification (26 entries for grouping by domain). Initial findings: 479 engines total, 212 wired direct, 77 via aggregator, 65 infrastructure, 6 expected infrastructure; 23 registry-backed unwired standalone (RESCUE TARGETS); 330 standards, 78.8% wiring coverage. Top 10 rescue priorities by LOC*standards weight: #1 reconciliation 500LOC 18stds + #2 audit_universe 684LOC 13stds + #3 issue_management 643LOC 8stds + #4 cross_sell_bandit 1276LOC + #5 model_governance_runtime 1105LOC + 5 more. BSC-specific finding: bsc_engine correctly classified as expected_infrastructure (NOT a rescue target - 16 internal callers, Std #1+#2 contract layer). 29th React-ready engine. AUDIT-ONLY batch: v10.440+ executes rescue. 360 harmony 100% preserved; BSC rescue 100%; HR section 61.7% preserved; engine 0/0/0/0.
