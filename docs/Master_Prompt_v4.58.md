# A2Z MIS 360 — Master prompt (v4.58)
...
**Current version: v10.415 (May 2026) - F2 part B: per-allocation stretch tuner.** Continues F2 architectural concern from v10.414. Engine extension to utils/cascade_buffer_engine.py: apply_stretch_to_allocations (returns StretchApplicationResult with new_allocations, violations list, updated_count, new_total_amount), derive_base_for_allocation helper, cascade_stretch_breakdown rollup. NEW '🛡️ Step 3 (optional) F2 stretch tuning' expander in Set team targets - per-report stretch slider (capped by MD's per-KPI cap), live preview of final amount, apply button validates + persists via CascadeManager.set_allocation. NEW POST /api/v1/cascade/buffer/apply endpoint (router now 22 routes). Pydantic StretchApplyRequest/Response. Allocations gain optional stretch_pct + base_amount fields (backward compat: absent = 0 stretch, base = amount). Cap validation re-uses validate_buffer from v10.414 (single source of truth). Engine preserved 0/0/0/0. F3 (per-line-manager retain) targets v10.416; F5 (dual-view BSC render) targets v10.417.
