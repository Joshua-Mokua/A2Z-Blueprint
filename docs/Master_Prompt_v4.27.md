# A2Z MIS 360 — Master prompt (v4.27)
...
**Current version: v10.383 (May 2026) — RM Profitability Canonical Refactor (Phase B parallel-engines unification COMPLETE).** Per Phase B roadmap commitment from v10.378/v10.381/v10.382 wrap-ups. Mirrors v10.381 customer_profitability refactor pattern but exposes a pre-existing silent failure: `customer_intelligence.json` has NO rm_code field, so `_default_rm_customer_lookup` silently returned [] for every RM lookup. Pre-v10.383 RM dashboards showed 0 customers for every RM — a constitutional §5.4 violation already in production. After v10.383, canonical-first lookup uses CBS-authoritative rm_code with legacy fallback. **Live evidence:** RM 300046 had 0 customers visible pre-v10.383, 4 customers via canonical path post-v10.383. Ships `docs/RM_PROFITABILITY_CANONICAL_REFACTOR_v10.383.md` (7 Parts). Module changes to `utils/rm_profitability.py`: `_canonical_rm_customer_lookup_v10383` (NEW), `_legacy_rm_customer_lookup` (RENAMED), `reset_canonical_rm_cache` (NEW), `_RM_UNIFIED_MASTER_CACHE` + `_RM_BY_RM_CODE_INDEX` (module-level dual caches for O(1) lookups). AST-verified canonical-first ordering. All 34 existing engine tests pass unchanged. Phase B parallel-engines arc complete: customer (v10.381) + RM (v10.383) BOTH consume v10.378 canonical master. Circulatory organ fully wired through recognition organ. Next: v10.384 = rescue body's prioritization organ (pillar weights admin module consolidation per Joshua directive at v10.383 wrap-up). Then v10.385+ = proper deep diagnosis of the entire body.
