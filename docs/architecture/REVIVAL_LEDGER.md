...
The Revival Ledger is the **chronological harmonization log** of the A2Z system. Where the constitutional artifacts (Waves 1-5) declare *what* the system is, this ledger records *what was done, when, why, and by whom* to bring it into compliance with that constitution.
...
5. **pipeline, credit_*, treasury_*, compliance_cases** — domain data
6. **cbs_baseline_*** — large; can use efficient bulk loads

Each migration triggers a new REVIVAL_LEDGER entry. The migration gate (`gate_pg_<file>_migrated`) increments as files complete.
