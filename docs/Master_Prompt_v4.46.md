# A2Z MIS 360 — Master prompt (v4.46)
...
**Current version: v10.403 (May 2026) — Cascade cleanup batch.** Per Joshua's 20-chief observation + deep-review findings: pure data cleanup, no design changes. (A1) **Deleted 10 EXEC-* synthetic chiefs** from users.json (exec_cro/cco/coo/cfo/cio/crso/ccmp/cia/chro/ccmo_001) — residual seed data. Real chiefs (10) at staff codes 300002-300010 + 300178 (GM Banc). (A4 + E-C5) **Added Admin role + EXEC-* prefix to cascade_regenerator EXCLUDED_ROLES** filter at _build_staff_index — Admin is Joshua's monitoring/login account, not a P&L responsibility center. (A5) Cleaned 13 test entries from canonical_change_log.json. (D1) Retired stale test_v10397_total_unique_codes_increased. (B1-B4) Marked 4 KPI library duplicates with _v10403_alias_of (full dedup deferred to v10.409). Updated virtual_bank_kpi_unifier::SBU_HEAD_STAFF_CODE from EXEC-* placeholders to real chief codes (300002, 300003, 300007). Re-regenerated target_cascade: 24,192 → 24,024 entries (168 phantom entries removed). **MD's cascade now correctly shows 10 chief recipients** (was 20). Cascade allocations to EXEC-*/ADMIN001 = 0 (was 616). Engine state preserved: 0/0/0/0.
