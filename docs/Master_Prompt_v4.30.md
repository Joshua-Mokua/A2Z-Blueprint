# A2Z MIS 360 — Master prompt (v4.30)
...
**Current version: v10.386 (May 2026) — KPI Library Pillar Weights Admin Tab Migrated to Canonical Save (FIRST PHASE C EXECUTION BATCH).** Per v10.385 body diagnosis Tier-1 fix sequence (Finding P3 — prioritization organ consolidation). The working admin pillar-weights UI now consumes the v10.384 canonical accessor. Pre-v10.386: tab wrote `_lib["pillar_weights"] = _new_pw` then `save_kpi_library(_lib)` with sum=100 validation only. Post-v10.386: tab calls `save_pillar_weights(weights, actor=uname, reason=user_input)` which validates (sum=1.0, all 4 pillars, no zero/dead organs per §12 Flow Principle), appends to `pillar_weights_history.json` with OLD/NEW + timestamp + reason, and reports explicit error messages on rejection. Bundles v10.387 (History view) since accessor already exposed `get_pillar_weights_history`. Admin UI adds: (a) reason text input (optional, captured in history); (b) recent history panel below form (last 5 changes as expandable cards with OLD/NEW side-by-side); (c) explicit success/error toast messaging. Ships `docs/PILLAR_WEIGHTS_ADMIN_MIGRATION_v10.386.md` (7 Parts). Bank Identity tab deprecated form preserved untouched for v10.388 removal — single concern held strictly. Body-system framing: prioritization organ's working pathway now breathes through canonical lungs; the dead branch (Bank Identity tab) is visibly disconnected. The prioritization organ now ACTS healthily via the canonical UI. Phase C continues: v10.388 = remove Bank Identity dead form; v10.389 = remove pillars[].weight shadow data; v10.390 = remove org_config orphan + start Tier 1 Class B KPI implementation.
