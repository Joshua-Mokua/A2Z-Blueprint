# A2Z MIS 360 — Master prompt (v4.66)
...
**Current version: v10.423 (May 2026) - Pillar weights decision: Kaplan-Norton 40/25/25/10.** Per Joshua's decision: switched from bank-current 68/14/6/12 (Financial-heavy) to Kaplan-Norton balanced scorecard standard (Financial 40%, Customer Focus 25%, Operational Excellence 25%, People & Learning 10%). Saved via existing canonical path utils.pillar_weights_canonical.save_pillar_weights — single source of truth, same path admin UI uses. Change recorded in data/pillar_weights_history.json with OLD/NEW + actor + reason audit trail (§8.1). Admin can re-vary at any time via pages/7_admin.py -> KPI Library -> Pillar weights (canonical editor built v10.386 with validation: sum=1.0, no dead organs, all 4 pillars required). NO new engine module - admin editor and canonical save path were already in place. This batch ships ONLY the data change + G309 audit gate + decision documentation.
