# A2Z MIS 360 — Master prompt (v4.7)
...
**Current version: v10.363 (May 2026) — Charter §2 Football Team Test PASSES.** The acceptance criterion for the platform is now met and ratcheted. Adds `utils/teller_actions.py` (~250 lines) with discrete teller-action primitives: `fire_teller_deposit()`, `fire_teller_withdrawal()`, `find_first_deposit_account()`, `TellerActionResult` dataclass. Each helper mutates the bank via the production-grade `VirtualBankCore.update_account_balance` (preserves frozen-dataclass semantics by replacing rather than in-place mutation). Adds `tests/integration/test_v10363_charter_section_2.py` (11 tests) — THE canonical test proving the chain works end-to-end. The flagship test `test_v10363_teller_deposit_propagates_to_md_tile` fires a KES 100M teller deposit, persists via the v10.359 bridge, reads back via `compute_bank_aggregates`, and **asserts the bank-wide Deposit Growth changes by exactly the deposit amount within a 5s latency budget**. Additional coverage: deposit appears in correct segment, withdrawal decreases bank totals, multiple actions aggregate correctly, determinism preserved across runs, idempotent re-persist. **`virtual_bank_readiness.capture_readiness_report().chain.end_to_end_verified` is now `True`** — the readiness module runs a live propagation probe to verify the chain works. G249 audit gate locks: teller_actions module present + self_test passes + canonical test file exists + end-to-end propagation probe passes + latency < 5s. After v10.363 the Football Team Test chain reports: 7/7 WIRED, end_to_end_verified=True, READY_AND_VERIFIED. **Charter §2 is met: "The MD can see, in real-time, the impact of a teller's action on the bank's ROE."**
...
**Open / partially-closed gaps at v10.363:**
...
- **CBS-wired actuals arc — item 4 (PBT)** — PBT computation from CBS transactions. The slot is wired (bank_targets has PBT|2026 = 650B), but compute_bank_aggregates doesn't yet produce a PBT value. Calculation: NII + non-interest income - OpEx - impairment. Future batch.
- **BSC coverage 2.78% → 100%** — only 40 of 1,439 active staff have BSC actuals. Sustained data-engineering effort, not architecture.
- **System stocks live wiring** — 6/6 have snapshot accessors but return demo defaults. Wirable via compute_bank_aggregates.
...
- **Hard branch delete UI gap** — deliberate gap; soft delete via active=False is audit-traceable.
