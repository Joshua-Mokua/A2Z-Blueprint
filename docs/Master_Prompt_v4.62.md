# A2Z MIS 360 — Master prompt (v4.62)
...
**Current version: v10.419 (May 2026) - Role weight renormalization engine. Phase 2d data integrity housekeeping opens.** Per Joshua's locked backlog (225/227 roles broken with weight sums != 1.0). NEW utils/role_weight_engine.py (~300 LOC) API-first per v10.412 discipline (ZERO streamlit imports). Public API: audit_role_weight (per-role audit returning RoleWeightAudit dataclass with sum/is_normalized/normalization_factor), bank_role_weight_audit (bank-wide rollup), compute_role_normalized_weights (returns dict summing to 1.0), migrate_normalize_all_roles (additive migration), get_role_normalized_weight (single lookup). 2 dataclasses JSON-serializable. ADDITIVE migration adds role_normalized_weights field to kpi_library.json without touching existing kpi_weights dict - existing scoring code (auto-normalize-on-the-fly in core.py L6470-6476) continues unchanged. Live migration on sandbox library: pre 6/227 normalized, all 227 roles now have explicit normalized weights summing to 1.0. NEW scripts/normalize_role_weights.py runner with --dry-run flag. NEW 4 FastAPI endpoints in utils/api.py: GET /api/v1/role-weights/audit (bank-wide), GET /{role}/audit (single role), GET /{role}/normalized (one role's weights), POST /migrate (admin). Engine preserved 0/0/0/0.
