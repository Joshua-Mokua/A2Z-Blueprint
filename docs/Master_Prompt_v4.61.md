# A2Z MIS 360 — Master prompt (v4.61)
...
**Current version: v10.418 (May 2026) - Cascade-validation surgery (F3 integration).** Ties v10.416 F3 retain authorization into the cascade compliance display. Engine extension: compute_allocation_compliance (returns AllocationCompliance dataclass with 5 status values: fully_cascaded, retained_authorized, under_no_auth, over_allocated, no_target — 0.1%% tolerance for rounding). When manager has retain auth from boss, under-allocation status is "retained_authorized" (green/positive), not "under_no_auth" (red/violation). Coverage display in Health and coverage tab now shows compliance-aware Status column with appropriate colour mapping. NEW POST /api/v1/cascade/retain/compliance endpoint (router 29 routes). Pydantic ComplianceCheckRequest/Response. No save-logic change - the 100%% rule was display-only soft enforcement, so surgery happens at the display + audit layer only. Phase 2c closed with F-series; Phase 2d (data integrity housekeeping) begins next batch with v10.419. Engine preserved 0/0/0/0.
