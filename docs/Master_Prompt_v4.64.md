# A2Z MIS 360 — Master prompt (v4.64)
...
**Current version: v10.421 (May 2026) - Backup retention cleanup engine.** Per Joshua's locked backlog (122 MB stale .before snapshots; live sandbox shows 173 MB across 17 _v10*_backups dirs). NEW utils/backup_retention_engine.py (~250 LOC) API-first per v10.412 (ZERO streamlit). Public API: audit_backup_retention (returns BackupRetentionAudit + per-dir BackupDirInfo) + apply_retention_policy (with dry_run=True DEFAULT for safety). 3 dataclasses JSON-serializable. Default policy: preserve <1MB + top-3 most recent batch backup dirs. SAFETY: destructive operations require explicit opt-in (--confirm flag on runner script OR confirm=True query param on API). Live sandbox audit shows 99.2 MB reclaim available across 5 stale dirs (v10.392, v10.397, v10.398, v10.399, v10.401). Engine NEVER touches data/_canonical_backups (different purpose). NEW scripts/cleanup_backups.py runner with arg parser. NEW 2 FastAPI endpoints GET/POST /api/v1/backup-retention/audit + apply. Engine preserved 0/0/0/0.
