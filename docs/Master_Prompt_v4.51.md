# A2Z MIS 360 — Master prompt (v4.51)
...
**Current version: v10.408 (May 2026) — Target Scenario Simulator (E3).** Third of 7 QA-Standards enhancements. Per QA standards: "Managers cannot test what-if allocation scenarios." NEW engine utils/target_scenario_simulator.py (~420 LOC) — distinct from existing utils/scenario_simulator.py which handles risk/compliance scenarios. Provides load_current_scenario, simulate_alternative, compute_scenario, split_equal, split_weighted_by_history, _classify_likelihood (6-band classifier: very likely → unrealistic). PURE COMPUTATION — does not write to target_cascade.json. New tab "🧪 What-if simulator" added between Strategic impact and Cascade tree (is_mgr gated). UI: KPI+period selectors → current allocation with coverage → 4 preset split buttons (Equal / Weight by history / Reset / Zero) → per-row alternative number_inputs with historical hint → Compare scenarios button → side-by-side comparison table. Engine preserved 0/0/0/0.
