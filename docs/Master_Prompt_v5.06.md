# A2Z MIS 360 — Master prompt (v5.06)
...
**Current version: v10.462 (May 2026) - Manifest File Existence Hotfix; StreamlitAPIException eliminated; G343 enhanced.** Per Joshua reported runtime error: streamlit.errors.StreamlitAPIException 'Unable to create Page. The file 82_system_vitals.py could not be found.' at app.py:826 in _Page constructor (Streamlit's navigation builder). Plus Joshua's note: 'we are getting 100% REACT migration Ready but streamlit is what we are currently using to test what we are developing.' Root cause: manifest references pages that don't exist on every install (typically because an earlier batch updated pages/_manifest.json without shipping the actual page file). G343 (manifest invariant gate from v10.457) checked field completeness but not file existence. v10.462 fixes: (1) utils/page_manifest_loader.pages_in_department() now filters out manifest entries whose page files don't exist on disk - defensive: Streamlit will no longer crash on ghost entries even if they exist in the manifest; (2) NEW list_ghost_entries() public function for diagnostics; (3) G343 enhanced: now ALSO verifies every manifest entry points at a file that actually exists on disk (would have caught this class of bug); (4) Patch ships 82_system_vitals.py (340 LOC System Vital Signs Dashboard from v10.444 era) so Joshua's install has the actual file. Health unchanged from v10.461: 10 organs at Admin 78.4% / HR 78.3% / BSC 82.3% / Credit 75.8% / ICT 74.0% / Finance 73.9% / Treasury 68.2% / Legal 72.1% / Risk 74.7% / Compliance 74.5%; avg 75.2%. v10.463+ rescue continues: v10.463 deepen revival of 5 new organs (Phase 2 QA docs + Phase 4 cascade roles); ~80%. v10.464 bring Operations + CRM + Reporting/Analytics organs into revival fold; ~80% with body complete. v10.465+ final cert toward CERTIFIED revival x N organs. Body health 91.1%; 360 100%; BSC rescue 100% preserved.
