# A2Z MIS 360 — Master prompt (v4.59)
...
**Current version: v10.416 (May 2026) - F3: per-line-manager retain authorization.** Per Joshua's F3 design: 100% cascade required up to Branch Manager tier; below BM, the line manager's IMMEDIATE BOSS ticks "can retain" per direct report, allowing that report to retain (not cascade 100%) of their received targets. NEW utils/cascade_retain_engine.py (~280 LOC) API-first per v10.412 discipline - ZERO streamlit imports (AST-verified). Public API: is_eligible_for_retention (tier-1 role check via TIER1_ROLE_KEYWORDS covering MD/Director/Head/Regional Head/Branch Manager/Chief variants), set/get/remove_retain_authorization with full audit trail, is_retention_allowed convenience, get_team_retain_authorizations for boss-side rollup, retention_audit_summary bank-wide. 2 dataclasses (RetainAuthorization, RetentionAuditSummary) JSON-serializable. Persistence: data/retain_authorizations.json. NEW '🎯 Step 4 (optional) F3 Retain authorizations' expander in Set team targets - per-eligible-report row with checkbox + note + save + GRANTED/REVOKED status indicator with audit detail; tier-1 reports auto-excluded. NEW retention badge in My targets - green callout if granted, red if explicitly revoked, with authorizing manager + date. NEW 4 FastAPI endpoints in api_cascade.py (router now 26 routes total): GET/PUT/DELETE /retain/{staff_code}/{period}, GET /retain/summary/{period}. Cascade-percentage validation surgery (relaxing 100% rule for authorized reports) is DEFERRED - this batch ships the authorization surface; enforcement integration follows after the surface is exercised. F2 mechanics complete v10.415; F3 surface complete v10.416; only F5 dual-view BSC remains for full F-series closure at v10.417. Engine preserved 0/0/0/0.
