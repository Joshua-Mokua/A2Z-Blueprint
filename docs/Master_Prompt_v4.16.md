# A2Z MIS 360 — Master prompt (v4.16)
...
**Current version: v10.372 (May 2026) — Engine B Refactor (CONVERGED).** Fifth and final concrete unification step from the v10.367 architecture arc. Refactors `utils.sbu_pnl_rollup.bank_total_pnl` to accept `cost_source="canonical"` mode that consumes from `compute_pbt_by_customer` (the v10.370 atomic engine). Engine A (`compute_pbt_from_cbs`) and Engine B canonical now produce the **same** bank PBT — delta on seeded bank is 9 KES (0.0000001% of 7.9B). **G253 ratchets from INFORMATIONAL to ENFORCING**: when both engines are CBS-sourced via canonical mode, they MUST converge within 1% of bank PBT. Legacy `matrix` and `proxy` modes preserved for backward compatibility but documented as deprecated paths (they walk `customer_intelligence.json` — a different data source — so they'll always show ~98% divergence by design). Mapping PBTComponents → Engine B bucket schema: revenue ← operating_income, direct_cost ← impairment_charge (customer-specific LLPs), indirect_cost ← total_opex (allocated overhead), pbt ← pbt. **The full unification arc closes**: bottom-up atomic (G250, G254-G257), top-down atomic (G258), engine convergence (G253 ratchet). With all six identities locked, the MD's "Is the bank on track?" answer can be derived from any level — Bank, SBU, Branch, individual RM, individual customer, against both actuals and targets — and any consumer (Engine A direct or Engine B legacy/canonical) gets the same number. Module purity preserved (`_bank_total_pnl_canonical` uses lazy import of `customer_pbt_allocator` to avoid module-load cycles).
