# A2Z MIS 360 — Master prompt (v4.28)
...
**Current version: v10.384 (May 2026) — Canonical Pillar Weights Accessor (rescues body's prioritization organ).** Per Joshua's directive at v10.383 wrap-up: "after we need rescue body's prioritization organ". v10.382 deep review surfaced 3 storage locations + 2 admin UIs (one ORPHAN — §5.4 silent failure). v10.384 **SMOKING-GUN FINDING:** canonical (kpi_library.json::pillar_weights) currently 68/14/6/12 (financial-heavy crisis); orphan (org_config.json) currently 40/25/25/10 (Kaplan-Norton balanced) — TWO DIFFERENT views depending on which admin tab opened. Whoever set the balanced 40/25/25/10 via Bank Identity tab thought they'd rebalanced — the BSC engine never received the change. Three deliverables: (1) `docs/PILLAR_WEIGHTS_CANONICAL_v10.384.md` (7 Parts). (2) `utils/pillar_weights_canonical.py` (leaf module, 10 self-tests) — `get_pillar_weights` / `save_pillar_weights` (validates + writes canonical + appends history) / `validate_pillar_weights` (sum=1.0, all positive, no dead organs per §12 Flow Principle) / `get_pillar_weights_history` / `detect_orphan_pillar_weights` / `health_check`. (3) Admin Bank Identity tab gets prominent `st.warning` deprecation notice making the §5.4 silent failure VISIBLE; commitment captured in code: removal in v10.388. `data/pillar_weights_history.json` initialised; future saves append OLD/NEW values per §8.1 audit traceability. Consolidation continues v10.385-v10.390. Body-system framing: validation rules align with §12 Flow Principle (no dead organs, attention budget conserved). Next: v10.385 = proper deep body-wide diagnosis per Joshua's second directive.
