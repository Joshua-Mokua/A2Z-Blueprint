# A2Z MIS 360 — Master prompt (v4.36)
...
**Current version: v10.393 (May 2026) — Cascade Structure Audit Engine SHIPPED + TC32 (CRITICAL) Discovered.** Original v10.393 plan was surgical cross-branch cleanup (TC18, TC21, TC22). Attempted fix removed 25,137 cross-branch allocations but caused **771 staff (96.5% of leaf roles — 240/244 Tellers, 140/142 CSOs, etc.) to lose ALL cascade**. Investigation surfaced new CRITICAL finding **TC32**: the cascade has "representative sender" architecture — only **50 of 1449 staff (3.5%) appear as cascade senders**. ONE Branch Operations Supervisor (300228 @ Kenyatta Avenue) cascades to 386 Tellers bank-wide. ONE Branch Manager cascades for all 86 BMs. ONE Senior Branch Manager covers all 8. **TC32 is the root cause** explaining TC1 (MD over-allocates 10×), TC18 (cross-branch), TC22 (multi-sender ambiguity), TC25 (63% over-allocation), TC26 (ratio 386× over) — all are symptoms of representative-sender pattern. Surgical cleanup is impossible because cleanup removes the only cascade source for most leaf staff. **v10.393 pivots**: leaf module `utils/cascade_structure_engine.py` (~330 LOC, AST-verified leaf, zero upward utils.* imports) shipped with 4 result dataclasses (CycleFinding, RepresentativeSenderFinding, CrossBranchFinding, MultiSenderFinding), 4 detection functions (detect_cycles, detect_representative_sender_pattern, detect_cross_branch_violations, detect_multi_sender_ambiguity), full_audit aggregator, WITHIN_BRANCH_ROLE_PAIRS canonical set (15 pairs — Senior Branch Manager intentionally excluded as regional supervision is legit). 9 self-tests pass. Engine returns diagnostic data with no data changes. Live findings: 0 cycles (v10.392 still in effect), **58 CRITICAL representative-sender roles**, 3 WARN, **25,137 cross-branch violations**, **10,269 multi-sender ambiguities**. Cleanup attempt rolled back; `target_cascade.json` in v10.392 state; no v10.393 backup directory created (no real delta). **Joshua C5 decision noted**: "ratios use Fixed KPI mechanism — replicate MD-set value across all staff with that KPI; no cascade allocation" — the mechanism already exists in CascadeManager (get_fixed_kpis, get_fixed_value); BSC already consumes it; v10.394 just needs to register ratio KPIs as fixed. **TC18/TC21/TC22/TC25/TC32 all resolved by v10.395 re-cascade** (single batch using engine for validation). Foundation pattern continues: diagnostic engine before fix engine (same as v10.390 financial_ratios_engine). Ships docs/CASCADE_STRUCTURE_ENGINE_AND_TC32_v10.393.md (8 Parts).
