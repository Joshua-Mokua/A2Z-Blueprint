# A2Z MIS 360 — Master prompt (v4.21)
...
**Current version: v10.377 (May 2026) — Universal BSC Data Contract + Virtual Bank KPI Flow Unifier (Phase B opens).** Per the official Technical Governance Framework §5.1: "Every module MUST output data in the following standardized structure: staff_code, kpi_id, value, period, source_module." Per Joshua's directive: "have our virtual bank unify how all KPIs flow, test all modules and ensure every staff works and is measured." Establishes the **nervous-system layer** of the body-system framing — the signal-carrying contract that every canonical engine output flows through. Three deliverables: (1) `docs/A2Z_GOVERNANCE_CONSTITUTION_INTERNAL_v10.377.md` (~14KB, 8 Parts) codifying the constitutional mandates + mapping current state to target state + migration arc to PostgreSQL (Phase F). (2) `utils/bsc_universal_contract.py` (leaf module — zero upward imports; 10 self-tests) with `UniversalBSCRecord` + `make_record` + `validate_universal_record` + `validate_batch` + period format validation + source_module convention check. Schema mirrors `bsc_engine.submit()` signature exactly for forward-compat write-bridge. (3) `utils/virtual_bank_kpi_unifier.py` (~340 LOC, 8 self-tests) with `unify_bank_pbt` / `unify_sbu_pbt` / `unify_branch_pbt` / `unify_staff_pbt` / `unify_all_kpi_flow`. **End-to-end demonstration**: seeded bank → all canonical engines → 98 universal records (1 bank + 6 SBUs + 63 branches + 28 staff), 0 contract violations, Σ-identity reconciliation passes at every dimension within KES 6 (well under KES 100 tolerance). Each staff record carries v10.374 profitability_tier + sbu + role + department in metadata. Each record traceable via engine_gate (G250/G254/G255/G257). Body-system completion: skeleton (seniority v10.330) + circulatory (profitability v10.368-v10.375) + nervous (KPI flow v10.377 — JUST SHIPPED) + endocrine (audit gates G1-G263) + brain (constitution). Phase B opens: customer master merge moves to v10.378, write-bridge to v10.379, KPI-ID canonicalisation v10.380.
