# A2Z MIS 360 — Master prompt (v4.57)
...
**Current version: v10.414 (May 2026) - F2 part A: Cascade buffer engine + MD per-KPI cap.** Per Joshua's F2 architectural design (per-layer buffer, hidden from layers below, BSC dual-view). NEW utils/cascade_buffer_engine.py (~360 LOC) API-first per v10.412 discipline - ZERO streamlit imports (AST-verified). Public API: set/get/remove buffer cap (full audit trail set_by/set_at/note), validate_buffer (returns BufferValidation with ok/reason/cap_pct/proposed_pct), compute_effective_amount and extract_base_from_amount math helpers, summarize_cascade_buffer (rolls up cap utilization + violation detection). 3 dataclasses all JSON-serializable via asdict(). Persistence: data/buffer_caps.json. NEW '🛡️ F2: Per-KPI stretch caps' expander in Bank targets page (MD only) - set/update/remove caps + summary metrics + caps table with audit trail. NEW 6 FastAPI endpoints in api_cascade.py (router now 21 routes total): GET /buffer/caps, GET/PUT/DELETE /buffer/cap/{kpi}, POST /buffer/validate, GET /buffer/summary/{kpi}/{period}. F2 part B (per-allocation stretch slider in Set team targets) scoped to v10.415; dual-view BSC display scoped to v10.417 (F5). Engine preserved 0/0/0/0.
