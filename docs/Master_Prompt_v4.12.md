# A2Z MIS 360 — Master prompt (v4.12)
...
**Current version: v10.368 (May 2026) — SBU PBT Reconciliation.** First concrete unification step from the v10.367 architecture arc. Adds `compute_pbt_by_sbu(cbs_dir)` to `utils.pbt_computation` returning `Dict[str, PBTComponents]` keyed by SBU name (Retail Banking, Commercial Banking, Corporate Banking, Treasury, Digital/Agency, Unallocated). **The reconciliation identity holds exactly**: `Σ(SBU PBT) == Bank PBT` (within KES 100 — locked by G254). Six buckets are populated by attribution: customer-facing SBUs (Retail/Commercial/Corporate) get their NII + Non-Interest Income from accounts attributed via the customer's segment; operational SBUs (Treasury, Digital/Agency) get their config OpEx with zero customer-attributable income; Unallocated absorbs the ~700M OpEx gap between `opex_data::bank.total_opex` (7.9B) and `Σ(by_sbu.opex)` (7.2B) so the identity holds. The segment→SBU mapping lives in new `data/segment_sbu_mapping.json` (Rule N1 — admin-editable, handles all three concurrent naming conventions: segment_config locked codes AFFLUENT/CORE_MIDDLE/MASS/MICRO/SMALL/MEDIUM/CORPORATE; VirtualBankCore SegmentType RETAIL/HNW/PRIVATE_BANKING/SME/CORPORATE; opex_data SBU display names). The bridge (`virtual_bank_cbs_writer`) now writes `customers.csv` (cif, full_name, segment, branch_code, rm_code) as the CBS-native companion to `accounts.csv` — `compute_pbt_by_sbu` reads it for the segment lookup, so SBU computation works without external state. Bank-level `compute_pbt_from_cbs` unchanged; SBU dimension is purely additive. G254 audit gate locks: module exports + mapping config + customers.csv writing + the identity Σ(SBU PBT) = Bank PBT within tolerance. Charter §2 still passes (G249). v10.367 reconciliation diagnostic still informational (Engine B still uses different data source — closes in v10.371 Engine B refactor).
...
- ~~**SBU PBT reconciliation (closed v10.368)** — `compute_pbt_by_sbu` returns per-SBU PBTComponents; Σ(SBU PBT) == Bank PBT within KES 100. Mapping in `data/segment_sbu_mapping.json` (Rule N1). Bridge writes customers.csv as CBS-native lookup. G254 locks the identity. First concrete unification step done.~~

**Profitability unification arc (in flight — remaining batches):**

- **v10.369 — Per-branch PBT allocation engine.** `utils/branch_pbt_allocator.py` with admin-configurable driver (FTE-weighted default, revenue or hybrid available). Replaces legacy `aggregate_cbs_by_branch` formula. G255 locks Σ(Branch PBT) = Bank PBT. **G253 ratchets** to require ΔPBT < 5%.
- **v10.370 — Per-RM canonical refactor.** `rm_profitability.py` refactored to consume from canonical engine. G256 locks Σ(RM PBT) = Bank PBT.
- **v10.371 — Multi-level bank_targets.json schema extension.** `PBT|<level>|<entity>|<year>` keys for SBU / Branch / RM. Σ(child targets) == parent target. **G253 ratchets** to CONVERGED (<1%).
- **v10.372 — Engine B refactor.** `sbu_pnl_rollup.bank_total_pnl` refactored to optionally consume from `compute_pbt_by_sbu` (so Finance hub's drill-down stays sourced from canonical). Proxy mode preserved for backward compatibility; documented as deprecated.

**Open / partially-closed gaps at v10.368:**
...
- **Branch roles data generation** — at 94-branch scale.
...
*Master prompt v4.12 for v10.368 — SBU PBT Reconciliation. **Thirteenth consecutive batch holding lockstep discipline.** First concrete unification step from the v10.367 architecture arc. Adds compute_pbt_by_sbu() returning Dict[str, PBTComponents] keyed by SBU. THE RECONCILIATION IDENTITY: Σ(SBU PBT) == Bank PBT (delta 0 on seeded bank; tolerance KES 100 in G254). Six SBU buckets: Retail Banking, Commercial Banking, Corporate Banking, Treasury, Digital/Agency, Unallocated. Customer-facing SBUs get income from accounts attributed via customer's segment; operational SBUs (Treasury, Digital/Agency) get config OpEx only; Unallocated absorbs the 700M OpEx gap between opex_data::bank.total_opex (7.9B) and Σ(by_sbu.opex) (7.2B). New data/segment_sbu_mapping.json (Rule N1, admin-editable) unifies the three concurrent segment naming conventions: segment_config locked codes (AFFLUENT/CORE_MIDDLE/MASS/MICRO/SMALL/MEDIUM/CORPORATE), VirtualBankCore SegmentType (RETAIL/HNW/PRIVATE_BANKING/SME/CORPORATE), opex_data SBU display names. Bridge writes customers.csv (cif, full_name, segment, branch_code, rm_code) as the companion to accounts.csv — provides CBS-native customer→segment lookup. compute_pbt_from_cbs unchanged; SBU dimension is purely additive. G254 audit gate locks: module exports + mapping config + customers.csv writing + the identity Σ(SBU PBT) = Bank PBT within KES 100 tolerance. Charter §2 still passes; v10.367 reconciliation diagnostic still informational (Engine B refactor in v10.371). Audit 254; verifier 235/235; tests 15/15 in v10.368 + 136 prior = 151 total passing. G162 baseline at 4022 (62 consecutive zero-drift batches).*
