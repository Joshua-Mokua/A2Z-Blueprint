# A2Z MIS 360 — Master prompt (v4.37)
...
**Current version: v10.394 (May 2026) — Line Manager Hierarchy & Fixed KPI Mechanism Review (REVIEW ONLY).** Per Joshua's directive after v10.393 ship: "the fixed KPI is the reserve of the MD since they might change, we have a tap in the target cascade module where the MD ticks what needs to be fixed, remember also not all ratios are fixed e.g NPL varies from branch to branch. Then the cascade follows the line manager hierarchy, we had one defined which you also need to do a deep dive to review and see, it is the same hierarchy that flows upwards right from the pipeline module". CONFIRMED 4 architectural truths: (A1) Fixed KPI status is MD's reserve via existing tab in cascade module — MD ticks + enters value, replicates to all staff with that KPI; (A2) not all ratios are fixed — NPL Ratio varies per branch (correctly in `_v10324_removed_from_fixed` list alongside PBT/Total NFI/NIM/ROE/CIR which all vary per unit); (A3) cascade follows canonical line manager hierarchy at `org_hierarchy_config.json::role_manager_whitelist` (26 subordinate roles, uses actual data names like "Chief Retail Banking Officer" not aspirational "Director Retail Banking", "Area Manager" not "Regional Head"); (A4) Fixed KPI tab UI is correct and replicates value to all staff with KPI. **9 findings TC33-TC41**: TC33 canonical hierarchy LOCATION confirmed (role_manager_whitelist not "hierarchy"); TC34 cascade page reads wrong field name (looks for "hierarchy", always falls back to hardcoded); TC35 pipeline `_HIER` uses pre-canonical names (8 mismatches Managing Director/CEO&MD, Director Retail/CRBO, Regional Head/Area Manager, etc); TC36 cascade fallback HIERARCHY same wrong names; TC37 8 role-name divergences between pipeline/cascade and canonical; TC38 period mismatch (fixed_kpis quarterly 2026-Q1, cascade/bank_targets annual); TC39 **CRITICAL** NPL Ratio (removed-from-fixed, human name) vs NPL_RATIO (currently fixed, UPPERCASE) contradiction — same KPI under two naming conventions has two fix-statuses; TC40 cascade_structure_engine WITHIN_BRANCH_ROLE_PAIRS has 9 missing + 6 extra pairs vs canonical (built from inspection, not canonical); TC41 spelling duplicates in role_manager_whitelist (Senior Manager-Credit Analysis vs Senior Manager -Credit Analysis). **No new Joshua decisions surfaced** — his guidance answered C5 (ratios via Fixed mechanism) AND the hierarchy question (use canonical role_manager_whitelist). Revised execution sequence: v10.395 align WITHIN_BRANCH_ROLE_PAIRS to canonical (single concern, derived from role_manager_whitelist); v10.396 re-cascade using canonical hierarchy + Fixed KPI mechanism; v10.397 cascade page reads canonical (fix field name lookup); v10.398 pipeline page reads canonical (remove inline _HIER); v10.399 period harmonization quarterly↔annual; v10.400 NPL naming consolidation. REVIEW ONLY — zero code/data/gate changes beyond doc + G279 + 12 integration tests verifying findings on live data. Sister batch to v10.391 diagnosis and v10.393 engine. Ships docs/LINE_MANAGER_HIERARCHY_AND_FIXED_KPI_REVIEW_v10.394.md (10 Parts).
