# A2Z MIS 360 — Master prompt (v4.44)
...
**Current version: v10.401 (May 2026) — Period harmonization (TC38 resolved).** The three core files had inconsistent period schemas: `fixed_kpis.json` quarterly only (2026-Q1, 2026-Q2), `bank_targets.json` annual (PBT|2026), `target_cascade.json` annual (300001|PBT|2026). Pre-v10.401 regenerator used a "union all quarters" workaround. Built `utils/period_harmonizer.py` leaf module with explicit annual-period support: `get_fixed_kpis_for_period` (prefers annual key, falls back to quarterly union — backward compatible), `promote_quarters_to_annual`, `validate_period_consistency`, `set_annual_fixed_kpis`, `list_periods`. Updated cascade_regenerator `_get_fixed_kpi_set` to prefer annual key. Seeded explicit annual `2025` + `2026` keys by promoting from quarters (already consistent — 16 KPIs each). Production admin can now: maintain quarterly granularity (MD per Joshua A1 changes lock state per quarter), OR set authoritative annual list directly. Engine state preserved: 0/0/0/0. Cascade size unchanged (25,488 entries) since 2026 union = 2026 annual.
