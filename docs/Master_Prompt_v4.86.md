# A2Z MIS 360 — Master prompt (v4.86)
...
**Current version: v10.443 (May 2026) - HR Auto-Actuals Engine + Chief HR 360 Command Centre.** Per Joshua 5-strand directive: 'we need to ascertain that HR staff right from the Chief HR KPI in the BSC are fetching the actuals from the HR modules since part of what we're solving for is people having to key in actuals or send excels.' v10.443 addresses 3 of 5 strands directly + documents the other 2 for v10.444+. NEW utils/hr_actuals_engine.py ~650 LOC API-first ZERO streamlit. 4 public functions: compute_kpi_actual (per staff/KPI/period), compute_all_hr_actuals_for_staff (whole role_kpis), compute_bank_wide_hr_kpi (K018 K030 etc.), audit_auto_actuals_coverage. 2 JSON-serializable dataclasses. 8 per-KPI computers wiring HR module data: K016 Training Hours <- lms_enrollments, K121 Mandatory Training Completion <- lms_enrollments+lms_courses, K018 Staff Retention <- staff_history+register, K030 Headcount-vs-Budget <- register+branch_staff_config, Leave Days Taken + Leave Requests Approved <- leave_requests, PIPs Active + Disciplinary Active per-unit. HR_KPI_NON_AUTO set (K005/K021 Finance domain, K017 BSC self-ref, K019 360 feedback no source, K035 eNPS survey not in HR, K036/K037 projects). Training KPIs flagged partial=True so HR can supplement external training manually. Coverage 42.9% (6/14 HR-pillar KPIs auto-populated). NEW pages/81_chief_hr_centre.py ~553 LOC mirroring 100_md_cockpit design: 6 tabs (People Overview + HR KPI Auto-Actuals + Training & Dev + Performance Programs + Onboarding & Exit Risk + Financial Snapshot admin-configurable). Header shows total staff + on leave + active PIPs + discipline. Auto-actuals tab shows Chief HR their own auto vs manual KPI breakdown. Financial Snapshot has admin-configurable visibility (total comp, training budget, CI ratio, revenue vs budget, PBT - admin checkbox config stored in chief_hr_finance_visibility.json). 3 new FastAPI endpoints: hr-actuals/staff/{code}?period= + hr-actuals/bank-wide/{kpi}?period= + hr-actuals/coverage; all Depends(get_current_user). Department super-user role: design DOCUMENTED not built (v10.444+ RBAC across all depts). Staff loans + 1/3 salary rule: AUDITED not implemented in current modules (v10.444+ credit module work). 360 harmony 100% preserved; BSC rescue 100%; HR engine wiring 100%; engine 0/0/0/0.
