# A2Z MIS 360 — Master prompt (v4.15)
...
**Current version: v10.371 (May 2026) — Multi-Level bank_targets Schema (Top-Down Atomic).** Fourth concrete unification step from the v10.367 architecture arc. Extends `bank_targets.json` from a flat 2-segment schema (`<metric>|<year>`, 150 keys) to a hierarchical 4-segment schema (`<metric>|<level>|<entity>|<year>`) where `level ∈ {bank, sbu, branch, staff, customer}`. New `utils/bank_targets_schema.py` (~470 LOC, 16 self-tests) provides parse/compose/migrate/get/set/list/sum/validate primitives. **Critical backward compatibility**: legacy 2-segment keys are migrated IN-MEMORY to bank|all aliases (e.g. `PBT|2026` → also exposed as `PBT|bank|all|2026`); the file on disk stays exactly as written by admin, so existing tooling continues to work. **The hierarchy identity** (locked by G258): for any (metric, year) where child-level targets exist, `Σ(child) == bank|all target` within 0.1% tolerance (10bp, configurable). Sparse populations are explicitly OK — admins can populate only the levels they need (e.g. SBU targets first, branch later). Honors `_force_unbalanced_targets=True` override flag for admin edge cases. **Closes the top-down half of the reconciliation**: actuals atomic since v10.370 (G256, G257 lock Σ customer/staff PBT = bank); targets atomic since v10.371 (G258 locks Σ child targets = bank target). With both sides reconciled, the MD's "Is the bank on track to achieve its strategic goals?" answer is now derivable at every level: bank, SBU, branch, individual RM, individual customer. Module purity preserved (no upward imports; self_test uses synthetic dicts only). Production bank_targets.json validates clean (no child targets populated yet — sparse-OK). G253 still informational (Engine B refactor → v10.372 closes it). Three of four MD-level questions now answerable against the canonical reconciliation tree.
...
**Profitability unification arc (in flight — 4/5 done, 1 remaining):**

- ~~**v10.371 — Multi-level bank_targets.json schema (closed)**~~ — `utils/bank_targets_schema.py` extends keys to 4-segment `<metric>|<level>|<entity>|<year>`. Legacy 2-segment keys migrated in-memory. Σ(child targets) == bank target within 0.1% tolerance enforced by `validate_target_hierarchy`. G258 locks. Sparse populations OK.

- **v10.372 — Engine B refactor.** `sbu_pnl_rollup.bank_total_pnl` refactored to optionally consume from `compute_pbt_by_customer` (Finance hub drill-down stays sourced from canonical). Proxy mode preserved for backward compatibility one batch; documented as deprecated. **G253 ratchets to CONVERGED (<1%).**
...
*Master prompt v4.15 for v10.371 — Multi-Level bank_targets Schema. **Sixteenth consecutive batch holding lockstep discipline.** Fourth concrete unification step. Extends bank_targets.json from `<metric>|<year>` (150 flat keys) to `<metric>|<level>|<entity>|<year>` where level ∈ {bank, sbu, branch, staff, customer}. Ships utils/bank_targets_schema.py (~470 LOC, 16 self-tests) with full parse/compose/migrate/get/set/list/sum/validate primitives. Backward compatibility: legacy 2-segment keys exposed in-memory as bank|all aliases; file on disk unchanged for admins. THE FIFTH RECONCILIATION IDENTITY: Σ(child level targets) == bank|all target within 0.1% tolerance (10bp default, configurable). Sparse populations OK (only check levels with data). Honors _force_unbalanced_targets=True override flag for admin edge cases. G258 locks: module exports + live validation of production bank_targets.json. Closes the top-down half of the unification arc: actuals atomic since v10.370 (G256, G257); targets atomic since v10.371 (G258). MD's "Is the bank on track to achieve strategic goals?" can now be answered at every level. Module purity preserved (zero upward imports; self_test uses synthetic dicts only). Audit 258; verifier 269/269; tests 18/18 in v10.371 + 185 prior = 203 total. G162 baseline 4022 (65 consecutive zero-drift batches). G253 still informational pending v10.372 Engine B refactor.*
