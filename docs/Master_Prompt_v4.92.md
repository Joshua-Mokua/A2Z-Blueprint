# A2Z MIS 360 — Master prompt (v4.92)
...
**Current version: v10.449 (May 2026) - Credit Approval Hierarchy (4 levels) + Phone Disbursement.** Per Joshua directive: 'we are missing the Board Credit Committee (BCC) approval... we should have Branch Credit committee approval, Credit Analyst approval, Credit Committee approval, Board Credit Committee approval. Then within the process there are limits approved by a scoring matrix and disburses mostly by phone.' The credit_workflow engine already had branch tier infrastructure (TIER_BRANCH_AUTO/TIER_BRANCH_FWD + 3 branch roles + determine_branch_tier); v10.449 surfaces all 4 approval levels distinctly in the UI plus adds phone disbursement. pages/82_credit_approvals.py expanded 6 -> 8 tabs: (1) Swim Lane; (2) NEW Credit Analyst with scoring matrix AAA/AA/A/BBB/BB/B/CCC bands showing PD ceiling + auto-limit KES per band; (3) Branch Credit Committee (existing reindexed); (4) NEW Credit Committee (CCC) for TIER_2+TIER_3 head office central with Head of Credit/Risk/Business/Compliance; (5) NEW Board Credit Committee (BCC) for TIER_4 only with CEO+CFO+Risk+Credit+Board Credit Member, 80% threshold, total exposure metric; (6) Cast Vote (reindexed); (7) Decision History (reindexed); (8) Configuration (reindexed). pages/23_credit_admin.py NEW Phone Disbursement tab[3] with 3 metric cards (pending phone call/attempted-not-completed/disbursed) + phone call logging form with 5 outcomes (DISBURSED/CUSTOMER_NOT_REACHED/KYC_DOC_OUTSTANDING/CUSTOMER_WITHDREW/CALLBACK_REQUESTED) persisting data/phone_disbursement_log.json + recent call log + K028 BSC trigger; 23_credit_admin promoted stub 112 LOC -> substantial 286 LOC. Backups data/_v10449_backups/. Credit health 83.0% -> 84.8% (+1.8 pp); page completeness 57.1% -> 64.3% (9 substantial); severity 2 high -> 1 high. G335 enforces 8 tabs + 4 approval level labels + scoring matrix bands + BCC role list + phone disbursement markers + K028 trigger + 23_credit_admin >= 250 LOC + backups + credit health >= 84% + 360 + BSC preserved. Body health 91.1%; 360 100%; BSC 100% preserved; engine 0/0/0/0.
