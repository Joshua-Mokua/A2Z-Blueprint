# A2Z MIS 360 — Master prompt (v4.70)
...
**Current version: v10.427 (May 2026) - BSC completeness for 9 chiefs (BSC Rescue batch 3).** Per v10.424 audit: 8 chief-level staff with incomplete BSCs (6 at 2/8 KPIs, 2 at 7/8). v10.427 engine extended check to "configured vs current" via kpi_library.role_kpis - found 9 incomplete (added Gregory Chirchir at 9/14). NEW utils/bsc_completeness_engine.py (~500 LOC) API-first (ZERO streamlit, dry_run=True default). Public API: audit_bsc_completeness + repair_bsc_completeness + repair_code_alias_artifacts. CODE_ALIAS_MAP resolves 6 SNAKE_CASE codes (LOAN_GROWTH, AUDIT_SCORE, LEGAL_SLA_DOCS/ATTORNEY/VALUATION/SECURITY) to canonical human-readable library names. 4 JSON-serializable dataclasses. Live migration: 98 new BSC rows added across 9 chiefs with peer-median targets/actuals + 9 weight renorms to 1.0 + 6 SNAKE_CASE artifacts cleaned (renamed to human names + library aliases added) + 4 duplicate rows for Mark Charo deduped + 1 Loan Book Growth pillar correction. BSC health 57.1% -> 71.4% (+14.3 points; 5/7 categories now clean). NEW scripts/repair_bsc_completeness.py runner with --confirm. NEW 2 FastAPI endpoints /api/v1/bsc-completeness/audit + repair. 21st React-ready engine. Engine preserved 0/0/0/0.
