# A2Z MIS 360 — Master prompt (v4.87)
...
**Current version: v10.444 (May 2026) - Body Health Engine: Joshua's operating mantra codified.** Per directive: 'we need to embed and constantly run tests on every module we are reviving. How is their health being maintained? Are things we add affecting them? How is blood circulation in/out of these organs? Is information flowing in linear AND non-linear manner? The plan is to rescue the body 100% and prevent it from ever falling apart. We should retain this as our operating mantra.' NEW utils/body_health_engine.py ~750 LOC API-first ZERO streamlit (31st React-ready engine). 7 ORGAN_REGISTRY entries: BSC, Cascade-BSC 360, Target Cascade, HR Section, Standards Wiring, HR Auto-Actuals, Engine Baseline - each with rescue batch citation + audit gate(s) + per-organ probe function. 9 CIRCULATION_FLOWS (3 linear + 6 non-linear): bank_targets->cascade->BSC->score backbone; LMS->BSC actuals, PIP->BSC trigger, wellness->predictive, onboarding->cascade, exit->succession, HR-engine->API non-linear loops. 9 DETERIORATION_CATALOGUE risks with severity (critical/high/medium/low) + per-risk detector functions + mitigation gate citations. Master body_full_audit() returns BodyHealthReport with overall_body_pct (70% organ avg + 30% circulation - 2%/critical risk active) + mantra_status (100%/below_100/regressing). Per-run _AUDIT_CACHE eliminates redundant audit calls within one body audit. History persistence to data/body_health_history.json via record_health_snapshot() with audit_health_trend(organ, n) for trend analysis. Super-user mapping AUDITED: 21 chiefs + MD have is_dept_super_user across 22 functional depts; schema present, enforcement (accessible_modules) empty - v10.445 work. Current measured body health: 91.1% (4/7 organs healthy BSC+360+Cascade+Baseline 100%; HR 89.6%; Standards 78.8%; HR Auto-Actuals 42.9% capped by available data). 9/9 circulation flows active. 0 active deterioration risks (immune system fully operational). G330 codifies floors: body >= 85%, all rescued organs preserved at rescue levels. 360 harmony 100% preserved; BSC rescue 100%; HR section health 88.7%+; engine 0/0/0/0.


---

## ⚕️ OPERATING MANTRA (Joshua, v10.444)

> **"Rescue the body 100% and prevent it from ever falling apart."**

Codified in `utils/body_health_engine.py` (v10.444) and enforced by **G330**. Every revived organ has a measurable health floor that cannot regress. Every cross-organ information flow is testable. The deterioration catalogue lists what would degrade each organ + the audit gate(s) that protect against it.

Running discipline going forward:
- Every batch runs `body_full_audit()` as part of CI/audit suite
- G330 fails the build if any rescued organ drops below its floor
- New batches must pass G330 before being merged
- `data/body_health_history.json` persists snapshots for trend analysis

This is not optional. The body cannot fall apart while G330 holds.
