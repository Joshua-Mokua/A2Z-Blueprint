# A2Z MIS 360 — Master prompt (v4.52)
...
**Current version: v10.409 (May 2026) — Negotiation Escalation Chain (E4) + KeyError critical fix.** Fourth of 7 QA-Standards enhancements. Per Joshua's live error report: KeyError: 'from_code' at pages/12_cascade.py when cascade.items() iterated over underscore-prefixed meta-keys (_v10397_regenerated, etc.). v10.409 fixes: defensive guards on ALL cascade.items() loops in cascade page (lines 1235/2470/3642/3677) and utils/core.py — skip k.startswith("_") + isinstance(v, dict) + "from_code" presence check. Plus E4 escalation API: resolve_review extended with counter_target + escalate_to + escalate_to_name parameters; new statuses Counter-Proposed and Escalated; Escalated re-opens as Pending for skip-level. auto_escalate_overdue_reviews(sla_days=7) marks 7-day-old pending as SLA-breached. UI: 4-option decision selector, conditional counter/escalate inputs, SLA warning badges, history expander, admin manual SLA trigger.
