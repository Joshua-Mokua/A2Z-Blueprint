# A2Z MIS 360 — Master prompt (v4.1)
...
**Current version: v10.357 (May 2026) — Virtual Bank Readiness Audit (reconnaissance batch before live bring-up).** Reconnaissance batch surveying the existing virtual-bank infrastructure built over v10.30-v10.314+: `utils/virtual_bank.py` (foundation, 717 LOC), `utils/virtual_bank_core.py` (mock FLEXCUBE + entity simulator, 1167 LOC), `utils/virtual_bank_simulator.py` (daily ops simulator + scenario injection, 1323 LOC), `utils/scenario_simulator.py` (cross-arc executable scenarios, 18,026 LOC with 186 registered scenarios + ScenarioRunner), `utils/stress_testing.py` (CBK supervisory stress, 591 LOC), `utils/strategy_simulator.py` (628 LOC), `utils/hybrid_scheduling_simulator.py` (491 LOC), `utils/liquidity_stress.py` (744 LOC). v10.357 produces `data/virtual_bank_readiness.json` schema-locked report capturing: 8/8 modules load cleanly, 5/5 modules with self_test pass (virtual_bank_core 27 tests, virtual_bank_simulator 23 tests, scenario_simulator 18 tests, stress_testing 15 tests, liquidity_stress 18 tests), VirtualBankCore + Simulator boot end-to-end (5-day run executes; 0 customers/accounts/transactions generated against empty bank — confirms pipeline integrity, identifies seed-the-bank as v10.358 prerequisite), 4/4 scenario sample passes via ScenarioRunner with test engine bundle, virtual_bank.coverage_report shows 1,439 active staff with 100% KPI mapping but only 40 (2.78%) have BSC actuals, 22/22 departments have clean BSC submission path. **Football Team Test chain status: 5/7 WIRED + 2/7 PARTIAL.** WIRED: CBS→actuals_engine, actuals_engine→YoY sidecar (v10.355), YoY→BSC display (v10.355), BSC→branch score, branch→regional rollup. PARTIAL: teller→CBS (virtual_bank_simulator generates transactions in VirtualBankCore memory but does not persist to cbs_data/*.json — bridge module needed), regional→MD tile (bank-targets binding incomplete per roadmap item 2). G243 audit gate locks the load/self-test/boot/scenarios baseline. Status: **READY_BUT_NOT_VERIFIED** — infrastructure is sound; end-to-end Football Team Test verification deferred to v10.358+ with concrete roadmap (Link 1 bridge + Link 7 MD tile + end-to-end integration test + seed-the-bank step).
...
**Audit gates: 243** (G1 through G243). Distribution: G1-G14 foundational; G15-G117 standards-coverage; G118+ QA framework; G128 structural audit baseline (StructureAuditEngine with 7 rule families); G143 KPI source coverage; G161 manifest-canonical lock; G162 tenant-hardcoding ratchet (kaizen baseline 4022 — 51 consecutive zero-drift batches); G230 protected-files schema validation; G231 page module-load smoke; G232-G236 hub-consolidation thresholds; G237 redirect signaling; G238 static AST function checks; G239 dynamic render-function smoke; G240 CBS baseline snapshot; G241 live actuals YoY sidecar; G242 master prompt sync (anti-drift lockstep); G243 virtual bank readiness audit.
...
**Open / partially-closed gaps at v10.357:**

- **Football Team Test end-to-end verification** — 5/7 chain links WIRED, 2/7 PARTIAL (per v10.357 readiness audit). Link 1 (teller→CBS): virtual_bank_simulator generates transactions in VirtualBankCore memory but does not persist to cbs_data/*.json — bridge module needed (v10.358). Link 7 (regional→MD tile): bank-targets binding incomplete per roadmap item 2. End-to-end integration test = v10.360 candidate.
- **Seed-the-bank step** — v10.357 boot probe ran 5-day simulation against empty VirtualBankCore (0 customers/accounts/transactions generated). Infrastructure exists across the simulator modules but no single helper does the full seeding. Prerequisite for meaningful Football Team Test runs.
- **CBS-wired actuals arc — item 4 (deferred)** — PBT computation directly from CBS transactions. Currently the actuals xlsx carries PBT as an absolute value from upstream; goal is to derive it from CBS as NII + non-interest income - OpEx - impairment. Re-prioritized after v10.358-v10.360 close the Football Team Test chain.
- **BSC coverage 2.78% → 100%** — only 40 of 1,439 active staff have BSC actuals today (per v10.357 coverage report). Closing this is the live bring-up's primary outcome.
- **System stocks live wiring** — 6/6 have snapshot accessors but currently return demo defaults. Wiring to CBS aggregates (via v10.354 baseline structure) is the natural next maturity step.
- **MD BSC bank-targets binding (roadmap item 2)** — once bank targets are set in Target Cascade → Bank Targets, the MD BSC should display them. Maps directly to Link 7 of Football Team Test chain.
- **Branch roles data generation (roadmap item 5)** — some branches lack the full role complement in synthesized data. Generator-script fix.
- **Audit performance** — full audit takes >5 min due to pre-existing slow gates (G117 ~37s, G204/G217 ~15s each, G231 ~13s, G128 ~9s, G243 ~6s). Candidate for parallelization / incremental caching.
- **Strategic Initiative engine / Partnerships P&L / B-027 tail** — earlier roadmap items partially built; full integration deferred until live bring-up completes.
- **React executive frontend** — Standard #9 calls for React on executive dashboards. Build is the next major arc after virtual-bank live bring-up completes.
- **Documented data-shape divergences (deferred)** — `strategic_initiatives.rag_status` Title-vs-UPPER, `kpi_library.kpis[].direction` short-vs-long. Both locked in schemas with all values allowed.
