# A2Z MIS 360 — Master prompt (v4.50)
...
**Current version: v10.407 (May 2026) — Strategic Pillar Visualization (E2).** Second of 7 QA-Standards enhancements. Per QA standards: "Employees don't see how their targets connect to bank strategy." New engine `utils/pillar_impact_engine.py` (~380 LOC) provides `pillar_breakdown_for_staff/manager` + `bank_pillar_weights` + `kpi_to_strategic_pillar_map` with module-level caches (TARGET_CACHE/ACTUAL_CACHE) to avoid O(N²) on 808-sub subtree iteration. New tab "🎯 Strategic impact" added between Team progress and Cascade tree. Visible to ALL staff (Teller→MD). Renders: bank pillar weight horizontal bar (color-coded), personal pillar breakdown table (your KPIs / your weight / targets set / actuals), per-pillar KPI list expanders, and for managers an aggregated team pillar distribution recursive across subtree. Engine preserved 0/0/0/0.
