# A2Z MIS 360 — Master prompt (v4.33)
...
**Current version: v10.390 (May 2026) — Bundle: Prioritization Rescue 5/5 COMPLETE + Tier-1 Class B KPI Foundation.** Joshua-approved Rule N2 exception ("bundle"). Two concerns delivered together: **(A)** `data/org_config.json::pillar_weights` orphan field REMOVED — health_check.orphan_detected flips to None. Body's prioritization organ rescue arc COMPLETE in 5 batches: v10.384 canonical accessor → v10.386 UI migration → v10.388 dead form removal → v10.389 shadow data removal → **v10.390 orphan field removal**. Post-rescue: ONE storage location (kpi_library.json::pillar_weights), ONE admin UI (KPI Library tab), audit history captured, validation enforced, NO shadows, NO orphans. Constitution §12 Flow Principle achieved for pillar weights. **(B)** `utils/financial_ratios_engine.py` LEAF module shipped — AST-verified leaf (zero upward utils.* imports). Exports: 4 result dataclasses (NIMResult, CIRResult, ROEResult, DepGrowthResult) + 4 compute functions (compute_nim, compute_cir, compute_roe, compute_total_deposit_growth) + aggregator (compute_all_financial_ratios). Reads from `data/mgmt_accounts.json` (single source of truth — already had every needed field: income_statement.interest_income/expense/fee_income/forex_income/opex/provisions + balance_sheet.loans_net_b/investments_b/equity_b/customer_deposits_b). LIVE VALUES from production data: **NIM 1.15%** (vs target 4.5% — significant gap), **CIR 53.67%** (vs target 55% — slightly better than target ✓), **ROE 4.26%** (vs target 15% — gap; uses 30% Kenya tax proxy per K3 pending), **DEP_GROWTH 3.18%** (vs target 10% — uses 10%-prior placeholder bop, real growth needs cbs_baseline integration). Each result carries headline ratio + components used (§8.1 audit traceability) + status flag ("computed"/"partial"/"missing") + notes disclosing limitations. 9 embedded self-tests pass. 4 KPI library entries added (NIM/CIR/ROE/DEP_GROWTH) all active=False per Decision K7 (don't auto-activate; await MD targets). Backup preserved at `data/_v10390_backups/org_config.json.before`. Independent validation: computed CIR 53.67% matches bank's published key_ratios.cir_pct 53.7% within ±0.5% — engine is producing correct numbers. v10.391 = customer_focus_engine (NPS + DIGITAL_ACT). v10.392 = MD target setting + Tier-1 KPI activation. Ships `docs/RESCUE_COMPLETE_AND_FINANCIAL_RATIOS_v10.390.md` (8 Parts).
