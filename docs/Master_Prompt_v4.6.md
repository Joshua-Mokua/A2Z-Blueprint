# A2Z MIS 360 — Master prompt (v4.6)
...
**Current version: v10.362 (May 2026) — Link 7 MD tile bank-targets binding. ALL 7 CHAIN LINKS WIRED.** Closes the last PARTIAL link of the Football Team Test chain (Charter §2). The MD's BSC view now reads **bank-wide targets** from `data/bank_targets.json` (150 KPI×year entries) and **bank-wide actuals** from `utils.actuals_engine.compute_bank_aggregates` (24 KPIs computable from CBS), with 20+ KPIs in overlap providing the "on track?" view. Mechanical wiring verified end-to-end: `bank_targets.json` → `CascadeManager._load_bank` → `pages/1_perform.py::_is_md_view` branch → `_casc_targets` populated → MD's KPI rows display target column. For actuals: `_get_bank_aggregate_roles` identifies CEO + 11 direct reports (the 12 executive roles); `_build_from_cbs` injects `compute_bank_aggregates` values into those roles' `YTD_Actual`. v10.362's end-to-end verification surfaced a **category-case bug** in v10.359's bridge: `_ACCT_TYPE_TO_CATEGORY` was writing `"LOAN"` / `"TERM"` (uppercase), but `actuals_engine` expects `"Loan"` / `"Term Deposit"` (Title case) for its branch-routing logic. v10.362 fixed the bridge: `LOAN→Loan`, `FIXED_DEPOSIT→Term Deposit`. After the fix, `compute_bank_aggregates` correctly produces Loan Book Growth and full Deposit Growth from seeded CBS data. G248 audit gate locks the binding + the case fix + end-to-end probe. **Football Team Test chain: 7/7 WIRED.** `end_to_end_verified=False` is correct — that requires the integration test (v10.363). After v10.363 → **Charter §2 PASSES.**
...
**Audit gates: 248** (G1 through G248). Distribution: G1-G14 foundational; G15-G117 standards-coverage; G118+ QA framework; G128 structural audit baseline; G143 KPI source coverage; G161 manifest-canonical lock; G162 tenant-hardcoding ratchet (kaizen baseline 4022 — 56 consecutive zero-drift batches); G230 protected-files schema validation; G231 page module-load smoke; G232-G236 hub-consolidation thresholds; G237 redirect signaling; G238 static AST function checks; G239 dynamic render-function smoke; G240 CBS baseline snapshot; G241 live actuals YoY sidecar; G242 master prompt sync (anti-drift lockstep); G243 virtual bank readiness audit; G244 virtual bank seed determinism; G245 CBS persistence bridge integrity (Link 1); G246 branch single source of truth (v10.361 strengthened — no hardcoded fallback); G247 admin CRUD coverage (branch + staff Add/Edit/Delete); G248 Link 7 MD tile bank-targets binding (Charter §2 mechanical wiring).
...
**Open / partially-closed gaps at v10.362:**

- **Football Team Test end-to-end verification** — 7/7 chain links WIRED but `end_to_end_verified=False`. The integration test (v10.363) fires a synthetic teller transaction and asserts it propagates to the MD tile within a deterministic latency budget. **After v10.363 → Charter §2 PASSES.**
- **FLEXCUBE branch/staff live integration** — Stubbed at v10.361. When Apigee gateway access provisioned, replace stub bodies with real REST calls. No caller-code changes.
...
- **System stocks live wiring** — 6/6 have snapshot accessors but currently return demo defaults. Wirable now via compute_bank_aggregates.
- **Branch roles data generation (roadmap item 5)** — at 94-branch scale.
- **Region cleanup (45/94 branches marked "Other")** — admin module can re-classify.
- **Audit performance** — full audit >5 min.
- **strategy_simulator + hybrid_scheduling_simulator lack self_test**.
- **Bridge NPL aging buckets are zeros** — `npl_aggregate.json::by_aging_kes` zeroed.
- **Hard branch delete UI gap** — soft delete (active=False) is the audit-traceable pattern; documented deliberate gap.
