# A2Z MIS 360 — Master prompt (v4.74)
...
**Current version: v10.431 (May 2026) - Admin polish: validation engine + library cleanup.** Per Joshua roadmap: finish fixing admin before 360 cascade review. v10.431 builds the guardrails that prevent re-drift via admin editors (the BSC Rescue fixed historic data; v10.431 stops it returning). NEW utils/admin_validation_engine.py (~600 LOC) API-first (ZERO streamlit). 5 public validators: validate_kpi_change, validate_pillar_weights, validate_role_kpis_change, validate_target_override, validate_full_library. Plus apply_legacy_code_aliases migration. 3 JSON-serializable dataclasses (ValidationIssue, ValidationResult, LegacyAliasResult). LEGACY_CODE_ALIAS_MAP: 22 SNAKE_CASE->canonical mappings. EXTENDED bsc_library_register_engine LIBRARY_PILLAR_FIX_MAP with "Risk"->"Financial" (caught 3 KPIs v10.426 missed: PRODUCT_NPL_RATE, LCR, NSFR). EXTENDED bsc_admin_panel.py with render_library_validation_panel showing errors/warnings/info + legacy-alias migration button. EDITED pages/7_admin.py BSC Health tab wires the new panel. NEW 2 FastAPI endpoints /api/v1/admin-validation/library + legacy-aliases. Live data fixes: 3 library entries Risk->Financial + 22 legacy code aliases added (warnings 27 -> 3, last 3 are ambiguous ACTIVE_ACCTS awaiting admin choice). 24th React-ready engine. BSC health 100% preserved. Engine 0/0/0/0.
