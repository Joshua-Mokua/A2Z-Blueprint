# A2Z MIS 360 — Master prompt (v4.53)
...
**Current version: v10.410 (May 2026) — Tab consolidation (10→6) + Co-KPI chief pairing.** Per Joshua's two UX directives: (1) "Tabs are now more than 6 and still continuing — after six we start a new" — _tab_defs reduced to 6 top-level (bank_setup / cascade_alloc / my_view / team_analytics / health / negotiation) with sub-tabs inside each via _SUBTAB_MAP + _build_sub_tabs(). _in_tab() now returns container; existing 10 handler blocks updated. (2) Co-KPI chief pairing — feature never built; now NEW data/kpi_ownership_map.json with 12 shared KPIs + NEW utils/kpi_ownership_pairing.py (~280 LOC) + 🤝 Co-KPI pairing sub-tab (MD-only) supporting 3 strategies (equal_split / by_prior_year / manual %). Engine preserved 0/0/0/0.
