# A2Z MIS 360 — Master prompt (v4.60)
...
**Current version: v10.417 (May 2026) - F5: dual-view BSC render. F-series CLOSED.** Per Joshua's F5 design (continuing F2): in BSC scorecard / My targets view, show STRETCH as the primary metric (what staff is striving for) with BASE as a secondary aside (the un-stretched cascaded amount). Engine extension: compute_dual_view (returns DualViewEntry per assigned KPI with base/stretch/effective/has_stretch/from_code/from_name), get_dual_view_summary (totals rollup across KPIs). NEW per-KPI base aside in My targets cards when stretch present, showing "Base: X · +Y% stretch" small text under primary amount. NEW stretch summary callout below cards: "X of Y KPIs include stretch, adding +Z% on average over base". NEW 2 FastAPI endpoints in api_cascade.py (router now 28 routes): GET /dual-view/{staff_code}/{period} (list) + GET /dual-view/.../summary (rollup). Pydantic DualViewEntryResponse/DualViewSummaryResponse. F-series complete: F2 (v10.414+v10.415) cap + per-allocation stretch tuner, F3 (v10.416) retain authorization surface, F4 (v10.404) regenerator preserves manual, F5 (v10.417) dual-view render. Engine preserved 0/0/0/0.
