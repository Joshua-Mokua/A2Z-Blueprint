# A2Z MIS 360 — Master prompt (v4.2)
...
**Current version: v10.358 (May 2026) — Seed-the-Bank Helper (closes v10.357 readiness audit's "empty bank" blocker).** Adds `utils/virtual_bank_seed.py` (~500 lines) with a deterministic seeder that populates VirtualBankCore from existing platform data: 21 branches sourced from `utils.core.BRANCH_REGION` (Mombasa, Nairobi CBD, Eldoret, etc. with their South/Central/North regions), 419 active RMs pulled from `data/users.json`, customer/account/loan distributions synthesized from a small fixed name pool with segment-mix and balance-range proportionality (RETAIL 70%, SME 20%, CORPORATE 6%, HNW 3%, PRIVATE_BANKING 1%). Configurable scale: `SeedConfig.small()` = 100 customers / 200 accounts / 30 loans / 21 branches / 30 RMs (default for unit tests + Football Team Test harness); `medium()` = 1,000 customers; `large()` = 10,000 customers. **The seeder is wired into the readiness audit's `_probe_boot()`**: the 5-day simulation now generates ~2,645 transactions against a seeded bank, up from 0 in v10.357. Deterministic — same `base_seed` produces same totals byte-for-byte across runs (Park-Miller LCG, no wall-clock). Referential integrity verified: every account/loan CIF maps to a real customer, every branch_code maps to a real branch. G244 audit gate locks the determinism contract + ECOBANK_BRANCHES sync with utils.core.BRANCH_REGION. The seeder is the prerequisite for v10.359's Link 1 teller→CBS persistence bridge and v10.361's end-to-end Football Team Test integration test. **Football Team Test chain unchanged: 5/7 WIRED, 2/7 PARTIAL** — v10.358 is enabling infrastructure, not a chain-link closure.
...
**Open / partially-closed gaps at v10.358:**

- **Football Team Test end-to-end verification** — 5/7 chain links WIRED, 2/7 PARTIAL. Link 1 (teller→CBS): virtual_bank_simulator generates transactions but does not persist to cbs_data/*.json — bridge module is v10.359. Link 7 (regional→MD tile): bank-targets binding incomplete per roadmap item 2 — v10.360. End-to-end integration test = v10.361 candidate.
- **CBS-wired actuals arc — item 4 (deferred)** — PBT computation from CBS transactions. Re-prioritized after v10.359-v10.361 close the Football Team Test chain.
- **BSC coverage 2.78% → 100%** — only 40 of 1,439 active staff have BSC actuals. Closing this is the live bring-up's primary outcome. Depends on Link 1 + actuals refresh propagation.
- **System stocks live wiring** — 6/6 have snapshot accessors but currently return demo defaults. Wiring to CBS aggregates is the next maturity step.
- **MD BSC bank-targets binding (roadmap item 2)** — once bank targets are set in Target Cascade → Bank Targets, the MD BSC should display them. Maps directly to Link 7.
...
- **strategy_simulator + hybrid_scheduling_simulator lack self_test** — surfaced by v10.357 readiness audit. If production-grade, they should grow self_tests.
