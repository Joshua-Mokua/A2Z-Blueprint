# A2Z MIS 360 — Master prompt (v4.25)
...
**Current version: v10.381 (May 2026) — Customer Profitability Canonical Refactor + Recommendations on Part 9 Decisions (Phase B continues).** Per Joshua's two-part directive: continue Phase B roadmap + recommend on the 8 v10.380 decisions through "body function as one" lens. Three deliverables: (1) `docs/CUSTOMER_PROFITABILITY_CANONICAL_REFACTOR_v10.381.md` (7 Parts) — surgical refactor making `customer_profitability.py` consume v10.378 unified customer master. Public API unchanged; dependency injection paid for itself. Engine consumer field (segment) round-trip verified — legacy 'Mass' == canonical 'Mass'. Engine now sees superset of fields including sources, enrichment_status, _field_lineage from v10.378. (2) `docs/V10380_DECISIONS_RECOMMENDATIONS_v10.381.md` (~12KB, 8 Decisions) — body-system recommendations: Decision 1 tiered add (NIM/CIR/ROE/NPS + 5 LEGAL_* + DEP_GROWTH + DIGITAL_ACT = 9 new; NEW_CUST → alias; 4 defer; PAR verify). Decision 2 return to balanced 40/25/25/10 (Donella Meadows: single-variable optimization destroys complex systems). Decision 3 phased K-code retirement. Decision 4 cascade_meta top-level field. Decision 5 active=null → false. Decision 6 role_status metadata. Decision 7 ~10 cbk_ref populations. Decision 8 SCREAMING_SNAKE wins (91% already). (3) Module changes to `utils/customer_profitability.py`: _canonical_customer_lookup_v10381 (NEW), _legacy_customer_intelligence_lookup (RENAMED from _default_customer_lookup impl), reset_canonical_customer_cache (NEW test helper), _UNIFIED_MASTER_CACHE (module-level). Default lookup now canonical-first with legacy fallback. All 42 existing `tests/test_customer_profitability.py` tests pass unchanged. v10.382 = apply same pattern to `rm_profitability.py`.
