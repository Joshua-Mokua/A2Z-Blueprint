# A2Z MIS 360 — Master prompt (v4.89)
...
**Current version: v10.446 (May 2026) - Credit Section Diagnostic (Phase 1 of Heart Rescue).** Per Joshua doctrine: 'Credit MODULE REVIVAL - the heart of the bank, we must endeavour to revive and rescue it to 100%. 13+ modules mapped; determine if all genuinely belong, if others fit as modules or tabs. Flow: pipeline -> analysis -> administration -> monitoring -> DRU; how it links with legal, compliance, credit approvals, swim lane.' v10.446 = Phase 1 audit-only diagnostic, same diagnose-first pattern that worked for BSC (v10.432) and HR (v10.436). NEW utils/credit_section_audit_engine.py ~700 LOC API-first ZERO streamlit. 6 audit dimensions: (1) module_placement 13/13 pages in credit dept = 100% (no misplacements like CIMS/SLA in HR); (2) page_completeness 7 substantial + 5 stubs + 1 redirect = 53.8% (stubs: 23_credit_admin 112LOC, 39_ews 131, 40_collateral 109, 70_retailer_finance 167, 71_bid_bond 178); (3) engine_wiring 5 of 8 = 62.5% (credit_committee + credit_risk_scoring + credit_alt_scoring + ifrs9_classification + provisions wired; credit_workflow=SWIM LANE completely unwired, credit_risk_irb in risk dept not credit, analytics_credit_workbench admin-only); (4) flow_coverage 6 of 9 stages = 66.7% (gaps: Pipeline+Approvals+Administration missing credit_workflow wiring; Approvals/Swim Lane has NO DEDICATED PAGE); (5) ifrs9_consolidation = keep_separate (2643 LOC across 32_ifrs9 + 88_ifrs_engines + 90_remaining_ifrs serve distinct concerns); (6) specialized_products = promote_to_tabs (retailer_finance + bid_bond avg 172 LOC each = sub-stub, could be tabs under 22_credit_analysis). 9 FLOW_STAGES: pipeline -> analysis -> approvals -> administration -> monitoring -> dru -> collateral -> specialized -> cockpit. 5 CROSS_ORGAN_BRIDGES documented: legal (collateral enforcement), compliance (AML+CBK PG/04), finance (provisions feed I/S + capital), risk (RWA + IRB), hr (staff loans + 1/3 rule MISSING strand 4). Credit health 65.8% baseline. Severity: 1 critical + 2 high + 1 medium. Rescue priorities: (1) wire credit_workflow into pages incl swim lane, (2) build dedicated Approvals/Swim Lane page, (3) build out 5 stubs. AUDIT-ONLY batch no engines built no pages created - v10.447-v10.450 execute rescue. Body health 91.1% preserved; 360 harmony 100%; BSC rescue 100%; engine 0/0/0/0.
