# A2Z MIS 360 — Master prompt (v4.65)
...
**Current version: v10.422 (May 2026) - Retired test audit engine.** Per Joshua's locked backlog (11 stale tests across 3 files; live audit shows 12 across 4 files). The codebase uses a _retired_v10YYY_test_v10XXX_ prefix convention to soft-retire obsolete tests - pytest skips them (not starting with test_), but they remain in source for in-context historical reference. v10.422 makes this queryable and archivable. NEW utils/test_cleanup_engine.py (~280 LOC) API-first (ZERO streamlit). audit_retired_tests + archive_retired_tests with 3 JSON-serializable dataclasses (RetiredTestInfo, TestCleanupAudit, ArchiveResult). RETIRED_PATTERN regex captures retired_by + original_version. Per-file + per-version aggregates. NEW data/_retired_tests_archive.json (13.7 KB; 12 retired tests with full bodies preserved). NEW scripts/audit_retired_tests.py runner with --archive flag. NEW 2 FastAPI endpoints GET/POST /api/v1/test-cleanup/audit + archive. SAFETY: archive is purely additive - does NOT delete retired functions from source. Engine preserved 0/0/0/0.
