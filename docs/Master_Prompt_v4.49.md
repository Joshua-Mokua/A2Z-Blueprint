# A2Z MIS 360 — Master prompt (v4.49)
...
**Current version: v10.406 (May 2026) — Real-Time Progress Rollup (E1) wired into cascade UI.** First of 7 QA-Standards enhancements. Engine (utils/manager_rollup.py, 544 LOC) was complete but only wired into admin page. Now surfaces in cascade page via new "📈 Team progress" tab between My targets and Cascade tree. Renders: top-line metrics (direct/total/scored/team avg BSC), KPI rollup table (team actual + team target + achievement % + BSC score + coverage), and up-to-9 direct reports drill-down cards. Period selector defaults to 2026-Q2 where actuals exist. Underlying bug fixed: _direct_report_codes used virtual_bank.direct_reports which returned 0 for real C-suite chiefs (no hr.json record); added canonical fallback to cascade_regenerator.build_reporting_tree (same source-of-truth as target_cascade). CRBO now correctly returns 6 direct + 808 total subordinates (was 0/0). Engine preserved 0/0/0/0.
