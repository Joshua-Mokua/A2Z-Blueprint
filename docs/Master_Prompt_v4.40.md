# A2Z MIS 360 — Master prompt (v4.40)
...
**Current version: v10.397 (May 2026) — Target cascade REGENERATED from canonical sources.** Phase C2 primary action batch — single operation resolves TC18 (cross-branch cascade), TC21 (BOM Kenyatta gets cascade from BM River Road), TC22 (multi-sender ambiguity), TC25 (63% over-allocation from MD), TC32 (representative-sender pattern where only 50 of 1449 staff appeared as senders). Built `utils/cascade_regenerator.py` leaf module (~500 LOC, AST-verified, 6 self-tests) that reads canonical `role_manager_whitelist` + `role_tiers` + `bank_targets` + `fixed_kpis` and rebuilds cascade from scratch: builds reporting tree with same-branch constraint for branch-level subordinates (tier >= 4), fallback for HQ specialists (report to MD), then BFS top-down with equal-split allocation. **Skips Fixed KPIs per Joshua A1** (MD reserve — 16 fixed for 2026: CX Score, Audit Score, NPL_RATIO, CASA Ratio, PAR, Account/Channel Dormancy, K010-K134, COMPLIANCE_SCORE, Staff Productivity). NPL Ratio (human name, per-unit per Joshua A2) IS cascaded. Cascade size: **1,051 → 23,069 entries** (per-staff, not rep-sender). Engine audit after regeneration: **0 cycles** ✓, **0 cross-branch violations** ✓ (was 25,893), **0 multi-sender ambiguities** ✓ (was 10,269). 53 critical rep-sender findings remain but ALL are HQ specialist roles (CFO, CRO, CIO, COO, CHRO, RM Diaspora, SRM Corporate, etc.) whose canonical reports aren't defined — **TC32 branch-level representative-sender pattern is COMPLETELY RESOLVED**. HQ canonical extension surfaced as new finding TC42 for future batch (admin UI to extend role_manager_whitelist for HQ in v10.398). Backup at `data/_v10397_backups/target_cascade.json.before`. Pattern: ship the regenerator as a leaf, run once to commit, gate verifies invariants forever after. 8 diagnostic tests (v10.391 TC25/TC26, v10.393 TC32/cross-branch/multi-sender/full-audit, v10.392 CRBO-many, v10.395 cross-branch-bounded) updated/retired since bugs resolved.
