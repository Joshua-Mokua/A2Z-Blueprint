# A2Z MIS 360 — Master prompt (v4.45)
...
**Current version: v10.402 (May 2026) — KPI naming consolidation (TC39 + deep review per Joshua).** Deep review of all KPI names across `kpi_library`, `bank_targets`, `fixed_kpis`, `target_cascade` surfaced **4 alias pairs** (not just NPL): `NPL Ratio`/`NPL_RATIO`, `New Accounts`/`NEW_ACCOUNTS`, `Net Interest Margin`/`NET_INTEREST_MARGIN`, `Compliance Score`/`COMPLIANCE_SCORE`. **Critical bug**: NPL_RATIO and COMPLIANCE_SCORE were in fixed_kpis (MD reserve) but human-form "NPL Ratio" and "Compliance Score" were being cascaded — MD's intent to fix was not being honored at display layer. Resolution: canonical = human-readable form (matches BSC + kpi_library). Migrations: (1) Extended KPI_ALIASES with 4 new mappings + redirected COMPLIANCE alias to human form; (2) Made alias map win in resolve_kpi_id (canonical decision overrides duplicate library entry); (3) Archived 8 active uppercase entries from bank_targets (preserved in `_v10402_archived_uppercase_aliases`); (4) Replaced 12 uppercase KPI names in fixed_kpis across all 6 periods; (5) Per Joshua A2, **NPL Ratio removed entirely from fixed_kpis** (cascadable, varies per branch — A2 directive), while Compliance Score retained as fixed (bank-wide); (6) Regenerated target_cascade → 25,488 → 24,192 entries (1,296 alias-cascade dups removed = 3 KPIs × 432; NPL Ratio now correctly cascaded back per A2); (7) Engine state preserved 0/0/0/0; (8) Updated v10.380 + v10.394 test assertions to reflect new canonical choices.
