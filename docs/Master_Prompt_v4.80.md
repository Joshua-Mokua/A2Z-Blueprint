# A2Z MIS 360 — Master prompt (v4.80)
...
**Current version: v10.437 (May 2026) - HR Rescue Arc Batch 1: CIMS+SLA relocation.** Per v10.436 audit finding (HR HEALTH 53%): 2 misplaced pages (CIMS, SLA Tracker). v10.437 executes the first rescue batch. Relocations applied: 13_sla.py (people_hr -> operations, module_path operations.sla_tracker) + 18_cims.py (people_hr -> operations, module_path operations.cims; joins its 5 sub-pages 105-109 that were already in operations). Manifest department_primary + module_path updated. Pages require_access strings updated. Backups in data/_v10437_backups/ (manifest + 2 pages). Stamp _v10437_relocations on manifest. Result: HR module placement 50% -> 100% (0 misplaced); HR Health 53.0% -> 57.5%. Forward-compat patch for v10.436 test_v10436_misplaced_pages_identified (now checks MISPLACED_HR_PAGES constant, not actual misplacement). AUDIT-AND-DATA batch (no engines built). 360 harmony 100% preserved; BSC rescue 100%; engine 0/0/0/0.
