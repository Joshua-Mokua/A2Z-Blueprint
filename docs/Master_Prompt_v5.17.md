# A2Z MIS 360 — Master prompt (v5.17)
...
**Current version: v10.473 (May 2026) - Phase O1 Stabilization & Wiring Completion (first batch of Joshua's Master Prompt: Enterprise Banking Digital Twin / Olympic-Grade Readiness Framework).** Per doctrine 'No new expansion proceeds until enterprise wiring is complete, enterprise actuals propagate correctly, KPI ecosystem is clean, and public interfaces are certified.' Closed 5 known foundational defects: B-100 (period format normalisation - added _normalise_period helper to virtual_bank_kpi_unifier accepting '2026Q1'->'2026-Q1' transformation across all unify_* functions); B-101 (VB to live BSC actuals wiring - created utils/vb_actuals_bridge.py NEW orchestrating CBS aggregates -> virtual_bank_kpi_unifier -> canonical_bsc_writer v10.379 -> live_actuals.refresh_yoy -> audit_log with dry_run default safety); B-102 (KPI library rationalisation - 15 KPIs activated by linking to obvious roles Recovery/Legal/HR/Credit/RM, 91 KPIs marked deprecated=True deprecated_v=v10.473 deprecation_note preserving historical data integrity, 0 truly-unused active KPIs remaining); B-103 (virtual_bank.py facade self-test coverage - appended 15 self-tests, surfaced + fixed 2 latent v10.314 bugs: active_kpi_definitions not filtering deprecated, direct_reports missing users.json reports_to coverage so MD returned 0 direct reports); B-104 (Legal SLA revalidation - original v10.314 'Legal failed' assertion RESOLVED, but surfaced 8 hr.json records with INTEGER staff_codes vs string everywhere else AND 6 phantom 901xxx records not in users registry; fixed by normalising all hr.json staff_codes+manager_codes to strings and deactivating 6 phantoms preserving history; 22/22 departments now clean). G359 added (391 gates). All prior cert preserved (G354 data, G355 structure, G356 13-organ cert, G357 enterprise discharge, G358 360 compliance). Phase O1 foundation locked - body is ready for Phase O2 (Truth, Telemetry & Observability layer).
