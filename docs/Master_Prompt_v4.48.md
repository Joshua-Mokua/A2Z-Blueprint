# A2Z MIS 360 — Master prompt (v4.48)
...
**Current version: v10.405 (May 2026) — Target guidance wired + enhanced weight visibility.** Per Joshua's 3 user-flagged fixes: (1) **suggest_target wired** — utils/core.suggest_target() (~200-LOC engine returning historical + AI-recommended target range) was only imported in pages/12_cascade.py but NEVER invoked. Now rendered as a 🎯 Target guidance ribbon per KPI showing prior year actual, suggested min/target/stretch range, confidence badge, NEW HIRE flag, italic rationale. Skips Fixed KPIs. Tolerates exceptions silently (informational). get_bank_growth_trajectory also wired. (2) **Weight check row always visible** — was gated by `if _bad_wts:`, now always rendered (green ✅ when sums = 100%, red ⚠️ when not) per Joshua's directive "let the fixed kpi be greyed out for full visibility and to ensure weights can be summed up to 100%". (3) **Allocation sum/remaining indicator** — verified intact at lines 1872-1894 (live X over/remaining as manager types). Fixed KPI rendering verified — already shown as read-only 🔒 cells with auto-cascade label + yellow background (no changes needed). Engine preserved 0/0/0/0.
