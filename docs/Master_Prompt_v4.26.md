# A2Z MIS 360 — Master prompt (v4.26)
...
**Current version: v10.382 (May 2026) — Three Deep Reviews (review-before-action discipline, Phase B continues).** Per Joshua's layered directive at v10.381 wrap-up: before continuing with rm_profitability refactor, conduct deep reviews of Customer 360, KPI implementation plan, and pillar weights admin module. v10.382 = REVIEWS ONLY (no code changes). The rm_profitability commitment defers to v10.383. Three deliverables: (1) `docs/CUSTOMER_360_DEEP_REVIEW_v10.382.md` (8 Parts) — surveys pages/34_customer360.py (3,314 lines, 7 tabs: Customer Lookup, Portfolio Intelligence, Churn Risk with 3 sub-tabs, NBA, Segment Analytics with 5+ sub-tabs, CLV with 3+ sub-tabs, IFRS 7 + IAS 24 Disclosures). Surfaces 7 integration gaps: no v10.378 canonical master consumption (largest unmigrated page); no canonical PBT view per customer; no segment cross-reference; no BSC integration; no staff/RM context; no campaign linkage; no pipeline linkage. Proposes phased migration starting v10.385. (2) `docs/KPI_IMPLEMENTATION_PLAN_v10.382.md` (9 Parts) — concrete spec for 9 new KPIs: Tier 1 (NIM 4.5% target, CIR 55% target, ROE, NPS +30 target, DEP_GROWTH); Tier 2 (DIGITAL_ACT, 5 LEGAL_* SLAs). Per-KPI: library entry schema, formula, data source, cascade structure, implementation steps. Two new utils modules: `financial_ratios_engine.py` (NIM/CIR/ROE/DEP_GROWTH) + `customer_focus_engine.py` (NPS/DIGITAL_ACT). 7-batch schedule v10.384→v10.390. (3) `docs/PILLAR_WEIGHTS_ADMIN_MODULE_REVIEW_v10.382.md` (8 Parts) — surfaces 3 storage locations (kpi_library.json::pillar_weights = CANONICAL, kpi_library.json::pillars[].weight = SHADOW, org_config.json::pillar_weights = ORPHAN) and 2 admin UIs (Bank Identity tab writes to dead branch = silent failure per §5.4; KPI Library Pillar weights tab works). Documents 6 defects + consolidation plan over v10.384-v10.388. Each review body-system-framed with Joshua decisions queue (C1+, K1+, W1+). 343 total integration tests pass (333 prior + 10 v10.382). After Joshua approvals: v10.383 = rm_profitability canonical refactor; v10.384+ implements approved decisions.
