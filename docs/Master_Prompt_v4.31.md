# A2Z MIS 360 — Master prompt (v4.31)
...
**Current version: v10.388 (May 2026) — Bank Identity Pillar Weights Form REMOVED (v10.384 deprecation promise kept).** Per v10.385 body diagnosis Tier-1 fix sequence (Finding P3). The deprecation period (v10.384 → v10.388, 4 batches) ended; dead UI form amputated from `pages/7_admin.py` Bank Identity tab. Removed: 4 widgets (Financial/Customer/Operations/People number_inputs), sum-validation display, dead-branch write `_org["pillar_weights"] = {...}`, and form-submit sum-validation gate `if _wt_total != 100`. Replaced with brief `st.info` redirect: "Pillar weights moved — managed at Admin → KPI Library → Pillar weights tab." Bank Identity tab still saves bank metadata (name/code/country/currency/regulator/etc) cleanly. KPI Library Pillar Weights working tab (v10.386 contract) unchanged. `org_config.json::pillar_weights` data preserved on disk for v10.390 removal (two-stage: stop writing v10.388, delete data v10.390). `pillars[].weight` shadow data preserved for v10.389 removal. AST-verified: admin parses cleanly. Ships `docs/BANK_IDENTITY_PILLAR_WEIGHTS_REMOVED_v10.388.md` (8 Parts). G270 + v10.384 admin test updated to accept either pre-v10.388 deprecation OR post-v10.388 redirect (forward-compatible). v10.386 admin test similarly updated. Phantom limb gone; body has one pillar-weights UI pathway. Phase C continues: v10.389 = remove `pillars[].weight` shadow data; v10.390 = remove `org_config` orphan + Tier 1 Class B KPIs start.
