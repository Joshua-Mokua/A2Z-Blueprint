# A2Z MIS 360 — Master prompt (v5.18)
...
**Current version: v10.474 (May 2026) - Phase O8 Environment Isolation Governance (pulled forward early per roadmap; second batch of Olympic-Grade arc).** Per Joshua Master Prompt Phase O8: 'No simulation artifacts may contaminate production DNA.' Doctrine pulled forward because every subsequent batch (O2 telemetry, O3 channel sims, O4 macro events, O5 chaos, O6 AI evolution, O7 training) generates simulation artefacts; the isolation boundary MUST be locked before those run. Built: (1) utils/environment.py NEW - Environment enum (DEV/SIM/UAT/STAGING/PROD) + get_environment + set_environment + ALLOWED_PROMOTIONS one-way ladder + environment_paths per-mode write roots + helper predicates (is_production/is_simulation/etc); (2) data/environment.json NEW - canonical mode state file; (3) utils/data_isolation_guard.py NEW - guarded_write_path (auto-redirects to data/sim or data/uat based on mode) + is_write_allowed + PROTECTED_PROD_FILES (users.json/hr.json/kpi_library.json/cascade/bsc_scores etc) + assert_not_production for sim-only ops; (4) utils/data_migration.py NEW - promote_dataset honouring one-way ladder + audit-logged + dry-run default + list_environment_inventory; (5) data/sim + data/uat + data/staging sandbox dirs each with README describing disposability; (6) docs/environment_isolation_policy.md governance doc (5 envs + promotion ladder + protected files + onboarding checklist + audit evidence); (7) vb_actuals_bridge wires the guard - refuses live writes from non-PROD modes unless dry_run. G360 added (392 gates). Mechanically refuses PROD->DEV demotion (terminal). All prior cert preserved (G354 data, G355 structure, G356 13-organ, G357 discharge, G358 360 compliance, G359 O1 stabilization).
