# A2Z MIS 360 — Master prompt (v4.32)
...
**Current version: v10.389 (May 2026) — Pillar Shadow Weights Removed from kpi_library.json::pillars[].** Per v10.385 body diagnosis Tier-1 fix sequence (Finding P3 prioritization organ consolidation). Each entry in pillars[] array no longer has shadow `weight` field; structural fields (id, name, color) preserved (needed for pillar grouping + BSC styling). Pre-v10.389 SHADOW values were 40/25/25/10 (Kaplan-Norton balanced); canonical `pillar_weights` dict has 68/14/6/12 (financial-heavy). Two parallel values for same concept inside same file — drift surfaced and silenced. Canonical dict unchanged (still 68/14/6/12). Body sheds dead weight; `health_check.shadow_pillars_field` flips True → False as diagnostic confirmation. Backup preserved at `data/_v10389_backups/kpi_library.json.before` per operational rollback pattern. Smallest batch of Phase C: 4 field deletions from data file, no code changes. **Pre-existing bug DISCOVERED but NOT FIXED** (logged as Finding N7 for future batch): `utils/core.py::get_active_kpis()` calls `.items()` on pillars which is a list — raises AttributeError. Separate concern per Rule N2 single concern; documented in v10.389 design doc Part 5. Ships `docs/PILLAR_SHADOW_WEIGHTS_REMOVED_v10.389.md` (9 Parts). Phase C continues: v10.390 = remove `org_config.json::pillar_weights` orphan field (final prioritization rescue step) + start Tier 1 Class B KPIs implementation (NIM/CIR/ROE/NPS/DEP_GROWTH).
