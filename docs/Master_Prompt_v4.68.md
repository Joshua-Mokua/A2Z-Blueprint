# A2Z MIS 360 — Master prompt (v4.68)
...
**Current version: v10.425 (May 2026) - Pillar canonical merge (BSC Rescue batch 1).** Per v10.424 audit: 221 BSC actuals rows used non-canonical "Operational" pillar (should be "Operational Excellence"). Root cause: simulate_v2.py had 19 KPI definitions hardcoded with the alias. v10.425 ships the fix at BOTH levels: (1) NEW utils/bsc_pillar_normalize_engine.py (~250 LOC) API-first (ZERO streamlit, dry_run=True default) with ALIAS_MAP constant; (2) SOURCE FIX: simulate_v2.py 19 occurrences of "pillar":"Operational" changed to "pillar":"Operational Excellence"; (3) DATA MIGRATION: 221 rows in data/actuals_2025_Dec_25.xlsx flipped from "Operational" to "Operational Excellence" via the engine, with backup at data/_v10425_backups/actuals_2025_Dec_25.xlsx.before. NEW scripts/normalize_pillars.py runner with --confirm gate. NEW 2 FastAPI endpoints /api/v1/bsc-pillar/audit + migrate. BSC health 28.6% -> 42.9% (+14.3 points). 19th React-ready engine. Engine preserved 0/0/0/0.
