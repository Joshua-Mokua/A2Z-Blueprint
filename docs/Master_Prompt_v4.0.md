# A2Z MIS 360 — Master prompt (v4.0)
...
**Current version: v10.355 (May 2026) — Live Actuals Engine + YoY Growth (CBS-wired actuals sub-batch 2 of 3).** Building on v10.354's CBS baseline foundation, v10.355 wires the frozen YoY reference into the actuals pipeline via `utils/live_actuals.py` (340 lines). For every CBS-driven actuals refresh, the engine writes a sidecar `data/actuals_yoy.json` keyed by `<staff_code>__<kpi_name>` containing `(current_value, baseline_value, growth_pct, direction)`. 20 default KPI-to-baseline mappings cover deposits, loans, NPL, customer counts, and loan-to-deposit at the bank-aggregate and segment level — first-match semantics with specific patterns ordered before generic ones, override-able via `data/kpi_baseline_mapping.json`. Pattern Q `validate_before_save` enforced via `data/_schemas/actuals_yoy.schema.json`. Minimal BSC display added in `pages/1_perform.py`: collapsed `st.expander` after the header cards showing top growth KPIs for the selected staff member. **v10.356 mid-review correction**: original v10.355 wired `refresh_yoy()` INSIDE `actuals_engine.compute_actuals_from_cbs`, creating an `actuals_engine → live_actuals → cbs_baseline → actuals_engine` cycle that G128 flagged. v10.356 inverted: callers (`pages/7_admin.py` admin refresh; `app.py` startup) now call `refresh_yoy` AFTER `compute_actuals_from_cbs` returns. The lower data-extraction layer no longer depends on the higher orchestration layer. G128 passes 0/0. **CBS-wired actuals arc trajectory**: v10.354 ✅ baseline foundation · v10.355 ✅ live actuals + YoY in BSC · v10.356 ⏳ PBT from CBS transactions (closes arc).
...
**Audit gates: 241** (G1 through G241). Distribution: G1-G14 foundational; G15-G117 standards-coverage; G118+ QA framework; G128 structural audit baseline (StructureAuditEngine with 7 rule families); G143 KPI source coverage; G161 manifest-canonical lock; G162 tenant-hardcoding ratchet (kaizen baseline 4022 — 49 consecutive zero-drift batches); G230 protected-files schema validation; G231 page module-load smoke; G232-G236 hub-consolidation thresholds; G237 redirect signaling; G238 static AST function checks; G239 dynamic render-function smoke; G240 CBS baseline snapshot; G241 live actuals YoY sidecar.
...
**Open / partially-closed gaps at v10.355:**

- **CBS-wired actuals arc — item 4 (v10.356 pending)** — PBT computation directly from CBS transactions. Currently the actuals xlsx carries PBT as an absolute value from upstream; goal is to derive it from CBS as NII + non-interest income - OpEx - impairment. Closes the 1+3+4 arc.
- **Football Team Test end-to-end verification** — pieces exist; the through-line (teller action → MD ROE tile) hasn't been live-tested under load. This is the v10.356+ live bring-up objective.
...
- **MD BSC bank-targets binding (roadmap item 2)** — once bank targets are set in Target Cascade → Bank Targets, the MD BSC should display them. Small fix.
...
- **Audit performance** — full audit takes >5 min due to pre-existing slow gates (G117 ~37s, G204/G217 ~15s each, G231 ~13s). Candidate for parallelization / incremental caching.
- **Strategic Initiative engine / Partnerships P&L / B-027 tail** — earlier roadmap items partially built; full integration deferred until the CBS-wired actuals arc closes.
- **React executive frontend** — Standard #9 calls for React on executive dashboards. The Integration Layer API (v10.115 four endpoints + subsequent additions) provides the consumption contract. Build is the next major arc after virtual-bank live bring-up.
- **Documented data-shape divergences (deferred)** — `strategic_initiatives.rag_status` Title-vs-UPPER (1 divergent consumer), `kpi_library.kpis[].direction` short-vs-long (7 vs 2 consumers). Both locked in schemas with all values allowed; consolidation deferred to a focused naming-cleanup batch.
