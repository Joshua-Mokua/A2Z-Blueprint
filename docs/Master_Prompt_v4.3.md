# A2Z MIS 360 — Master prompt (v4.3)
...
**Current version: v10.359 (May 2026) — Link 1 CBS Persistence Bridge.** Closes Link 1 of the Football Team Test chain (teller→CBS). Adds `utils/virtual_bank_cbs_writer.py` (~470 lines) that takes a populated VirtualBankCore and persists it to `cbs_data/` in shapes `actuals_engine.aggregate_cbs_by_rm` + `aggregate_cbs_by_branch` + `compute_bank_aggregates` already consume. Produces six files: `accounts.csv` (per-account rows with 16 columns: account_no, cif, branch_code, branch_name, relationship_manager_code, category, account_type_name, current_balance, date_opened, dormancy_status, interest_income_ytd, fee_income_ytd, loan_amount, loan_outstanding, npl_status, npl_days), plus `deposits_aggregate.json` / `loans_aggregate.json` / `npl_aggregate.json` / `customer_aggregate.json` / `dormant_aggregate.json`. **Atomic** (every write uses `tmp.replace(final)` so readers never see partial files), **idempotent** (same bank state → same totals on repeated persist — verified by self-test), **coherent** (aggregate JSONs computed from the same account records written to CSV — no drift between formats). Stand-alone loans (no matching account) synthesized as LN_<loan_id> rows so they show up in loan portfolio aggregation. **Football Team Test chain: 6/7 WIRED, 1/7 PARTIAL** — Link 1 closed; only Link 7 (regional→MD tile bank-targets binding) remains for v10.360. End-to-end verified: seed bank → persist → actuals_engine.aggregate_cbs_by_rm reads back 29 RMs across 21 branches with real aggregations (KES 69M deposits, KES 5.6M loans on a default small seed). G245 audit gate locks the bridge integrity (self-test passes + aggregate-CSV coherence). Readiness audit updated to mark Link 1 WIRED.
...
**Open / partially-closed gaps at v10.359:**

- **Football Team Test end-to-end verification** — 6/7 chain links WIRED, 1/7 PARTIAL. Only Link 7 remains: regional→MD tile bank-targets binding per roadmap item 2 — v10.360. End-to-end integration test = v10.361 candidate; **after v10.361 Charter §2 passes**.
- **CBS-wired actuals arc — item 4 (deferred)** — PBT computation from CBS transactions. Re-prioritized after v10.361 closes the Football Team Test chain.
- **BSC coverage 2.78% → 100%** — only 40 of 1,439 active staff have BSC actuals. Now technically unblocked (the bridge can populate CBS aggregates for any cut of the bank); closing this is a sustained data-engineering effort, not a single batch.
- **System stocks live wiring** — 6/6 have snapshot accessors but currently return demo defaults. v10.359 makes this trivially easy: cbs_baseline + bridge mean live values are computable.
- **MD BSC bank-targets binding (roadmap item 2)** — Link 7 — v10.360.
...
- **Bridge aging buckets are zeros for now** — `npl_aggregate.json::by_aging_kes` is zeroed (DAYS_91_180, DAYS_181_365, DAYS_OVER_365). Computing aging requires DPD-band granularity from the loan records; deferred.
...
- **Documented data-shape divergences (deferred)** — `strategic_initiatives.rag_status`, `kpi_library.kpis[].direction`. Locked in schemas with all values allowed.
