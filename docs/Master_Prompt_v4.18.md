# A2Z MIS 360 — Master prompt (v4.18)
...
**Current version: v10.374 (May 2026) — Role Taxonomy Alignment (Phase A first batch).** First execution batch from the v10.373 system review. Establishes the **profitability axis** for every role — orthogonal to the existing seniority axis (role_tiers 0-6). Per Joshua's directive: "ensure the tree also aligns from the pipeline management... eliminate ambiguities, let's have a system operating as one... all the organs all functioning perfectly and in harmony to make the one body". Five profitability tiers: `portfolio_owner` (tagged to customers — Branch sales like RM PB/BB/DSO/BRM/SRO/RO, plus HO sales like Sector Specialists / HO Corporate / SME RMs whose customers span multiple branches but fit one SBU); `proposition_owner` (overlap propositions like Women Banking / Diaspora — NOT tagged; per Rule 6 propositions overlap by design); `structural_owner` (owns PBT at structural level via rollup — Branch Manager / Area Manager / Heads of Banking / Directors / MD / Chiefs of business lines — NOT tagged); `service` (Teller / CSO / BOS — branch operational, occasionally tagged when introducing accounts but not primary sales); `support` (HO functions — Risk / Compliance / IT / HR / Finance / Audit — owns cost center, not direct PBT). Extends `data/org_hierarchy_config.json` with `profitability_axis` subtree (41 explicit role classifications + tier_keyword_fallback covering "relationship manager", "women banking", "branch manager", "teller", "compliance officer" etc.). Ships `utils/role_taxonomy.py` (12 self-tests) with classify_role / get_profitability_tier / get_branch_scope / get_sbu / can_be_tagged / validate_role_coverage. **100% role coverage achieved**: 126 distinct roles in users.json + hr.json → 41 explicit + 85 keyword + **0 unclassified**. **Taggability invariant locked (G260)**: only portfolio_owner + service tiers may appear in accounts.csv::relationship_manager_code; proposition / structural / support roles MUST NOT tag. SBU values align with segment_sbu_mapping.json (Retail / Commercial / Corporate Banking, plus Treasury / Digital_Agency / Support / Executive as axis-level operational SBUs). Body-system analogy: this is the profitability circulatory system — orthogonal to the skeleton (seniority hierarchy). Customer master merge (Joshua approved "merge into 1") deferred to v10.377 (after Phase A UI surfacing v10.375-v10.376). Phase C role granularity agreed as grouped (per Joshua), to avoid drift while maintaining velocity.
