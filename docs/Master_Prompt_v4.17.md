# A2Z MIS 360 — Master prompt (v4.17)
...
**Current version: v10.373 (May 2026) — System State Review.** Strategic review batch (no engine changes). Per Joshua's directive: "a deep review of what the entire system has helps to ensure we do not repeat what is there...continue with the complete virtual bank set up to meet the objective set for today which is ultimately to run real virtual simulations of every staff role and module we have...what we have done with profitability unification happens across." Ships `docs/SYSTEM_STATE_REVIEW_v10.373.md` — a deep survey mapping the 123 pages + 439 utils + 208 data files surface. Identifies: (a) the **simulation gap** — only `teller_actions.py` provides a live action interface (used by Charter §2 G249); the other ~25 staff roles need analogous interfaces to enable real virtual simulations per the stated objective; (b) **remaining parallel profitability engines** — `customer_profitability.py`, `rm_profitability.py`, `product_profitability.py` still parallel to the v10.370 atomic engine; (c) **other modules where the unification pattern applies** — risk (credit/market/operational), customer 360, treasury/ALM, compliance/CIMS, HR; (d) **proposed phasing** — Phase A UI surface the work just shipped (v10.374-v10.375), Phase B close remaining parallel profitability engines (v10.376-v10.379), Phase C live action interfaces for every staff role (v10.380-v10.400, the big simulation arc), Phase D module-by-module unification (risk/customer/treasury/compliance/HR, v10.40X-v10.44X), Phase E executive UI (v10.45X+, React frontend per Standard #9). G259 locks the document's presence + key sections so the strategic anchor doesn't get lost. No new engine code; no parallel engines closed; no roles simulated yet — this batch is pure strategic mapping. The recommended next concrete batch is v10.374 (role-aware filter for staff PBT — first visible UI surfacing of v10.370 work).
