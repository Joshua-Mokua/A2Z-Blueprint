# A2Z MIS 360 — Master prompt (v4.43)
...
**Current version: v10.400 (May 2026) — Admin UI for canonical hierarchy editing.** Per Joshua's "reporting lines can be set from the admin" production-time requirement. Built **two new components**: (1) `utils/canonical_admin.py` — leaf module backend (~300 LOC, AST-verified, 6 self-tests) with read helpers (list_role_managers, list_role_tiers, get_branch_tier_threshold), mutators with auto-backup + change logging (set_role_managers, set_role_tier, set_branch_tier_threshold, remove_role), validator (cycle detection + tier inversion + missing referenced managers), regenerator caller (regenerate_cascade_from_canonical), and provenance ledger (log_change/read_change_log at data/canonical_change_log.json, capped at 1000 entries). (2) `pages/_admin_canonical.py` — Streamlit UI with 6 views: Overview (metrics + tier distribution + validation issues), Reporting Lines (search + edit/add/remove role mappings), Role Tiers (search + edit tier 0-9), Threshold (set branch tier threshold), Regenerate (cascade rebuild button), Change Log (provenance audit trail). **Integrated into `pages/7_admin.py`** as 8th sub-tab "🎯 Canonical Hierarchy" in "People & Org" section. Auto-backup of canonical to `data/_canonical_backups/` on every save with timestamp. Every change logged with who/when/what/old/new/reason. **Architectural endpoint of Phase C2 rescue arc**: canonical-driven design now complete — engine reads canonical (v10.395) → admin edits canonical from UI (v10.400) → regenerator rebuilds cascade → engine reads new cascade. Bank deploys with Joshua-aligned default canonical; production admin customises in-app without developer involvement. Engine state preserved: 0/0/0/0 all metrics zero.
