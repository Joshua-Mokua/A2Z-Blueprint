# A2Z MIS 360 — Master prompt (v4.39)
...
**Current version: v10.396 (May 2026) — Canonical hierarchy aligned with Joshua's clarification.** Per Joshua's directive 2026-05-13: *"at branches we can either have a senior branch manager or a branch manager, this is depended on the size of the branch. The branch operations manager, in BRM (Branch Relationship manager — in big branches), SRO senior Relationship Officer, RO Personal/Business Banker and DSR reports to the branch manager, the branch operations supervisor, teller, Customer Service report to the branch operations manager"*. **Three canonical changes** to `data/org_hierarchy_config.json` (no code changes — pure config alignment): (1) Senior Branch Manager tier 3 → 4 (branch top for big branches per Joshua, NOT regional); (2) SBM added as alt manager for branch subordinates (BOM, BRM, BSRO, RO PB, RO BB — big-branch tops have same reports as BM); (3) DSR (and DSR Assets & Liabilities) reporting line changed from BOS/BOM to BM/SBM per Joshua's "DSR reports to the branch manager". Engine auto-derives new WITHIN_BRANCH_ROLE_PAIRS via v10.395 dynamic loading: **17 → 22 pairs** (5 SBM→X added since SBM is now branch-top; 4 BOS/BOM→DSR removed since DSR moved). Cross-branch violation count climbs 19,026 → **25,893** because SBM cross-branch cascades that were tier-3 (regional, ignored) are now tier-4 (branch-level, properly flagged). 8 SBMs across 8 distinct big branches in data (Kenyatta Avenue, FB Towers, Thika, Mombasa, etc.) confirm 1-per-branch big-branch pattern matches Joshua's description. Provenance note `_v10396_joshua_clarification` documents the changes. Backup at `data/_v10396_backups/org_hierarchy_config.json.before`. **Demonstrates config-driven architecture pays off**: Joshua's clarification implemented as data update only; engine reflects change without recompilation. v10.393 test_canonical_within_branch_pairs updated (SBM now in within-branch pairs). 10 new v10.396 integration tests verify alignment.
