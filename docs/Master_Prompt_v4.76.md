# A2Z MIS 360 — Master prompt (v4.76)
...
**Current version: v10.433 (May 2026) - Cascade-BSC harmonization, full body alive at 100%.** Per Joshua directive after v10.432 found 60% harmony: KPIs only stem from library, obsolete entries deleted, BSCs supplemented, admin configuration superb. NEW utils/cascade_bsc_harmonize_engine.py (~800 LOC) API-first ZERO streamlit. 5-stage harmonization: Stage A docs Staff Productivity 1-5 BSC grading scale vs 0-100 bank index (no-op data preservation); Stage B two-pass cascade pruning (library-orphan + role_kpis mismatch) - recomputes total_target AND allocated_sum; Stage C supplements BSC with missing cascade allocations using canonical name resolution; Stage D reuses v10.428 weight renormalization; Stage E aligns BSC Annual Target to cascade allocations. 6 JSON-serializable dataclasses (StageA-E + HarmonizeAllResult). EXTENDED cascade_bsc_360_engine with BSC_SCORE_KPIS (7 grading-scale KPIs) and canonical resolver in audit_cascade_to_bsc_targets. EXTENDED bsc_admin_panel with render_harmonize_panel admin-gated dry-run->confirm. EDITED pages/7_admin.py wires the panel. NEW 2 FastAPI endpoints /api/v1/harmonize/all + stage. Live data fixes applied: cascade 24024->5050 entries (18974 pruned + 69572 allocations dropped); 2264 BSC rows added across 1160 staff (avg 1.95 KPIs/staff); 26901 weight rows renormalized; 8412 BSC target values aligned to cascade. **360 harmony 60% -> 100% (5/5 stages PASS)**. BSC rescue health 100% preserved. 26th React-ready engine. Engine 0/0/0/0.
