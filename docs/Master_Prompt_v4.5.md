# A2Z MIS 360 — Master prompt (v4.5)
...
**Current version: v10.361 (May 2026) — Configurability Hardening (Rule N1 enforcement).** Joshua flagged: "branches are not to be hardcoded, even the bank... since this is a system we are building that can be adopted by any bank... we shall be integrating to either flexcube core banking which hosts this branch data." v10.361 implements the cleanup: **deletes `_BRANCH_REGION_FALLBACK` from `utils/core.py`** and **`_FALLBACK_BRANCHES` from `utils/virtual_bank_seed.py`** (the v10.360 carryovers that violated Rule N1 by carrying tenant-specific branch data even as fallback). Missing/malformed `org_config.json` now returns an empty dict — the configuration error surfaces in the admin UI rather than being masked by a stale 21-entry fallback. **Wires the FLEXCUBE integration seam**: adds `utils.flexcube_adapter.fetch_branches_from_flexcube()` and `fetch_staff_from_flexcube()` as the seamless production hooks (stubbed when `flexcube_config.json::mode != "live"`, with documented contract for the live wire-up). `utils.virtual_bank_seed.get_ecobank_branches()` now follows priority order: FLEXCUBE → org_config.json → empty dict (no fallback). When FLEXCUBE comes online, no caller code changes. **Confirms admin CRUD coverage**: pages/_admin_org.py `render_branch_manager` provides Add + Edit + soft-delete (active=False, the audit-traceable pattern); pages/7_admin.py provides staff Add + Edit + Delete (with `UserManager.can_delete_user` protection + audit log). G247 added to lock the CRUD coverage — verifies render_branch_manager, create_user_form, edit_user_form, can_delete_user are all present and that mutations write to audit_log. G246 strengthened to forbid hardcoded fallback dict literals (regex-precise assignment-form check). **Football Team Test chain still 6/7 WIRED** — v10.361 is configurability-hardening, not chain closure. Link 7 (regional→MD tile bank-targets binding) remains for v10.362.
...
**Audit gates: 247** (G1 through G247). Distribution: G1-G14 foundational; G15-G117 standards-coverage; G118+ QA framework; G128 structural audit baseline; G143 KPI source coverage; G161 manifest-canonical lock; G162 tenant-hardcoding ratchet (kaizen baseline 4022 — 55 consecutive zero-drift batches); G230 protected-files schema validation; G231 page module-load smoke; G232-G236 hub-consolidation thresholds; G237 redirect signaling; G238 static AST function checks; G239 dynamic render-function smoke; G240 CBS baseline snapshot; G241 live actuals YoY sidecar; G242 master prompt sync (anti-drift lockstep); G243 virtual bank readiness audit; G244 virtual bank seed determinism; G245 CBS persistence bridge integrity (Link 1); G246 branch single source of truth (v10.361 strengthened — no hardcoded fallback); G247 admin CRUD coverage (branch + staff Add/Edit/Delete).
...
**Open / partially-closed gaps at v10.361:**

- **Football Team Test end-to-end verification** — 6/7 chain links WIRED, 1/7 PARTIAL. Only Link 7 (regional→MD tile bank-targets binding) remains — v10.362. End-to-end integration test = v10.363 candidate; **after v10.363 Charter §2 passes**.
- **FLEXCUBE branch/staff live integration** — `fetch_branches_from_flexcube` + `fetch_staff_from_flexcube` are STUBBED at v10.361 (return None in non-live modes). The integration seam is wired but the live API call is not yet implemented. When Ecobank's Apigee gateway access is provisioned, the stub bodies replace with the real REST call (contract documented in adapter docstring). No caller-code changes required.
- **CBS-wired actuals arc — item 4 (deferred)** — PBT computation from CBS transactions.
- **BSC coverage 2.78% → 100%** — only 40 of 1,439 active staff have BSC actuals. Sustained data-engineering effort.
- **System stocks live wiring** — 6/6 have snapshot accessors but currently return demo defaults.
- **MD BSC bank-targets binding (roadmap item 2)** — Link 7 — v10.362.
- **Branch roles data generation (roadmap item 5)** — some branches lack the full role complement. With 94-branch scale, amplified.
- **Region cleanup (45/94 branches marked "Other")** — data quality task on org_config.json. Admin module can re-classify via Edit branch.
- **Audit performance** — full audit takes >5 min due to pre-existing slow gates.
- **strategy_simulator + hybrid_scheduling_simulator lack self_test**.
- **Bridge aging buckets are zeros** — `npl_aggregate.json::by_aging_kes` zeroed.
- **Hard branch delete UI gap** — admin has soft delete (active=False, audit-traceable) but not hard delete from disk. Acceptable per banking audit standards but documented as a deliberate gap.
- **Strategic Initiative engine / Partnerships P&L / B-027 tail** — partially built.
- **React executive frontend** — Standard #9 calls for React on executive dashboards.
