# A2Z MIS 360 — Master prompt (v4.24)
...
**Current version: v10.380 (May 2026) — KPI Alias Resolver + Deep Review (Phase B continues).** Per Joshua's explicit directive: "do a deep review of the target cascade and kpi library for more understanding and appreciation also on how they are configured, what can be fixed." Review-first batch. Three deliverables: (1) `docs/TARGET_CASCADE_KPI_LIBRARY_REVIEW_v10.380.md` (~15KB, 10 Parts) — deep survey of target_cascade (1,051 entries, 51 cascading staff, 21 distinct KPIs, plus 1 corrupted `deadline|300001|2026` metadata key polluting the dict) and kpi_library (185 KPIs/109 active, three ID conventions: numeric K-codes / Title Case / SCREAMING_SNAKE — with 167/185 KPIs showing id≠name drift). Surfaces 34 orphan KPI IDs across role_kpis (referenced but not defined): 19 Class A (alias drift — short SCREAMING_SNAKE versions of existing Title Case definitions) + 15 Class B (genuinely missing — DEP_GROWTH, NIM, ROE, NPS, CIR, FEES_COMM, DIGITAL_ACT, NEW_CUST, ACTIVE_ACCTS, PAR, TRANSACTIONS, 5 LEGAL_*). (2) `utils/kpi_alias_resolver.py` (leaf module — 10 self-tests) with `KPI_ALIASES` (19 mappings), `CLASS_B_ORPHANS` (15 documented), `resolve_kpi_id` / `get_kpi_definition` / `list_class_b_orphans` / `clean_cascade_dict` / `scan_role_kpis_coverage`. Read-only — does NOT modify kpi_library.json or target_cascade.json; opt-in for consumers. After v10.380: zero unknown orphans (everything resolves via alias or is documented Class B). MD's BSC submission readiness: 4/12 resolvable, 8 Class B still missing (deferred — needs Joshua product-mgmt input on KPI definitions). Documented Joshua decisions: add Class B KPIs? reconcile pillar weights 40/25/25/10 vs 68/14/6/12? retire K-code duplicates? clean up deadline|* corruption in place? Phase B continues: v10.381 = refactor customer_profitability.py to canonical (v10.378 unified master). v10.382 = refactor rm_profitability.py.
