# A2Z MIS 360 — Master prompt (v4.23)
...
**Current version: v10.379 (May 2026) — Canonical Write-Bridge (Phase B continues).** Closes the constitutional data-flow loop (§5.3 Source → Staging → Transformation → Clean → BSC Integration → Reporting) — the last arrow was missing. Canonical PBT records from the v10.377 unifier now flow into `bsc_actuals_*.json` via `bsc_engine.submit()`. The MD's BSC tab gets its PBT actual from the canonical engine, not the legacy `management_accounts` source. Three deliverables: (1) `docs/CANONICAL_WRITE_BRIDGE_v10.379.md` (~7KB, 7 Parts) documenting safety design + idem-hash collision filter + period translation + reconciliation gate + remaining SBU-head collision (Commercial/Corporate/Treasury → one Chief). (2) `utils/canonical_bsc_writer.py` (~330 LOC, 10 self-tests) with `WriteResult` / `write_canonical_pbt_to_bsc` / `preview_canonical_pbt_writes` / `_should_write` filter / `_record_to_submit_kwargs`. **SAFETY: `dry_run=True` default — AST-verified by G265.** Filter skips fallback records (SBU absorbed → MD; branches without configured BMs → MD) to prevent idem-hash collisions. Reconciliation gate refuses to write when v10.377 Σ-identity broken. Idempotent via bsc_engine's `_idempotency_hash` mechanism. (3) End-to-end sandbox demonstration: 100 unifier records → 35 eligible (65 skipped with documented reasons) → 30 created + 2 updated → 0 errors; idempotent re-runs (32 records, run twice → 32 records, 0 duplicates); MD PBT readback via `get_actual("300001", "PBT", "2026-Q4")` returns canonical -7.9B. Body-system: all five organ systems (skeleton + circulatory + nervous + recognition + endocrine) now wired to the brain via the constitutional flow path. Phase B continues: v10.380 = KPI-ID canonicalisation. v10.381/v10.382 = refactor parallel profitability engines to consume v10.378 unified master + v10.379 write-bridge.
