# A2Z MIS 360 — Master prompt (v4.47)
...
**Current version: v10.404 (May 2026) — Regenerator preserves manual allocations.** Per Joshua F4 directive ("Regenerate Cascade button — preserve"). Critical bug fix: admin clicking Regenerate Cascade no longer wipes managers' personalized allocations. Architecture: (1) `regenerate_target_cascade` accepts `preserve_manual` parameter (default True); (2) new helper `_cascade_recursive_with_skip(skip_set)` — when a manager has manual allocation, their entry AND subtree below are not regenerated (manager's responsibility); (3) `CascadeManager.set_allocation` stamps `_v10404_manual=True` + `updated_by` on every UI write; (4) `canonical_admin.regenerate_cascade_from_canonical` propagates the flag + logs manual_preserved count; (5) Admin UI Regenerate tab gets radio toggle "Preserve manual allocations (recommended)" vs "Force full rebuild". End-to-end test verified: manual 60/40 split survives regen. Engine state preserved: 0/0/0/0.
