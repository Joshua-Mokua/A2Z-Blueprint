# A2Z MIS 360 — Master prompt (v4.19)
...
**Current version: v10.375 (May 2026) — Role-aware Staff PBT page (Phase A second batch).** First UI surface of v10.370 (atomic per-staff engine) + v10.374 (profitability axis). Ships `pages/120_staff_pbt.py` (~290 LOC) with three role-aware filters (tier / sbu / branch_scope), a top reconciliation strip making the G257 identity visible to users (Bank PBT / Σ Staff PBT / Δ KES), four tabs (Staff ranking / Tier distribution / SBU contribution / Unassigned), and a footer documenting the full data lineage from seeded VirtualBankCore through canonical engines to UI. **Resolves the teller-vs-RM framing from v10.370**: data engine remains role-neutral (every tagged staff counts including tellers per Joshua's point about occasional account introduction); UI surfaces who actually OWNS the PBT via the v10.374 tier filter (default portfolio_owner only). Body-system framing: this is the first UI making the circulatory system (profitability flow) visible — orthogonal to the skeleton (seniority hierarchy) already shown in other pages. Manifest entry registered with module_path "sales_customer.staff_pbt" and department_primary "sales_customer", piggybacking the existing branch log access scope. Cached via @st.cache_data(ttl=300) so the seed → persist → compute pipeline runs once per 5 minutes per session. **Phase A is now 2/3 complete** (v10.374 taxonomy + v10.375 UI filter); v10.376 will surface SBU + Branch drill-down in the MD cockpit using the canonical engine. Then Phase B opens with v10.377 (customer master merge per Joshua's "merge into 1" approval).
