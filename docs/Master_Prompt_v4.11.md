# A2Z MIS 360 — Master prompt (v4.11)
...
**Current version: v10.367 (May 2026) — Profitability Reconciliation Diagnostic.** Ships a measurement-first batch in response to the surfacing that A2Z has **two parallel profitability engines** that don't reconcile: Engine A (`utils.pbt_computation`, v10.364 — CBS-driven, bank-level only, used by MD's BSC) and Engine B (`utils.sbu_pnl_rollup`, v10.338 — customer-driven via cost_allocation_rules.json matrix, drills to segment/sector/RM/proposition, used by Finance hub). Plus a **third** legacy formula in `aggregate_cbs_by_branch` for per-branch PBT. Pre-v10.367 these three engines were unaware of each other (90%+ PBT divergence on the small seed). v10.367 ships `utils/profitability_reconciliation.py` with `reconcile()` that runs Engine A and Engine B side-by-side, normalizes time horizons (Engine B quarterly × 4 → annual), and reports ΔPBT / ΔRevenue / ΔOpEx with named reasons (customer basis differs, revenue source differs, OpEx allocation differs, impairment treatment differs). Also ships `docs/PROFITABILITY_ARCHITECTURE_REVIEW.md` — the structural review document with the five-batch unification arc (v10.368-v10.372). **G253 is INFORMATIONAL** — it passes whenever the diagnostic runs cleanly; the delta is reported in summary as a metric but does not fail the gate. Will ratchet in v10.370+ to require status='CONVERGED' or 'TOLERANCE'. No engine changes in v10.367 — Engines A, B, C, D are untouched. v10.367 is pure measurement + roadmap. The reconciliation identity that must hold once unification ships: `∑(SBU PBT) = ∑(Branch PBT) = ∑(RM PBT) = Bank PBT`. Propositions overlap by Rule 6 and are excluded from this identity (informational only). Open design questions documented in the review awaiting Joshua's direction on Q1 (which engine is canonical), Q2 (proxy revenue sunset cadence), Q3 (per-branch allocation driver default), Q4 (proposition visibility for MD). The diagnostic confirms current state DIVERGENT; the architecture review confirms the path to CONVERGED.
...
**Audit gates: 253** (G1 through G253). Distribution: G1-G14 foundational; G15-G117 standards-coverage; G118+ QA framework; G128 structural audit baseline; G143 KPI source coverage; G161 manifest-canonical lock; G162 tenant-hardcoding ratchet (kaizen baseline 4022 — 61 consecutive zero-drift batches); G230 protected-files schema validation; G231 page module-load smoke; G232-G236 hub-consolidation thresholds; G237 redirect signaling; G238 static AST function checks; G239 dynamic render-function smoke; G240 CBS baseline snapshot; G241 live actuals YoY sidecar; G242 master prompt sync; G243 virtual bank readiness audit; G244 virtual bank seed determinism; G245 CBS persistence bridge integrity; G246 branch single source of truth; G247 admin CRUD coverage; G248 Link 7 MD tile binding; G249 Charter §2 Football Team Test end-to-end verification; G250 PBT computation from CBS; G251 FLEXCUBE live wire-up; G252 CBS accruals synthesizer; G253 profitability reconciliation diagnostic (INFORMATIONAL — passes whenever diagnostic runs; reports ΔPBT as metric; ratchets in v10.370+).
...
- ~~**Profitability architecture review (closed v10.367)** — `docs/PROFITABILITY_ARCHITECTURE_REVIEW.md` maps the four engines (pbt_computation, sbu_pnl_rollup, aggregate_cbs_by_branch, customer_profitability), identifies the reconciliation identity, proposes the five-batch unification arc. `utils/profitability_reconciliation.py` measures the current 90%+ ΔPBT gap. G253 informational. Open Q1-Q4 await direction.~~

**Profitability unification arc (open, batches defined):**

- **v10.368 — Align data sources.** Make sbu_pnl_rollup walk the same accounts.csv that pbt_computation reads. Same customer base for both engines.
- **v10.369 — Add SBU dimension to canonical engine.** `compute_pbt_from_cbs(by_sbu=True)` returns per-SBU PBTComponents. Uses `opex_data.json::by_sbu` for per-SBU OpEx. G254 locks Σ(SBU PBT) = Bank PBT.
- **v10.370 — Per-branch allocation engine.** `utils/branch_pbt_allocator.py` with admin-configurable driver (FTE / revenue / hybrid). Replaces Engine C in `aggregate_cbs_by_branch`. G255 locks Σ(Branch PBT) = Bank PBT. **G253 ratchets** to require ΔPBT < 5%.
- **v10.371 — Per-RM canonical refactor.** rm_profitability.py refactored to use the canonical engine. G256 locks Σ(RM PBT) = Bank PBT.
- **v10.372 — Multi-level bank_targets.json schema extension.** `PBT|<level>|<entity>|<year>` keys for SBU / Branch / RM. Σ(child targets) == parent target. **G253 ratchets** to CONVERGED (<1%).

**Open / partially-closed gaps at v10.367:**

- **FLEXCUBE production deployment** — code is ready (v10.365), requires Apigee gateway access provisioning + env vars + flipping `mode` to `"live"`.
- **Total NFI in compute_bank_aggregates** — still uses legacy formula. Cleanup candidate after v10.369.
- **BSC coverage 2.78% → 100%** — only 40 of 1,439 active staff have BSC actuals. Sustained data-engineering effort.
- **System stocks live wiring** — 6/6 have snapshot accessors but return demo defaults.
...
- **Bridge NPL aging buckets are zeros** — affects v10.364's impairment computation.
- **Hard branch delete UI gap** — deliberate; soft delete is audit-traceable.
- **51 of 75 bank_targets KPIs come from non-CBS sources** — HR/LMS/surveys.
...
*Master prompt v4.11 for v10.367 — Profitability Reconciliation Diagnostic. **Twelfth consecutive batch holding lockstep discipline.** A measurement-first batch in response to Joshua's structural review request: "down top should equal", "showcase SBU profitability", "propositions overlapping", "review the entire structure". Survey revealed FOUR parallel profitability engines (pbt_computation v10.364, sbu_pnl_rollup v10.338, aggregate_cbs_by_branch legacy, customer_profitability v10.65) producing different bank PBTs from different starting points (90%+ divergence on small seed). Ships utils/profitability_reconciliation.py — runs Engine A and Engine B side-by-side, normalizes time horizons (B quarterly × 4 → annual), reports ΔPBT / ΔRevenue / ΔOpEx with named reasons (customer basis differs, revenue source differs, OpEx allocation differs). Module imports allowed = ONLY pbt_computation + sbu_pnl_rollup (its legitimate consumers); G253 enforces. Also ships docs/PROFITABILITY_ARCHITECTURE_REVIEW.md — the structural review document with the five-batch unification arc (v10.368: align data sources, v10.369: SBU dimension, v10.370: per-branch allocator, v10.371: per-RM canonical, v10.372: multi-level bank_targets schema). G253 is INFORMATIONAL — passes whenever diagnostic runs cleanly; delta reported in summary as metric; ratchets in v10.370+ to require ΔPBT <5%, then v10.372 to CONVERGED (<1%). No engine changes in v10.367 — pure measurement + roadmap. Reconciliation identity to be established: Σ(SBU)=Σ(Branch)=Σ(RM)=Bank PBT. Propositions overlap by Rule 6 (existing behavior, preserved — excluded from reconciliation identity). Open design Q1-Q4 documented awaiting Joshua's direction. Audit 253; verifier 226/226; smoke trio still green; tests 15/15 in v10.367 file + 121 prior = 136 total passing. G162 baseline at 4022 (61 consecutive zero-drift batches).*
