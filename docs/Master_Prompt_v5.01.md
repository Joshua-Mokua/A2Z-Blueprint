# A2Z MIS 360 — Master prompt (v5.01)
...
**Current version: v10.457 (May 2026) - Manifest Invariant Hotfix; KeyError 'current_module_key' fixed; G343 locks manifest invariant against future drift.** Per Joshua reported error: 'KeyError current_module_key' at app.py line 900 in _build_dept_pages calling _pg(path, entry["title"], entry["icon"], entry["current_module_key"]). Root cause investigation: v10.448 added pages/82_credit_approvals.py to pages/_manifest.json but never set current_module_key field; v10.454 added pages/85_chief_credit_centre.py (Chief Credit 360 Command Centre with 6 doctrine tabs) but never registered it in pages/_manifest.json at all. The crash hit when navigation tried to build credit-dept pages and encountered the partial 82_credit_approvals entry. v10.457 fixes both: (1) Sets 82_credit_approvals.py current_module_key = 'approvals'; (2) Registers 85_chief_credit_centre.py with current_module_key = 'chief_centre' mirroring 81_chief_hr_centre.py pattern (department_primary='credit' / module_path='credit.chief_centre' / title='Chief Credit — 360 Command Centre' / icon='🏛️'). 130 pages now validated; 0 entries missing required fields. NEW G343 (manifest invariant gate) locks the contract: every manifest entry MUST have title + icon + current_module_key + department_primary. Future page additions cannot ship without complete manifest registration. Health unchanged from v10.456: 5 organs at Admin 74.8% / HR 70.5% / BSC 75.6% / Credit 60.3% / ICT 63.1%; avg 68.9%; 0/5 certified; zero crisis. 360 100% BSC 100% body 91.1% preserved. v10.458+ rescue continues toward CERTIFIED × 5: v10.458 stress test harness + scalability validation; ~74%. v10.459 cross-organ event sync + super users + notification broadcast; ~80%. v10.460 missing credit roles + credit→HR bridge; ~85%. v10.461 final cert: module_revival.md × 5 + capacity_plan.md × 5 toward CERTIFIED × 5 organs.
