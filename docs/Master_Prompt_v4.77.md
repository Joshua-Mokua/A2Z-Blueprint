# A2Z MIS 360 — Master prompt (v4.77)
...
**Current version: v10.434 (May 2026) - New staff onboarding fit-in test.** Per Joshua roadmap: after 360 harmony to 100%, confirm a new staff 'fits in so well'. NEW utils/staff_onboarding_engine.py (~600 LOC) API-first ZERO streamlit. 4 public functions: validate_new_staff, simulate_onboarding, audit_staff_completeness, audit_all_staff_completeness. 5 JSON-serializable dataclasses (ValidationIssue, ValidationResult, OnboardingResult, CompletenessAudit, FullCompletenessAudit). _resolve_canonical_names helper for cascade<->BSC<->library name resolution. EXTENDED bsc_admin_panel render_onboarding_fit_panel with bank-wide audit summary + interactive simulator. EDITED pages/7_admin.py wires panel. NEW 3 FastAPI endpoints /api/v1/onboarding/audit + audit/{code} + simulate. Bank-wide audit findings: 81.8% fully fit (1176/1437), 261 partial (minor BSC gaps), 0 failing; 100% weight invariant; 100% score computable; 99.51% pillar coverage; avg 91.61% role_kpi coverage. The 18.2% partial-fit reflects role_kpis configuration drift (e.g. MD's role_kpis has 12 wrong KPIs while BSC has correct 15). Admin can fix per role_kpis editor. 27th React-ready engine. 360 harmony 100% preserved. BSC rescue health 100% preserved. Engine 0/0/0/0.
