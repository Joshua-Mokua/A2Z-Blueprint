# A2Z MIS 360 — Master prompt (v4.79)
...
**Current version: v10.436 (May 2026) - HR section diagnostic, this body needs rescue.** Per Joshua directive: HR section is in a mess - CIMS+SLA Tracker misplaced, 4 of 5 proper HR pages are stubs, 6 of 8 HR-domain engines unwired into pages. NEW utils/hr_section_audit_engine.py (~700 LOC) API-first ZERO streamlit. 6 audit functions: module_placement (CIMS+SLA misplaced detection), page_completeness (stub vs substantial), engine_wiring (which HR engines unwired), react_readiness (zero-streamlit invariant), api_coverage (FastAPI endpoint presence), data_backing (JSON vs Excel vs PostgreSQL). 8 JSON-serializable dataclasses. HR_DOMAIN_ENGINES constant (8 engines: Std #14-#20 PeerLearning/Coaching/PredictivePerf/Gamification/Efficiency/Wellness + v10.434 onboarding + v10.435 exit). EXTENDED bsc_admin_panel with render_hr_section_audit_panel (also removed duplicate render_exit_risk_panel from prior session). EDITED pages/7_admin.py wires panel. NEW 2 FastAPI endpoints /api/v1/hr-audit/full + dimension/{d}. Initial finding: HR HEALTH 53.0% - rescue required. Audit findings: 2 misplaced pages (CIMS, SLA Tracker), 4 stubs (LMS, PIP, Workforce, Disciplinary), 25% engine wiring, 100% REACT ready, 25% API coverage, 0 PostgreSQL. AUDIT-ONLY batch: v10.437+ executes 6-batch HR Rescue Arc. 28th React-ready engine. 360 harmony 100% preserved. BSC rescue 100%. Engine 0/0/0/0.
