# A2Z MIS 360 — Master prompt (v4.34)
...
**Current version: v10.391 (May 2026) — Target Cascade Deep End-to-End Diagnosis (review-only; sister to v10.385 body-wide diagnosis).** Per Joshua's directive at v10.390 wrap-up: *"do a deep review and test out the target cascade from the MD all the way down the tree to the last staff, confirm if the KPI and weights flow well down as well and that everything connects well from the admin across then lets fix what we can"*. Comprehensive cascade-flow survey across 5 organs: skeleton (org structure), nervous system (cascade flow), recognition (KPI vocabulary), prioritization (weights), and brain (admin/control plane). **31 distinct findings** documented (TC1-TC31, with TC15/TC19 merged into other findings = 29 unique) with severity classifications: **9 CRITICAL** (TC1 MD over-allocates 10×, TC2 MD under bank target 30×, TC6 duplicate MD roles "Chief Executive & Managing Director" + "Managing Director", TC7 synthetic C-suite EXEC-CRO-001/EXEC-CFO-001/etc isolated from cascade, TC8 two parallel staff trees 300xxx vs EXEC-*, TC18 cross-branch cascade FB Towers staff receiving from Kenyatta Avenue managers, TC20 circular cascade MD↔CRBO, TC25 63% of 1051 entries over-allocated, TC26 ratio KPIs summed across subordinates 386× over CASA Ratio 60%→23,160%, TC28 role KPI weights sum 0.45-0.93 not 1.0, TC29 ~76% of role_kpis don't resolve in library). **8 HIGH** (TC3 three parallel KPI naming conventions, TC11 K-prefix vs uppercase mismatch, TC14 two MDs use different KPI vocabularies, TC17 Branch Credit Manager phantom in canonical missing in data, TC21 BOM gets cascade from wrong branch BM, TC22 multi-sender ambiguity, TC23 broken chain depth, TC24 canonical hierarchy ≠ data, TC30 Branch Manager+Teller have identical KPIs). **8 MEDIUM** and **2 LOW**. Cross-organ interactions documented: two-staff-universes (TC7×TC8×TC6), ratio-summing (TC26×TC1), vocabulary-fracture (TC3×TC11×TC29), weights≠1.0 (TC28×TC29). Prioritized fix sequence **v10.392-v10.404** in 4 tiers: T1 cascade engine + structure + MD resolution + re-cascade + weight normalization (v10.392-v10.396); T2 vocabulary consolidation (v10.397-v10.399); T3 coverage expansion (v10.400-v10.402); T4 canonical alignment (v10.403-v10.404). **6 Joshua decisions required to unblock progress** (C1 canonical MD role, C2 canonical hierarchy direction, C3 Branch Credit Manager populate-or-delete, C4 canonical KPI ID scheme, C5 ratio KPI semantics, C6 weight renormalization approach). Diagnosis traces real Teller (Ishmael 300230 @ Kenyatta Avenue) up to MD; trace works mechanically (5 hops) but produces wrong outputs (cross-branch detour, circular at top, ratio-aggregation downstream). 12 integration tests verify diagnosis claims by probing live data. **REVIEW ONLY** — zero code/data/gate changes beyond the diagnosis doc + G276 verification gate + tests; v10.392+ executes fixes. Body-system framing: cascade = endocrine system. Ships docs/TARGET_CASCADE_DEEP_DIAGNOSIS_v10.391.md (11 Parts, ~24KB).
