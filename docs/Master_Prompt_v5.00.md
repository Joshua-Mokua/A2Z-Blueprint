# A2Z MIS 360 — Master prompt (v5.00)
...
**Current version: v10.456 (May 2026) - Flexcube Integration Readiness Facade + ICT Module as Lungs (5th organ); avg health across 5 organs 68.9%.** Per Joshua: 'on flexcube and any other build it is important to first review across the whole system what is existing since had done pretty much of builds only that they are scattered, we have a whole ICT Modules where several modules are and at this point i just remembered we have not lined it up for rescue and we should mark it as our lungs. There is a flexucube infrastructure and we even had mapped a second level of system admin from the supper user to come from ICT.' Plus: 'the bank we are targeting is using Flexcube as their core banking, the system we are putting together is not meant to replace flexcube but use it as one of its biggest data sources through read only integration for real time, or daily uploads or the bank may have a data warehouse but we need to demonstrate that we are 100% flexcube integration ready. the idea is not to integrate each module separately but have a single integration that shall serve all the modules that would require flexcube data.' v10.456 DISCOVERS and WIRES (doesn't rebuild). Investigation revealed substantial existing Flexcube infrastructure: utils/flexcube_adapter.py (1729 LOC, 12 fetchers, 5 aggregators, 3 modes synthetic/mock/live, circuit breaker, retry telemetry); utils/flexcube_connection.py (358 LOC); utils/flexcube_mappings.py (304 LOC); utils/flexcube_staging.py (281 LOC); pages/86_flexcube.py (324 LOC); virtual bank simulator + readiness + core + writer + KPI unifier + seeder (whole sub-system). Plus ICT pages already exist: 6_integrate / 50_cybersecurity / 72_observability / 86_flexcube / 91_systems_view / 96_it_digital_pt1 / 97_it_digital_pt2 / 98_platform_health / 119_platform_hub; ICT engines: it_api_gateway / it_cbk_compliance / it_cicd / it_cloud_architecture / it_data_encryption / it_digital_banking / it_disaster_recovery / it_itsm / it_multi_tenancy / it_observability (10 engines). NEW utils/flexcube_integration_readiness.py - thin facade with 7 domains (credit/customer/deposits/branch/staff/treasury/risk) + 4 public APIs (probe_flexcube_readiness / declare_flexcube_ready / get_integration_status / get_data_source_for) + ReadinessReport + IntegrationCoverageAudit dataclasses. Probe returns 100% integration readiness score (adapter 1729 LOC + 12 fetchers + 5 aggregators + circuit breaker + retry + virtual bank harness + config + sample data all present). Each Chief Centre (Credit, HR, BSC) imports the facade once via declare_flexcube_ready() - Admin already had Flexcube refs. ICT added to MODULE_REGISTRY as 5th organ (Lungs per Document 2 organ analogy - 'System-wide Oxygen Exchange'): 9 pages + 18 engines + ICT Super User role per Joshua 2nd-level admin doctrine. ICT doctrine docs generated (22 docs). Health: Admin 74.8% / HR 70.5% (+2.1pp from Phase 3 Flexcube ref) / BSC 75.6% (+2pp) / Credit 60.3% / **ICT 63.1% NEW**. Avg 5-organ 68.9%. Cert criteria: Admin 9, HR 9, BSC 8, Credit 5, ICT 7. Zero crisis. Doctrine criterion #6 (Flexcube compatibility) now passes for all 4 original organs. Body health 91.1%; 360 100%; BSC rescue 100% preserved; engine 0/0/0/0.
