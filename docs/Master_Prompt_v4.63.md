# A2Z MIS 360 — Master prompt (v4.63)
...
**Current version: v10.420 (May 2026) - KPI library dedup migration.** Per Joshua's locked backlog: 4 KPI alias pairs marked as duplicates in _v10403_dedup_pending (v10.403) but never actually merged. v10.420 ships the consolidation. NEW utils/kpi_dedup_engine.py (~280 LOC) API-first (ZERO streamlit imports). Public API: audit_kpi_dedup (returns DedupAudit with per-pair AliasPairAudit reference counts across role_kpis + kpi_weights + kpis list + bank_targets), migrate_dedup_kpi_library (consolidates all references; integrates v10.419 by clearing + re-migrating role_normalized_weights). 3 dataclasses JSON-serializable. KPI_ALIAS_PAIRS constant. Migration is IDEMPOTENT. Live migration: pre 4/4 pending -> post 0/4 pending. Consolidated NEW_ACCOUNTS->K006, K069->K024, K048->K028, NIM->NET_INTEREST_MARGIN; 17 role lists updated, 4 KPI definitions removed, 2 kpi_weights entries removed, role_normalized_weights re-migrated. NEW scripts/dedup_kpi_library.py runner with --dry-run. NEW 2 FastAPI endpoints in utils/api.py: GET /api/v1/kpi-dedup/audit, POST /api/v1/kpi-dedup/migrate. v10.403 test + G289 made forward-compatible to accept either pre-v10.420 (marked but not merged) or post-v10.420 (physically consolidated) library state. v10.390-era verifier check on NIM also made forward-compat (accepts NET_INTEREST_MARGIN as canonical replacement). Engine preserved 0/0/0/0.
