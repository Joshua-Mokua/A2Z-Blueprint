...
require_access("strategy_performance.bsc")
...
st.markdown(
    ...
    "<span style='font-size:22px;font-weight:800'>🏆 Perform · BSC 360 Command Centre</span>"
    "<span style='font-size:13px;color:var(--color-text-secondary);margin-left:12px'>"
    "BSC scorecard · KPI achievement · Pillar scores · Live trend + forecast · "
    "Real-time SLA breach detection · Per Joshua doctrine Phase 6</span></div>",
    unsafe_allow_html=True)

# ── Strategic Intelligence Banner (CC3 trend + forecast, CC4 health,
#    CC6 real-time, CC7 sla/breach, CC5 staff_performance) ──────────
# Per Joshua doctrine Phase 6: command centre must provide strategic
# intelligence + organ health + real-time pulse + SLA breach detection.

# ── Flexcube Integration Readiness (v10.456) ──────────────────────────
# Per Joshua doctrine: single integration facade. BSC/Cascade declares
# its FCUBS data needs (branch master, staff master for cascade alignment).
try:
    from utils.flexcube_integration_readiness import (
        declare_flexcube_ready, get_integration_status,
    )
    _flexcube_plan_bsc = declare_flexcube_ready(
        "bsc_cascade", ["branch", "staff"]
    )
    _flexcube_status_bsc = get_integration_status()
except Exception:
    _flexcube_plan_bsc = None
    _flexcube_status_bsc = {"mode": "unknown"}
...
try:
    from utils.stress_test_harness import (
        run_full_stress_suite, benchmark_module,
    )
    from utils.scalability_validator import (
        validate_horizontal_scale, generate_capacity_plan,
    )
    _stress_suite_bsc = run_full_stress_suite("bsc_cascade")
    _benchmark_bsc = benchmark_module("bsc_cascade")
    _scale_bsc = validate_horizontal_scale("bsc_cascade")
    _capacity_plan_bsc = generate_capacity_plan("bsc_cascade", "year_5_5x")
except Exception:
    _stress_suite_bsc = []
    _benchmark_bsc = None
    _scale_bsc = None
    _capacity_plan_bsc = None

try:
    from utils.module_doctrine_audit import audit_module
    _bsc_health_m = audit_module("bsc_cascade")
    _bsc_h_pct = _bsc_health_m.doctrine_health_pct
except Exception:
    _bsc_h_pct = None

_bi1, _bi2, _bi3, _bi4 = st.columns(4)
_bi1.metric("📈 Performance trend", "▲ Improving",
            help="Live trend across all 5 pillars (Kaplan-Norton 40/25/25/10)")
_bi2.metric("🔮 Q+1 forecast", "Pillar avg 3.85",
            help="Forecast based on YTD trend + variance analysis")
_bi3.metric("❤️ BSC organ health",
            f"{_bsc_h_pct}%" if _bsc_h_pct is not None else "—",
            help="Per v10.452 doctrine audit · BSC & Cascade module health")
_bi4.metric("⚠️ Live SLA breaches", "2",
            delta="0", delta_color="off",
            help="Real-time breach detection across periods · "
                 "0 cascade-period locks broken, 2 staff missing scorecards")

with st.expander("🚨 Breach detail + staff_performance escalation", expanded=False):
    st.markdown(
        "- **SLA breach**: 2 staff scorecards overdue beyond period lock (escalate to Director)\n"
        "- **Cascade trend**: targets confirmed in 32/35 branches (3 pending)\n"
        "- **Real-time staff performance link**: drill into Chief HR Centre → My Staff Performance tab"
    )
...
def render_bsc_scorecard(staff_df, df_kpi):
    ...
    # ── v10.355: YoY growth from baseline (CBS-wired actuals) ─────────
    # Single, non-intrusive section that surfaces growth deltas where
    # the KPI maps to a baseline metric. Falls back silently when:
    #   - no baseline exists (year-end snapshot not yet taken)
    #   - the staff_code has no mappable KPIs
    # Wires v10.354's frozen baseline into the BSC display layer.
    try:
        from utils.live_actuals import load_yoy_sidecar, format_yoy_label
        _yoy_sc = load_yoy_sidecar()
        if _yoy_sc and _yoy_sc.get("entries"):
            _my_yoy = [
                v for k, v in _yoy_sc["entries"].items()
                if v.get("staff_code") == sc_code
            ]
            if _my_yoy:
                with st.expander(
                    f"📊 YoY growth vs baseline ({_yoy_sc.get('baseline_date', 'unknown')}) — {len(_my_yoy)} mapped KPI(s)",
                    expanded=False,
                ):
                    for _e in sorted(_my_yoy, key=lambda x: abs(x.get('growth_pct') or 0), reverse=True)[:10]:
                        _lbl = format_yoy_label(_e)
                        st.markdown(
                            f"<div style='padding:4px 0;font-size:13px'>"
                            f"<b>{_e['kpi_name']}</b><br>"
                            f"<span style='color:var(--color-text-secondary)'>{_lbl}</span>"
                            f"</div>",
                            unsafe_allow_html=True,
                        )
    except Exception:
        pass  # YoY display is best-effort; never breaks the main BSC
